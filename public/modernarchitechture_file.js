...
createGallery() {
    ...
    const roomWidth = 40;
    const roomDepth = 40;
    const roomHeight = 9; // Extra tall for grandeur
    
    // ========================================
    // PREMIUM ARCHITECTURAL MATERIALS
    // ========================================
    
    // Polished Carrara marble floor
    ...
    marbleTextureObj.repeat.set(8, 8);
    marbleFloorMaterial.map = marbleTextureObj;
    
    // Warm oak wood accents
    ...
    // Textured gallery walls (warm white)
    ...
    // Floating ceiling panels (light gray)
    ...
    // ========================================
    // POLISHED MARBLE FLOOR WITH INLAY PATTERN
    // ========================================
    ...
    // Brass inlay strips (geometric pattern)
    const inlayPositions = [-12, -6, 0, 6, 12];
    inlayPositions.forEach(pos => {
        const inlayX = new THREE.Mesh(
            new THREE.BoxGeometry(roomWidth * 0.9, 0.02, 0.08),
            brassAccentMaterial
        );
        inlayX.position.set(0, 0.01, pos);
        inlayX.receiveShadow = true;
        room1.add(inlayX);
        
        const inlayZ = new THREE.Mesh(
            new THREE.BoxGeometry(0.08, 0.02, roomDepth * 0.9),
            brassAccentMaterial
        );
        inlayZ.position.set(pos, 0.01, 0);
        inlayZ.receiveShadow = true;
        room1.add(inlayZ);
    });
    
    // Circular center medallion
    const medallion = new THREE.Mesh(
        new THREE.CylinderGeometry(2.5, 2.5, 0.05, 64),
    ...
    // ========================================
    // CURVED FEATURE WALLS WITH WOOD PANELING
    // ========================================
    
    // Create curved wall segments using multiple boxes
    const createCurvedWall = (centerX, centerZ, radius, startAngle, endAngle, height) => {
        const segments = 20;
        ...
        for (let i = 0; i < segments; i++) {
            ...
            const segment = new THREE.Mesh(
                new THREE.BoxGeometry(1.2, height, 0.3),
                wallMaterial
            );
            ...
            if (i % 3 === 0) {
                const woodPanel = new THREE.Mesh(
                    new THREE.BoxGeometry(1.0, height * 0.6, 0.15),
                    woodMaterial
                );
                woodPanel.position.set(
                    x + Math.cos(angle) * 0.2,
                    height / 2,
                    z + Math.sin(angle) * 0.2
                );
                woodPanel.rotation.y = angle + Math.PI / 2;
    ...
    // Left curved wall
    createCurvedWall(-8, 0, 12, -Math.PI / 2, Math.PI / 2, roomHeight);
    
    // Right curved wall
    createCurvedWall(8, 0, 12, Math.PI / 2, 3 * Math.PI / 2, roomHeight);
    
    // Straight back wall with vertical wood slats
    for (let x = -18; x <= 18; x += 1.5) {
        const slat = new THREE.Mesh(
            new THREE.BoxGeometry(0.12, roomHeight * 0.85, 0.25),
            woodMaterial
        );
        slat.position.set(x, roomHeight * 0.425, -19.5);
    ...
    backWall.position.set(0, roomHeight / 2, -20);
    backWall.receiveShadow = true;
    room1.add(backWall);
    
    // ========================================
    // FLOATING CEILING PANELS & SKYLIGHTS
    // ========================================
    
    // Main ceiling with recessed lighting coves
    const mainCeiling = new THREE.Mesh(
        new THREE.BoxGeometry(roomWidth, 0.25, roomDepth),
        ceilingMaterial
    );
    ...
    panelPositions.forEach(pos => {
        ...
        panel.position.set(pos.x, roomHeight - 1.5, pos.z);
        panel.castShadow = true;
        panel.receiveShadow = true;
        room1.add(panel);
        
        // LED edge lighting
        const edgeLight = new THREE.PointLight(0xffe6cc, 1.2, 8);
        edgeLight.position.set(pos.x, roomHeight - 1.6, pos.z);
        room1.add(edgeLight);
    });
    
    // Simulated skylights (glowing panels)
    ...
    skylightPositions.forEach(pos => {
        ...
        skylight.position.set(pos.x, roomHeight - 0.1, pos.z);
        room1.add(skylight);
        
        // Skylight illumination
        const skylightLight = new THREE.RectAreaLight(0xffffee, 3, 4, 8);
        skylightLight.position.set(pos.x, roomHeight - 0.5, pos.z);
        skylightLight.rotation.x = -Math.PI / 2;
        room1.add(skylightLight);
    });
    
    // ========================================
    // GLASS PARTITIONS & ARCHITECTURAL COLUMNS
    // ========================================
    
    // Elegant brass-framed glass partitions
    ...
    partitionPositions.forEach(pos => {
        ...
        const glass = new THREE.Mesh(
            new THREE.BoxGeometry(0.08, roomHeight * 0.7, 4),
            glassMaterial
        );
        glass.position.set(pos.x, roomHeight * 0.35, pos.z);
        ...
        const frame = new THREE.Mesh(
            new THREE.BoxGeometry(0.12, roomHeight * 0.7, 0.12),
            brassAccentMaterial
        );
    ...
    columnPositions.forEach(pos => {
        const column = new THREE.Mesh(
            new THREE.CylinderGeometry(0.4, 0.5, roomHeight, 24),
            new THREE.MeshStandardMaterial({
                color: 0xf0f0f0,
                roughness: 0.5,
                metalness: 0.2
        ...
        column.position.set(pos.x, roomHeight / 2, pos.z);
        ...
        const capital = new THREE.Mesh(
            new THREE.CylinderGeometry(0.6, 0.4, 0.4, 24),
            brassAccentMaterial
        );
        capital.position.set(pos.x, roomHeight - 0.2, pos.z);
        room1.add(capital);
    });
    
    // ========================================
    // DYNAMIC TRACK LIGHTING SYSTEM
    // ========================================
    
    this.glassSpotlights = [];
    
    // Sophisticated track system
    const trackConfigs = [
        { x: -12, z: 0, length: roomDepth * 0.85 },
        { x: -6, z: 0, length: roomDepth * 0.85 },
        { x: 0, z: 0, length: roomDepth * 0.85 },
        { x: 6, z: 0, length: roomDepth * 0.85 },
        { x: 12, z: 0, length: roomDepth * 0.85 }
    ];
    
    trackConfigs.forEach(track => {
        // Sleek aluminum track
        const rail = new THREE.Mesh(
            new THREE.BoxGeometry(1.5, 0.12, track.length),
            new THREE.MeshStandardMaterial({
                color: 0xa8a8a8,
                roughness: 0.25,
                metalness: 0.9
        ...
        rail.position.set(track.x, roomHeight - 0.4, 0);
        rail.castShadow = true;
        room1.add(rail);
        
        // Adjustable spotlights (8 per track)
        for (let i = -3.5; i <= 3.5; i += 1) {
            // Modern cylindrical fixture with brass accent
            const fixtureBody = new THREE.Mesh(
                new THREE.CylinderGeometry(0.1, 0.14, 0.3, 16),
                new THREE.MeshStandardMaterial({
                    color: 0x2a2a2a,
                    roughness: 0.3,
                    metalness: 0.8
            ...
            fixtureBody.position.set(track.x, roomHeight - 0.6, i * 5);
            ...
            brassRing.position.copy(fixtureBody.position);
            ...
            lens.position.set(track.x, roomHeight - 0.85, i * 5);
            room1.add(lens);
            
            // Warm spotlight with soft penumbra
            ...
            spotlight.position.set(track.x, roomHeight - 0.9, i * 5);
            spotlight.target.position.set(track.x, 0, i * 5);
    ...
    // ========================================
    // LED WALL WASHERS & ACCENT LIGHTING
    // ========================================
    ...
    washerPositions.forEach(pos => {
        ...
        // Warm wash light
        const washLight = new THREE.SpotLight(0xffd699, 1.8, 15, Math.PI / 3.5, 0.7);
        washLight.position.set(pos.x, pos.y, pos.z);
        washLight.target.position.set(pos.x, pos.y - 3, pos.z + 3);
        room1.add(washLight);
        room1.add(washLight.target);
    });
    
    // Color-changing LED strips (floor cove lighting)
    const ledStripPositions = [
        { start: new THREE.Vector3(-18, 0.1, -18), end: new THREE.Vector3(18, 0.1, -18) },
        { start: new THREE.Vector3(-18, 0.1, 18), end: new THREE.Vector3(18, 0.1, 18) }
    ];
    
    ledStripPositions.forEach(strip => {
        const points = [strip.start, strip.end];
        ...
        // Point lights along the strip
        for (let t = 0; t <= 1; t += 0.1) {
            const pos = new THREE.Vector3().lerpVectors(strip.start, strip.end, t);
            const ledLight = new THREE.PointLight(0x4a90e2, 0.8, 6);
    ...
    for (let i = 0; i < 3; i++) {
        const step = new THREE.Mesh(
            new THREE.BoxGeometry(14 - i * 2, 0.15, 2),
            platformMaterial
        );
        step.position.set(0, i * 0.15 + 0.075, -4 + i * 2);
        ...
        const trim = new THREE.Mesh(
            new THREE.BoxGeometry(14 - i * 2, 0.03, 0.05),
            brassAccentMaterial
        );
        trim.position.set(0, i * 0.15 + 0.15, -3 + i * 2);
        room1.add(trim);
    }
    
    // ========================================
    // LIVING GREEN WALL (Plant Installation)
    // ========================================
    
    const createGreenWall = (x, z, width, height) => {
        // Backing panel
        ...
        backing.position.set(x, height / 2, z);
        backing.receiveShadow = true;
        room1.add(backing);
        
        // Add "foliage" using small spheres
    ...
    createGreenWall(0, 18, 8, 6);
    
    // ========================================
    // DESIGNER SEATING PODS
    // ========================================
    ...
    podPositions.forEach(pos => {
        ...
        const cushion = new THREE.Mesh(
            new THREE.SphereGeometry(0.9, 32, 32, 0, Math.PI * 2, 0, Math.PI / 3),
            new THREE.MeshStandardMaterial({
                color: 0x4a90e2,
                roughness: 0.85,
                metalness: 0.05
    ...
    // Geometric rings sculpture
    for (let i = 0; i < 5; i++) {
        const ring = new THREE.Mesh(
            new THREE.TorusGeometry(0.8 + i * 0.3, 0.08, 16, 32),
            new THREE.MeshStandardMaterial({
                color: 0xc0c0c0,
                roughness: 0.2,
                metalness: 0.9,
    ...
    const sculptureLight = new THREE.SpotLight(0xffffff, 4.0, 15, Math.PI / 8, 0.3);
    sculptureLight.position.set(0, roomHeight - 1, 0);
    sculptureLight.target.position.set(0, 2, 0);
    sculptureLight.castShadow = true;
    room1.add(sculptureLight);
    room1.add(sculptureLight.target);
    ...
    // Hidden cove lights
    const covePositions = [
        new THREE.Vector3(-18, roomHeight - 0.5, 0),
        new THREE.Vector3(18, roomHeight - 0.5, 0),
        new THREE.Vector3(0, roomHeight - 0.5, -18),
        new THREE.Vector3(0, roomHeight - 0.5, 18)
    ];
    
    covePositions.forEach(pos => {
        const coveLight = new THREE.PointLight(0xffeedd, 1.5, 20);
        coveLight.position.copy(pos);
        room1.add(coveLight);
    });
    ...
    console.log("✨ Modern architectural gallery with premium aesthetics created");
}
