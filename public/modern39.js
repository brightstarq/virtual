...
createGallery() {
    // ========================================
    // BRUTALIST PARKING GARAGE SYSTEM
    // ========================================
    
    this.garageLevel = 0; // Start on ground level (0-4)
    this.parkingSpots = [];
    this.supportColumns = [];
    this.fluorescents = [];
    ...
    this.createRooftopView();
   
    
    console.log("🚗 Brutalist Parking Garage loaded!");
}

// ========================================
// GARAGE STRUCTURE (raw concrete)
// ========================================

createGarageStructure() {
    const garageRoom = new THREE.Group();
    garageRoom.visible = true;
    
    const levelWidth = 60;
    const levelLength = 80;
    const levelHeight = 3.5;
    const numLevels = 5; // 0-4 (ground to rooftop)
    
    // ========================================
    // MATERIALS (brutalist concrete)
    // ========================================
    ...
    for (let level = 0; level < numLevels; level++) {
        ...
        const floor = new THREE.Mesh(
            new THREE.PlaneGeometry(levelWidth, levelLength),
            concreteMaterial
        );
        ...
        // Concrete texture (roughness variation)
        for (let i = 0; i < 100; i++) {
            ...
            patch.position.set(
                (Math.random() - 0.5) * levelWidth * 0.9,
                yPos + 0.01,
                (Math.random() - 0.5) * levelLength * 0.9
            );
            levelGroup.add(patch);
        }
        
        // Ceiling (except top level - that's rooftop)
        if (level < numLevels - 1) {
            ...
            ceiling.position.y = yPos + levelHeight;
            ceiling.receiveShadow = true;
            levelGroup.add(ceiling);
            
            // Exposed rebar and structural elements
            for (let i = 0; i < 20; i++) {
                const rebar = new THREE.Mesh(
                    new THREE.CylinderGeometry(0.03, 0.03, 0.3 + Math.random() * 0.5, 6),
                    new THREE.MeshStandardMaterial({
                        color: 0x8a4a2a,
                        roughness: 0.8,
                        metalness: 0.6
                ...
                rebar.position.set(
                    ...
                    yPos + levelHeight - 0.2,
                    (Math.random() - 0.5) * levelLength * 0.8
                );
                levelGroup.add(rebar);
            }
        }
        ...
        // Side barriers (low concrete walls)
        [-levelWidth/2, levelWidth/2].forEach((x, index) => {
            const barrier = new THREE.Mesh(
                new THREE.BoxGeometry(0.3, 1.2, levelLength),
                concreteMaterial
            );
            barrier.position.set(x, yPos + 0.6, 0);
            barrier.castShadow = true;
            levelGroup.add(barrier);
        });
        
        levelGroup.userData.level = level;
        garageRoom.add(levelGroup);
    }
    
    // ========================================
    // ATMOSPHERIC LIGHTING (dim, oppressive)
    // ========================================
    
    const ambientLight = new THREE.AmbientLight(0x332211, 0.2);
    garageRoom.add(ambientLight);
    
    // Night sky glow from above
    const skyLight = new THREE.DirectionalLight(0x1a1a2a, 0.3);
    skyLight.position.set(0, 20, 0);
    garageRoom.add(skyLight);
    
    // Fog (urban smog)
    this.scene.fog = new THREE.FogExp2(0x1a1a1a, 0.01);
    
    // ========================================
    // ARTWORK DISPLAY LOCATIONS (on columns)
    // ========================================
    
    this.garageArtworkSpots = [];
    
    garageRoom.position.set(0, 0, 0);
    this.rooms.push(garageRoom);
    this.scene.add(garageRoom);
}

// ========================================
// SUPPORT COLUMNS (for spray paint art)
// ========================================
...
createSupportColumns() {
    ...
    // Grid of columns (every 15 units)
    for (let x = -20; x <= 20; x += 15) {
        for (let z = -30; z <= 30; z += 20) {
            if (Math.abs(x) < 5 && Math.abs(z) < 10) continue; // Leave center open
    ...
    columnPositions.forEach((pos, index) => {
        ...
        // Main column (extends through all levels)
        const column = new THREE.Mesh(
            new THREE.BoxGeometry(0.8, numLevels * 3.5, 0.8),
            columnMaterial
        );
        column.position.set(pos.x, (numLevels * 3.5) / 2, pos.z);
        ...
        for (let i = 0; i < 5; i++) {
            const crack = new THREE.Mesh(
                new THREE.PlaneGeometry(0.05, 1 + Math.random() * 2),
                new THREE.MeshStandardMaterial({
                    color: 0x1a1a1a,
                    roughness: 1.0
                })
            ...
            crack.position.set(
                pos.x + 0.41,
                Math.random() * numLevels * 3.5,
                pos.z
            );
            crack.rotation.y = Math.PI / 2;
            columnGroup.add(crack);
        }
        
        // Column number (stenciled)
        ...
        numberSign.position.set(pos.x, 2, pos.z + 0.41);
        columnGroup.add(numberSign);
        
        // Add artwork spot (spray paint canvas)
        this.garageArtworkSpots.push({
            x: pos.x,
            y: 2,
            z: pos.z + 0.5,
            rot: 0,
        ...
        this.rooms[0].add(columnGroup);
        this.supportColumns.push(columnGroup);
    });
}

// ========================================
// PARKING LEVELS (level indicators)
// ========================================

createParkingLevels() {
    const levelNames = ['P1', 'P2', 'P3', 'P4', 'ROOF'];
    
    for (let level = 0; level < 5; level++) {
        const yPos = level * 3.5;
        
        // Level indicator sign (on wall)
        const canvas = document.createElement('canvas');
        canvas.width = 512;
        canvas.height = 256;
        const ctx = canvas.getContext('2d');
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(levelNames[level], 256, 128);
        ...
        sign.position.set(28, yPos + 2, -35);
...
createCarRamps() {
    ...
    for (let level = 0; level < 4; level++) {
        ...
        const startY = level * 3.5;
        const endY = (level + 1) * 3.5;
        ...
        // Ramp markings (yellow stripes)
        for (let i = 0; i < 10; i++) {
            const stripe = new THREE.Mesh(
                new THREE.BoxGeometry(0.3, 0.31, 0.8),
                new THREE.MeshBasicMaterial({
                    color: 0xffff00,
                    emissive: 0xffaa00,
                    emissiveIntensity: 0.3
            ...
            stripe.position.set(
                25,
                (startY + endY) / 2 + 0.16,
                -20 + level * 10 - rampLength/2 + i * 1.5
            );
            stripe.rotation.x = rampAngle;
            this.rooms[0].add(stripe);
        }
        
        this.rooms[0].add(ramp);
        this.carRamps.push(ramp);
    }
}

// ========================================
// FLUORESCENT LIGHTS (flickering, buzzing)
// ========================================
...
createFluorescentLights() {
    ...
    for (let level = 0; level < 4; level++) {
        const yPos = level * 3.5 + 3.2;
        
        // Grid of lights (sparse, some broken)
        for (let x = -25; x <= 25; x += 10) {
            for (let z = -35; z <= 35; z += 15) {
                // Random chance of light being broken
                const isBroken = Math.random() > 0.7;
                
                // Fluorescent fixture
                const fixture = new THREE.Mesh(
                    new THREE.BoxGeometry(2, 0.15, 0.3),
                    new THREE.MeshStandardMaterial({
                        color: 0x888888,
                        roughness: 0.6,
                        metalness: 0.4
                ...
                fixture.position.set(x, yPos, z);
                this.rooms[0].add(fixture);
                ...
                if (!isBroken) {
                    ...
                    tube.position.set(x, yPos - 0.08, z);
                    this.rooms[0].add(tube);
                    
                    // Point light
                    const light = new THREE.PointLight(0xffffee, 1.5, 12);
                    light.position.set(x, yPos - 0.2, z);
                    ...
                    brokenTube.position.set(x, yPos - 0.08, z);
                    this.rooms[0].add(brokenTube);
                }
            }
        }
    }
...
// ========================================
// PARKING SPACES (numbered, some occupied)
// ========================================
...
createParkingSpaces() {
    ...
    for (let level = 0; level < 4; level++) {
        const yPos = level * 3.5 + 0.01;
        let spaceNumber = level * 20 + 1;
        
        // Rows of parking spaces
        for (let row = 0; row < 4; row++) {
            for (let spot = 0; spot < 5; spot++) {
                const x = -20 + spot * (spaceWidth + 0.5);
                const z = -30 + row * (spaceLength + 1);
                
                // Parking line (white)
                const line = new THREE.Mesh(
                    new THREE.PlaneGeometry(spaceWidth, 0.1),
                    new THREE.MeshBasicMaterial({
                        color: 0xffffff,
                        transparent: true,
                        opacity: 0.7
                ...
                ctx.textAlign = 'center';
                ctx.fillText(spaceNumber, 64, 45);
                ...
                numSign.rotation.x = -Math.PI / 2;
                numSign.position.set(x, yPos + 0.01, z + spaceLength/2 - 0.5);
                this.rooms[0].add(numSign);
...
// ========================================
// ELEVATOR SHAFT (access between levels)
// ========================================
...
createElevatorShaft() {
    ...
    const shaftWidth = 3;
    const shaftDepth = 3;
    const shaftHeight = 5 * 3.5; // All levels
    
    const shaftPosition = { x: -25, z: 30 };
    ...
    [-shaftWidth/2, shaftWidth/2].forEach(offset => {
        const wall = new THREE.Mesh(
            new THREE.BoxGeometry(0.2, shaftHeight, shaftDepth),
            metalMaterial
        );
        wall.position.set(shaftPosition.x + offset, shaftHeight/2, shaftPosition.z);
        shaftGroup.add(wall);
    });
    
    [-shaftDepth/2, shaftDepth/2].forEach(offset => {
        const wall = new THREE.Mesh(
            new THREE.BoxGeometry(shaftWidth, shaftHeight, 0.2),
            metalMaterial
        );
        wall.position.set(shaftPosition.x, shaftHeight/2, shaftPosition.z + offset);
        shaftGroup.add(wall);
    });
    
    // Elevator doors on each level
    for (let level = 0; level < 5; level++) {
        const yPos = level * 3.5;
        
        // Door frame
        const frame = new THREE.Mesh(
            new THREE.BoxGeometry(2.2, 2.3, 0.15),
            new THREE.MeshStandardMaterial({
                color: 0x3a3a3a,
                roughness: 0.5,
                metalness: 0.7
        ...
        frame.position.set(shaftPosition.x, yPos + 1.15, shaftPosition.z - shaftDepth/2 - 0.1);
        shaftGroup.add(frame);
        
        // Doors (slightly open/jammed)
        ...
        [-0.55, 0.55].forEach((offset, index) => {
            ...
            door.position.set(
                shaftPosition.x + offset + (index === 0 ? -0.1 : 0.1),
                yPos + 1.1,
                shaftPosition.z - shaftDepth/2 - 0.15
            );
            shaftGroup.add(door);
        });
        
        // Level indicator (broken LCD)
        const indicator = new THREE.Mesh(
            new THREE.PlaneGeometry(0.4, 0.4),
            new THREE.MeshStandardMaterial({
                color: Math.random() > 0.5 ? 0xff0000 : 0x111111,
                emissive: Math.random() > 0.5 ? 0xff0000 : 0x000000,
                emissiveIntensity: 0.5
            })
        );
        indicator.position.set(shaftPosition.x, yPos + 2.5, shaftPosition.z - shaftDepth/2 - 0.05);
        shaftGroup.add(indicator);
        
        // Call buttons
        [-0.3, 0.3].forEach(buttonOffset => {
            const button = new THREE.Mesh(
                new THREE.CylinderGeometry(0.05, 0.05, 0.05, 8),
                new THREE.MeshStandardMaterial({
                    color: buttonOffset < 0 ? 0x00ff00 : 0xff0000,
                    emissive: buttonOffset < 0 ? 0x00aa00 : 0xaa0000,
                    emissiveIntensity: 0.3
                })
            );
            ...
            button.position.set(
                shaftPosition.x + 1.3,
                yPos + 1.5 + buttonOffset,
                shaftPosition.z - shaftDepth/2 - 0.05
            );
            shaftGroup.add(button);
        });
    }
    
    // Cables hanging inside shaft
    for (let i = 0; i < 4; i++) {
        const cable = new THREE.Mesh(
            new THREE.CylinderGeometry(0.03, 0.03, shaftHeight, 8),
            new THREE.MeshStandardMaterial({
                color: 0x2a2a2a,
                roughness: 0.8,
                metalness: 0.6
        ...
        cable.position.set(
            shaftPosition.x + (Math.random() - 0.5) * shaftWidth * 0.6,
            shaftHeight/2,
            shaftPosition.z + (Math.random() - 0.5) * shaftDepth * 0.6
        );
        shaftGroup.add(cable);
    }
    
    // Dim shaft light
    const shaftLight = new THREE.PointLight(0xff0000, 0.5, 10);
    shaftLight.position.set(shaftPosition.x, shaftHeight - 2, shaftPosition.z);
    shaftGroup.add(shaftLight);
    
    this.elevatorShaft = shaftGroup;
    this.rooms[0].add(shaftGroup);
}

// ========================================
// URBAN DETAILS (oil stains, graffiti, debris)
// ========================================
...
createGraffiti() {
    const graffiti = [
        { text: "VOID", color: 0xff0066 },
        { text: "EXIT", color: 0x00ff88 },
        { text: "NO FUTURE", color: 0xffaa00 },
        { text: "DRIVE", color: 0x00aaff },
        { text: "★", color: 0xff00ff }
    ];
    
    // Spray paint on columns and walls
    this.supportColumns.forEach((column, index) => {
        if (Math.random() > 0.4) {
            const graf = graffiti[Math.floor(Math.random() * graffiti.length)];
            ...
            // Spray paint effect (rough edges)
            ctx.fillStyle = `#${graf.color.toString(16).padStart(6, '0')}`;
            ...
            ctx.fillText(graf.text, 256, 120);
            
            const texture = new THREE.CanvasTexture(canvas);
            const grafMesh = new THREE.Mesh(
            ...
            grafMesh.position.set(
                column.children[0].position.x + 0.45,
                1 + Math.random() * 4,
                column.children[0].position.z
            );
            grafMesh.rotation.y = Math.PI / 2;
            
            this.rooms[0].add(grafMesh);
            this.sprayPaintArt.push(grafMesh);
        }
    });
}
...
createDebris() {
    for (let level = 0; level < 4; level++) {
        ...
        if (level === 1) {
            const cart = this.createShoppingCart();
            cart.position.set(15, yPos, -25);
            cart.rotation.y = Math.PI / 4;
            this.rooms[0].add(cart);
        }
    }
}
...
createShoppingCart() {
    ...
    // Basket (wire mesh approximation)
    const basket = new THREE.Mesh(
        new THREE.BoxGeometry(0.75, 0.75, 1.15),
        new THREE.MeshStandardMaterial({
            color: 0x5a5a5a,
            roughness: 0.9,
            metalness: 0.4,
    ...
    [[-0.3, -0.5], [0.3, -0.5], [-0.3, 0.5], [0.3, 0.5]].forEach(pos => {
        ...
        wheel.position.set(pos[0], 0.08, pos[1]);
        cartGroup.add(wheel);
    });
...
createWarningTape() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        tape.rotation.z = (Math.random() - 0.5) * 0.3;
        
        this.rooms[0].add(tape);
    }
}

createSecurityCamera() {
    for (let level = 0; level < 4; level++) {
        const yPos = level * 3.5 + 3;
        ...
        for (let i = 0; i < 2; i++) {
            ...
            cameraGroup.userData.rotationSpeed = (Math.random() - 0.5) * 0.001;
            
            this.rooms[0].add(cameraGroup);
...
// ========================================
// ROOFTOP VIEW (city skyline at night)
// ========================================

createRooftopView() {
    const roofLevel = 4 * 3.5;
    ...
    sky.position.y = roofLevel;
    this.rooms[0].add(sky);
    
    // Distant city skyline (silhouettes)
    const buildingPositions = [
        { x: -200, z: -200, h: 80, w: 40, d: 40 },
        { x: -150, z: -220, h: 120, w: 30, d: 30 },
        { x: -100, z: -210, h: 60, w: 35, d: 35 },
        { x: -50, z: -230, h: 100, w: 45, d: 45 },
        { x: 0, z: -240, h: 150, w: 50, d: 50 },
        { x: 50, z: -235, h: 90, w: 40, d: 40 },
        { x: 100, z: -220, h: 110, w: 35, d: 35 },
        { x: 150, z: -215, h: 70, w: 38, d: 38 },
        { x: 200, z: -225, h: 95, w: 42, d: 42 }
    ];
    ...
    buildingPositions.forEach(pos => {
        ...
        building.position.set(pos.x, roofLevel + pos.h/2, pos.z);
        ...
        for (let i = 0; i < 30; i++) {
            if (Math.random() > 0.7) {
                ...
                window.position.set(
                    ...
                    roofLevel + Math.random() * pos.h,
                    pos.z + pos.d/2 + 0.1
                );
                this.rooms[0].add(window);
                this.cityLights.push(window);
            }
        }
        ...
        if (Math.random() > 0.5) {
            const antenna = new THREE.Mesh(
                new THREE.CylinderGeometry(0.3, 0.3, 15, 6),
                new THREE.MeshStandardMaterial({
                    color: 0x8a8a8a,
                    roughness: 0.4,
                    metalness: 0.9
            ...
            antenna.position.set(pos.x, roofLevel + pos.h + 7.5, pos.z);
            this.rooms[0].add(antenna);
            
            // Red beacon
            const beacon = new THREE.PointLight(0xff0000, 2, 30);
            beacon.position.set(pos.x, roofLevel + pos.h + 15, pos.z);
            this.rooms[0].add(beacon);
            
            this.cityLights.push({ light: beacon, blinkSpeed: 0.5 + Math.random() });
        }
    });
    
    // Rooftop barrier wall
    const barrierHeight = 1.2;
    [-30, 30].forEach(x => {
        const barrier = new THREE.Mesh(
            new THREE.BoxGeometry(0.3, barrierHeight, 80),
            new THREE.MeshStandardMaterial({
                color: 0x3a3a3a,
                roughness: 0.9
            })
        );
        barrier.position.set(x, roofLevel + barrierHeight/2, 0);
        this.rooms[0].add(barrier);
    });
    
    // Rooftop artwork spot (best view)
    this.garageArtworkSpots.push({
        x: 0,
        y: roofLevel + 2,
        z: 25,
...
// ========================================
// COMPLETE GARAGE ANIMATIONS
// ========================================

updateGarageAnimations() {
    const time = Date.now() * 0.001;
    
    // 1. FLUORESCENT LIGHTS FLICKERING
    this.fluorescents.forEach((light, index) => {
        // Random flicker
        if (Math.random() < light.flickerChance) {
            light.light.intensity = Math.random() * 0.5;
            light.tube.material.emissiveIntensity = Math.random() * 0.3;
            
            // Restore after brief flicker
            setTimeout(() => {
                if (light.light && light.tube) {
                    light.light.intensity = light.baseIntensity;
                    light.tube.material.emissiveIntensity = 0.8;
                }
            }, 50 + Math.random() * 150);
        }
        
        // Buzzing effect (subtle pulsing)
        if (light.buzzSound) {
            const buzz = 0.9 + Math.sin(time * 30 + index) * 0.1;
            light.light.intensity = light.baseIntensity * buzz;
        }
    });
   
    
    // 3. CITY LIGHTS BLINKING
    this.cityLights.forEach((light, index) => {
        if (light.light && light.blinkSpeed) {
            // Beacon blink
            const blink = Math.sin(time * light.blinkSpeed * Math.PI) > 0.5 ? 2 : 0;
            light.light.intensity = blink;
        } else if (light.material) {
            // Window flicker
            if (Math.random() < 0.001) {
                light.material.emissiveIntensity = Math.random() * 0.5;
                setTimeout(() => {
                    if (light.material) light.material.emissiveIntensity = 0.8;
                }, 100 + Math.random() * 500);
            }
        }
    });
    
    // 4. DISTANT CAR ALARM (periodic)
    if (!this.lastCarAlarm) this.lastCarAlarm = Date.now();
    
    if (Date.now() - this.lastCarAlarm > 15000 + Math.random() * 20000) {
        this.triggerCarAlarm();
        this.lastCarAlarm = Date.now();
    }
}

triggerCarAlarm() {
    // Visual car alarm effect (flashing lights on random car)
    const randomSpot = this.parkingSpots[Math.floor(Math.random() * this.parkingSpots.length)];
    
    if (randomSpot && randomSpot.occupied) {
        // Create flashing light effect
        const alarmLight = new THREE.PointLight(0xff0000, 0, 8);
        alarmLight.position.copy(randomSpot.position);
        alarmLight.position.y += 0.5;
        this.rooms[0].add(alarmLight);
        
        let flashes = 0;
        const maxFlashes = 12;
        const flashInterval = setInterval(() => {
            alarmLight.intensity = alarmLight.intensity === 0 ? 3 : 0;
            flashes++;
            
            if (flashes >= maxFlashes) {
                clearInterval(flashInterval);
                this.rooms[0].remove(alarmLight);
            }
        }, 200);
        
        console.log("🚨 Car alarm triggered at space #" + randomSpot.number);
    }
}
...
checkCollisions() {
    if (!this.isMobile) {
        ...
        // Keep on current level (prevent falling through floors)
        const currentLevel = Math.floor(this.camera.position.y / 3.5);
        const levelFloor = currentLevel * 3.5;
        
        if (this.camera.position.y < levelFloor + 0.1) {
            this.camera.position.y = levelFloor + (this.cameraHeight || 1.6);
        }
...
getSpawnPosition() {
    // Spawn on ground level (P1) near entrance
    return {
        x: 0,
...
animate() {
    ...
    this.updateGarageAnimations();      // ✓ ADD THIS LINE
