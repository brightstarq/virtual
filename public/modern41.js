...
createGallery() {
    ...
    this.frostPatterns = [];
    this.iceSculptures = [];
    this.frozenWaterfalls = [];
    this.iceBlocks = [];
    this.auroraLights = [];
    this.crackingSounds = [];
    this.frostAnimations = [];
    
    this.createIcePalace();
    this.createIceWalls();
    this.createIcicleChandeliers();
    this.createAuroraBorealis();
    this.createFrozenFloor();
    this.createIceSculptures();
    this.createFrozenWaterfalls();
    this.createIceBlockArtworks();
    this.createIcePalaceUI();
    
    console.log("❄️ Ice Palace Gallery loaded!");
}

// ========================================
// ICE PALACE (frozen castle hall)
// ========================================

createIcePalace() {
    const palaceRoom = new THREE.Group();
    palaceRoom.visible = true;
    ...
    this.darkIceMaterial = new THREE.MeshPhysicalMaterial({
        color: 0x8899bb,
        transparent: true,
        opacity: 0.7,
        roughness: 0.2,
    ...
    this.glowIceMaterial = new THREE.MeshStandardMaterial({
        color: 0xaaccff,
        emissive: 0x4477ff,
    ...
    // ========================================
    // ATMOSPHERIC LIGHTING (cold, ethereal)
    // ========================================
    
    const ambientLight = new THREE.AmbientLight(0xaaccff, 0.4);
    palaceRoom.add(ambientLight);
    
    // Main ice light (from aurora above)
    this.iceMainLight = new THREE.DirectionalLight(0xaaccff, 0.8);
    this.iceMainLight.position.set(0, 14, 0);
    this.iceMainLight.castShadow = true;
    palaceRoom.add(this.iceMainLight);
    
    // Blue/cyan atmospheric lights
    const atmosphericColors = [0x4477ff, 0x00ffcc, 0x88aaff];
    for (let i = 0; i < 6; i++) {
        const light = new THREE.PointLight(
            atmosphericColors[i % 3],
            0.8,
            20
        );
        light.position.set(
            (Math.random() - 0.5) * hallWidth,
            hallHeight - 3,
            (Math.random() - 0.5) * hallLength
        );
        palaceRoom.add(light);
        this.auroraLights.push(light);
    }
    
    // Fog (icy mist)
    this.scene.fog = new THREE.FogExp2(0xccddff, 0.01);
    
    palaceRoom.position.set(0, 0, 0);
    this.rooms.push(palaceRoom);
    this.scene.add(palaceRoom);
}

// ========================================
// ICE WALLS (frozen castle walls)
// ========================================
...
createIceWalls() {
    ...
    // Side walls (textured ice)
    [-hallWidth/2, hallWidth/2].forEach((x, index) => {
        // Main wall sections
        for (let z = -hallLength/2; z <= hallLength/2; z += 10) {
            const wall = new THREE.Mesh(
                new THREE.BoxGeometry(1, hallHeight, 9),
                this.iceMaterial
            );
            wall.position.set(x, hallHeight/2, z);
            wall.castShadow = true;
            wall.receiveShadow = true;
            this.rooms[0].add(wall);
            this.iceWalls.push(wall);
            
            // Frost buildup (textured bumps)
            for (let i = 0; i < 15; i++) {
                ...
                frost.position.set(
                    x + (index === 0 ? 0.5 : -0.5),
                    Math.random() * hallHeight,
                    z + (Math.random() - 0.5) * 8
                );
                this.rooms[0].add(frost);
                this.frostPatterns.push(frost);
            }
            
            // Ice crystals jutting out
            for (let i = 0; i < 5; i++) {
                const crystal = new THREE.Mesh(
                    new THREE.ConeGeometry(0.15, 0.8, 6),
                    this.glowIceMaterial
                );
                ...
                crystal.rotation.z = (index === 0 ? -1 : 1) * Math.PI / 2;
                crystal.rotation.y = Math.random() * Math.PI;
                this.rooms[0].add(crystal);
            }
        }
    });
    ...
    [-hallLength/2, hallLength/2].forEach((z, index) => {
        const wall = new THREE.Mesh(
            new THREE.BoxGeometry(hallWidth, hallHeight, 1),
            this.iceMaterial
        );
        wall.position.set(0, hallHeight/2, z);
        wall.castShadow = true;
        wall.receiveShadow = true;
        this.rooms[0].add(wall);
        ...
        if (index === 0) {
            ...
            // Arch opening (dark ice)
            const arch = new THREE.Mesh(
                new THREE.BoxGeometry(archWidth, archHeight, 1.2),
                this.darkIceMaterial
            );
            arch.position.set(0, archHeight/2, z);
            this.rooms[0].add(arch);
            
            // Arch top (curved)
            const curve = new THREE.TorusGeometry(archWidth/2, 0.5, 16, 32, Math.PI);
            const archTop = new THREE.Mesh(curve, this.iceMaterial);
            archTop.rotation.x = Math.PI / 2;
            archTop.position.set(0, archHeight, z);
            this.rooms[0].add(archTop);
        }
    });
    
    // Ice pillars (supporting columns)
    ...
    pillarPositions.forEach(pos => {
        const pillar = new THREE.Mesh(
            new THREE.CylinderGeometry(0.8, 1, hallHeight, 6),
            this.iceMaterial
        );
        pillar.position.set(pos.x, hallHeight/2, pos.z);
        pillar.castShadow = true;
        this.rooms[0].add(pillar);
        
        // Spiral ice growth on pillar
        for (let i = 0; i < 20; i++) {
            const angle = (i / 20) * Math.PI * 4;
            const height = (i / 20) * hallHeight;
            ...
            spiral.position.set(
                ...
                height,
                pos.z + Math.sin(angle) * 1
            );
            spiral.rotation.set(0, angle, Math.PI / 3);
            this.rooms[0].add(spiral);
        }
    });
}

// ========================================
// ICICLE CHANDELIERS (hanging ice)
// ========================================

createIcicleChandeliers() {
    ...
    chandelierPositions.forEach(pos => {
        ...
        // Icicles hanging from ring (20 per chandelier)
        for (let i = 0; i < 20; i++) {
            ...
            const icicle = new THREE.Mesh(
                new THREE.ConeGeometry(0.08 + Math.random() * 0.05, length, 6),
                this.glowIceMaterial
            );
            icicle.position.set(
            ...
            chandelier.add(icicle);
            
            // Tip glow (dripping ice)
            const tipLight = new THREE.PointLight(0x88ccff, 0.5, 3);
            tipLight.position.set(
                Math.cos(angle) * radius,
                -length,
                Math.sin(angle) * radius
            );
            chandelier.add(tipLight);
            
            this.icicles.push({ mesh: icicle, light: tipLight, baseY: -length/2 });
        }
        
        // Central icicle (largest)
        const centerIcicle = new THREE.Mesh(
            new THREE.ConeGeometry(0.2, 3, 8),
            this.iceMaterial
        );
        centerIcicle.position.y = -1.5;
        centerIcicle.castShadow = true;
        chandelier.add(centerIcicle);
        
        // Hanging chains (frozen)
        for (let i = 0; i < 4; i++) {
        ...
        chandelier.position.set(pos.x, 13, pos.z);
        this.rooms[0].add(chandelier);
        this.iceChandeliers.push(chandelier);
    });
    
    // Small icicles on ceiling (scattered)
    for (let i = 0; i < 50; i++) {
        const icicle = new THREE.Mesh(
            new THREE.ConeGeometry(0.05, 0.5 + Math.random() * 1, 6),
            this.iceMaterial
        );
        icicle.position.set(
            (Math.random() - 0.5) * 35,
        ...
        icicle.castShadow = true;
        this.rooms[0].add(icicle);
        
        this.icicles.push({ mesh: icicle, baseY: icicle.position.y });
    }
}

// ========================================
// AURORA BOREALIS (ceiling effect)
// ========================================

createAuroraBorealis() {
    const hallWidth = 40;
    const hallLength = 70;
    const ceilingHeight = 14.5;
    
    // Glass ceiling (to see aurora through)
    ...
    // Aurora waves (animated curtains of light)
    for (let i = 0; i < 5; i++) {
        const auroraGeometry = new THREE.PlaneGeometry(15, 10, 20, 10);
        const positions = auroraGeometry.attributes.position;
        
        // Wave the geometry
        for (let j = 0; j < positions.count; j++) {
            const x = positions.getX(j);
            const y = positions.getY(j);
            positions.setZ(j, Math.sin(x * 0.5 + i) * 0.5);
        }
        
        const colors = [0x00ff88, 0x00ffff, 0x4477ff, 0xff00ff, 0x88ff00];
        ...
        this.rooms[0].add(aurora);
        this.auroraLights.push({
            mesh: aurora,
    ...
    // Starry sky above aurora (night sky)
...
// ========================================
// FROZEN FLOOR (ice sheet)
// ========================================

createFrozenFloor() {
    const floorWidth = 40;
    const floorLength = 70;
    ...
    for (let i = 0; i < 30; i++) {
        ...
        crack.position.set(
            (Math.random() - 0.5) * floorWidth * 0.9,
            0.01,
            (Math.random() - 0.5) * floorLength * 0.9
        );
    ...
    for (let i = 0; i < 20; i++) {
        ...
        snowflake.position.set(
            (Math.random() - 0.5) * floorWidth * 0.8,
            0.03,
            (Math.random() - 0.5) * floorLength * 0.8
        );
        this.rooms[0].add(snowflake);
    }
}
...
createSnowflake() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        [-0.1, 0.1].forEach(offset => {
            ...
            branch.position.set(
                Math.cos(angle) * 0.15 + Math.cos(angle + Math.PI/4) * offset,
                0,
                Math.sin(angle) * 0.15 + Math.sin(angle + Math.PI/4) * offset
            );
            branch.rotation.y = angle + (offset > 0 ? Math.PI/6 : -Math.PI/6);
            group.add(branch);
        });
    }
...
// ========================================
// ICE SCULPTURES (decorative)
// ========================================

createIceSculptures() {
    const sculpturePositions = [
        { x: -12, z: -15, type: 'dragon' },
        { x: 12, z: -15, type: 'wolf' },
        { x: -12, z: 15, type: 'phoenix' },
        { x: 12, z: 15, type: 'bear' }
    ];
    
    sculpturePositions.forEach(pos => {
        const sculpture = this.createIceSculpture(pos.type);
        sculpture.position.set(pos.x, 0, pos.z);
        this.rooms[0].add(sculpture);
        this.iceSculptures.push(sculpture);
        ...
        const light = new THREE.SpotLight(0x88ccff, 2, 10, Math.PI / 6, 0.5);
        light.position.set(pos.x, 8, pos.z);
        light.target.position.set(pos.x, 1, pos.z);
        light.castShadow = true;
        this.rooms[0].add(light);
        this.rooms[0].add(light.target);
    });
}

createIceSculpture(type) {
    const group = new THREE.Group();
    ...
    switch (type) {
        case 'dragon':
            ...
            [-1, 1].forEach(side => {
                ...
                wing.position.set(side * 1.2, 2, 0);
                group.add(wing);
            });
            
            // Body
            const body = new THREE.Mesh(
                new THREE.CylinderGeometry(0.5, 0.6, 2, 8),
                this.iceMaterial
            );
            ...
            for (let i = 0; i < 5; i++) {
                const segment = new THREE.Mesh(
                    new THREE.SphereGeometry(0.3 - i * 0.04, 8, 8),
                    this.iceMaterial
                );
                segment.position.set(0, 0.8 - i * 0.2, 1 + i * 0.4);
                group.add(segment);
            }
            break;
        ...
        case 'wolf':
            ...
            wolfHead.position.set(0, 2, -0.5);
            wolfHead.rotation.x = -0.3;
            group.add(wolfHead);
            ...
            wolfSnout.position.set(0, 1.8, -1);
            group.add(wolfSnout);
            ...
            wolfBody.position.set(0, 1.5, 0.5);
            wolfBody.rotation.x = -0.2;
            group.add(wolfBody);
            
            // Legs (4)
            [[-0.3, -0.3], [0.3, -0.3], [-0.3, 0.8], [0.3, 0.8]].forEach(pos => {
                const leg = new THREE.Mesh(
                    new THREE.CylinderGeometry(0.12, 0.1, 1.2, 6),
                    this.iceMaterial
                );
            ...
            for (let i = 0; i < 4; i++) {
                ...
                segment.position.set(0, 1.5 + i * 0.2, 1.2 + i * 0.1);
        ...
        case 'phoenix':
            ...
            phoenixBody.position.set(0, 2, 0);
            phoenixBody.scale.set(1, 1.2, 0.8);
            ...
            phoenixHead.position.set(0, 2.8, 0);
            group.add(phoenixHead);
            ...
            for (let i = 0; i < 5; i++) {
                ...
                feather.position.set((i - 2) * 0.15, 3.2, 0);
            ...
            // Wings (spread)
            [-1, 1].forEach(side => {
                // Wing structure (3 segments)
                for (let i = 0; i < 3; i++) {
                    const wingPart = new THREE.Mesh(
                        new THREE.BoxGeometry(0.8, 0.1, 0.3),
                        this.iceMaterial
                    );
                    wingPart.position.set(side * (0.8 + i * 0.6), 2.2 - i * 0.2, i * 0.2);
                    wingPart.rotation.y = side * (0.3 + i * 0.2);
                    wingPart.rotation.z = side * 0.3;
                    group.add(wingPart);
                }
            });
            ...
            for (let i = 0; i < 5; i++) {
                ...
                tailFeather.position.set((i - 2) * 0.2, 1.5, 0.8 + i * 0.1);
                tailFeather.rotation.x = 0.5 + i * 0.1;
        ...
        case 'bear':
            ...
            bearHead.position.set(0, 2.2, -0.3);
            ...
            bearSnout.position.set(0, 2, -0.8);
            group.add(bearSnout);
            ...
            bearBody.position.set(0, 1.2, 0.5);
            group.add(bearBody);
            
            // Legs (4 thick legs)
            [[-0.4, -0.3], [0.4, -0.3], [-0.4, 0.8], [0.4, 0.8]].forEach(pos => {
                const leg = new THREE.Mesh(
...
// ========================================
// FROZEN WATERFALLS (cascading ice)
// ========================================

createFrozenWaterfalls() {
    ...
    waterfallPositions.forEach(pos => {
        const waterfall = this.createWaterfall(pos.height);
        waterfall.position.set(pos.x, 0, pos.z);
        this.rooms[0].add(waterfall);
        this.frozenWaterfalls.push(waterfall);
    });
}

createWaterfall(height) {
    const group = new THREE.Group();
    
    // Ice wall (where water would flow from)
    const wall = new THREE.Mesh(
        new THREE.BoxGeometry(4, height, 0.5),
        this.darkIceMaterial
    );
    wall.position.y = height / 2;
    group.add(wall);
    
    // Frozen water cascade (layered icicles)
    const cascadeLayers = Math.floor(height / 0.5);
    
    for (let layer = 0; layer < cascadeLayers; layer++) {
        const layerY = height - layer * 0.5;
        const layerWidth = 3 + (layer * 0.1);
        
        // Main frozen sheet
        const sheet = new THREE.Mesh(
            new THREE.PlaneGeometry(layerWidth, 0.6),
            new THREE.MeshPhysicalMaterial({
                color: 0xccddff,
                transparent: true,
        ...
        sheet.position.set(0, layerY, 0.3);
        group.add(sheet);
        
        // Icicle formations on the sheet
        for (let i = 0; i < 8; i++) {
            const icicle = new THREE.Mesh(
    ...
    const pool = new THREE.Mesh(
        new THREE.CylinderGeometry(2.5, 2, 0.3, 32),
        new THREE.MeshPhysicalMaterial({
            color: 0xaaccee,
            transparent: true,
            opacity: 0.7,
    ...
    // Blue glow light behind waterfall
    const waterfallLight = new THREE.PointLight(0x00ccff, 1.5, 8);
    waterfallLight.position.set(0, height / 2, -0.5);
...
// ========================================
// ICE BLOCK ARTWORKS (frozen in ice with colored lighting)
// ========================================

createIceBlockArtworks() {
    // Special artwork spots - frozen in ice blocks
    const iceBlockPositions = [
        { x: -8, z: -20, color: 0xff0088 },  // Magenta
        { x: 8, z: -20, color: 0x00ff88 },   // Cyan
        { x: -8, z: -5, color: 0xffaa00 },   // Orange
        { x: 8, z: -5, color: 0x0088ff },    // Blue
        { x: -8, z: 10, color: 0xff00ff },   // Purple
        { x: 8, z: 10, color: 0x00ffff },    // Aqua
        { x: -8, z: 25, color: 0xff8800 },   // Red-Orange
        { x: 8, z: 25, color: 0x88ff00 }     // Yellow-Green
    ];
    
    iceBlockPositions.forEach((pos, index) => {
        this.createIceBlockArtwork(pos.x, pos.z, pos.color, index);
    });
}

createIceBlockArtwork(x, z, lightColor, index) {
    const iceBlock = new THREE.Group();
    ...
    pedestal.castShadow = true;
    iceBlock.add(pedestal);
    
    // Main ice block (artwork will be inside)
    const blockWidth = 2;
    const blockHeight = 3;
    ...
    block.position.y = 2.3;
    block.castShadow = true;
    block.receiveShadow = true;
    iceBlock.add(block);
    
    // Frozen bubbles in ice
    for (let i = 0; i < 15; i++) {
        ...
        bubble.position.set(
            (Math.random() - 0.5) * (blockWidth - 0.2),
            2.3 + (Math.random() - 0.5) * (blockHeight - 0.2),
            (Math.random() - 0.5) * (blockDepth - 0.05)
        );
        iceBlock.add(bubble);
    }
    ...
    for (let i = 0; i < 8; i++) {
        const crack = new THREE.Mesh(
            new THREE.PlaneGeometry(0.02, 0.5 + Math.random() * 1),
            new THREE.MeshBasicMaterial({
                color: 0x88aacc,
                transparent: true,
                opacity: 0.4
        ...
        crack.position.set(
            (Math.random() - 0.5) * (blockWidth - 0.3),
            2.3 + (Math.random() - 0.5) * (blockHeight - 0.3),
            blockDepth / 2 + 0.01
        );
        crack.rotation.z = Math.random() * Math.PI;
        iceBlock.add(crack);
    ...
    // Frost on edges
    const edgePositions = [
        { x: -blockWidth/2, y: 2.3, z: 0, rot: [0, Math.PI/2, 0] },
        { x: blockWidth/2, y: 2.3, z: 0, rot: [0, -Math.PI/2, 0] },
        { x: 0, y: 2.3 + blockHeight/2, z: 0, rot: [Math.PI/2, 0, 0] },
        { x: 0, y: 2.3 - blockHeight/2, z: 0, rot: [-Math.PI/2, 0, 0] }
    ];
    ...
    edgePositions.forEach(edge => {
        for (let i = 0; i < 5; i++) {
            ...
            frost.position.set(
                edge.x + (Math.random() - 0.5) * 0.2,
                edge.y + (Math.random() - 0.5) * 0.2,
                edge.z + (Math.random() - 0.5) * 0.2
            );
            frost.rotation.set(edge.rot[0], edge.rot[1], edge.rot[2]);
            iceBlock.add(frost);
        }
    });
    
    // Colored lighting (shining through ice onto artwork)
    const colorLight = new THREE.SpotLight(lightColor, 2, 8, Math.PI / 4, 0.5);
    colorLight.position.set(x, 5, z + 2);
    colorLight.target.position.set(x, 2.3, z);
    colorLight.castShadow = true;
    this.rooms[0].add(colorLight);
    this.rooms[0].add(colorLight.target);
    
    // Back light (creates glow effect)
    const backLight = new THREE.PointLight(lightColor, 1.5, 5);
    backLight.position.y = 2.3;
    backLight.position.z = -0.3;
    iceBlock.add(backLight);
    
    // Ice crystals around base (decorative)
    for (let i = 0; i < 8; i++) {
        ...
        iceBlock.add(crystal);
    }
    
    iceBlock.position.set(x, 0, z);
    this.rooms[0].add(iceBlock);
...
// ========================================
// ICE PALACE ANIMATIONS
// ========================================

updateIcePalaceAnimations() {
    const time = Date.now() * 0.001;
    
    // 1. AURORA BOREALIS ANIMATION (flowing, color-shifting)
    this.auroraLights.forEach((aurora, index) => {
        if (aurora.mesh) {
            // Vertical wave motion
            const waveY = Math.sin(time * 0.5 + aurora.waveOffset) * 0.5;
            aurora.mesh.position.y = aurora.baseY + waveY;
            
            // Horizontal drift
            const driftX = Math.sin(time * 0.3 + index) * 2;
            aurora.mesh.position.x += driftX * 0.01;
            
            // Color pulsing
            const intensity = 0.2 + Math.sin(time * 0.8 + index) * 0.1;
            aurora.mesh.material.opacity = intensity;
            
            // Geometry wave deformation
            const positions = aurora.mesh.geometry.attributes.position;
            for (let i = 0; i < positions.count; i++) {
                const x = positions.getX(i);
                const y = positions.getY(i);
                const wave = Math.sin(x * 0.5 + time + aurora.waveOffset) * 0.5;
            ...
            aurora.intensity = pulse;
        }
    });
    
    // 2. ICICLE DRIPPING/GROWING (subtle animation)
    this.icicles.forEach((icicle, index) => {
        // Slow growth/shrink cycle
        const growth = Math.sin(time * 0.1 + index * 0.1) * 0.05;
        icicle.mesh.scale.y = 1 + growth;
        
        // Slight sway
        if (icicle.mesh.parent && icicle.mesh.parent.type === 'Group') {
            icicle.mesh.rotation.z = Math.sin(time * 0.5 + index * 0.3) * 0.05;
        }
        
        // Light intensity variation (drip glow)
        if (icicle.light) {
            icicle.light.intensity = 0.3 + Math.sin(time * 3 + index) * 0.2;
        }
    });
    
    // 3. FROST PATTERNS FORMING/MELTING
    this.frostPatterns.forEach((frost, index) => {
        // Opacity pulsing (forming/melting)
        const phase = Math.sin(time * 0.2 + index * 0.1);
        
        if (frost.material.opacity) {
            // Frost patches on floor
            frost.material.opacity = 0.3 + phase * 0.2;
        } else {
            // Frost bumps on walls
            const scale = 1 + phase * 0.1;
            frost.scale.set(scale, scale, scale);
        }
    });
    
    // 4. ICE BLOCK ARTWORK LIGHTING (colored glow pulsing)
    this.iceBlocks.forEach((iceBlock, index) => {
        // Color light intensity
        iceBlock.glowPhase += 0.02;
        const glowIntensity = 1.5 + Math.sin(iceBlock.glowPhase) * 0.5;
        iceBlock.light.intensity = glowIntensity;
        iceBlock.backLight.intensity = glowIntensity * 0.8;
        
        // Block opacity shimmer
        const shimmer = 0.35 + Math.sin(time * 1.5 + index) * 0.05;
        iceBlock.block.material.opacity = shimmer;
        
        // Slight rotation (frozen in ice, slowly spinning)
        iceBlock.model.rotation.y += 0.0002;
    });
    
    // 5. ICE CHANDELIER SWAYING
    this.iceChandeliers.forEach((chandelier, index) => {
        // Gentle pendulum motion
        const sway = Math.sin(time * 0.5 + index) * 0.02;
        chandelier.rotation.z = sway;
    });
    
    // 6. ICE SCULPTURES SUBTLE GLOW
    this.iceSculptures.forEach((sculpture, index) => {
        // Rotate slowly on pedestal
        sculpture.rotation.y += 0.0003;
    });
    
    // 7. FROZEN WATERFALL EFFECTS
    this.frozenWaterfalls.forEach((waterfall, index) => {
        // Light behind waterfall pulsing
        if (waterfall.children.length > 0) {
            waterfall.children.forEach(child => {
                if (child.type === 'PointLight') {
                    child.intensity = 1.2 + Math.sin(time * 0.8 + index) * 0.3;
                }
            });
        }
    });
    
    // 8. RANDOM ICE CRACKING SOUND EFFECT (visual indicator)
    if (!this.lastIceCrack) this.lastIceCrack = Date.now();
    
    if (Date.now() - this.lastIceCrack > 8000 + Math.random() * 12000) {
        this.triggerIceCrack();
        this.lastIceCrack = Date.now();
    }
    
    // 9. TEMPERATURE DISPLAY (simulated)
    const tempUI = document.getElementById('iceTemp');
    if (tempUI) {
        const temp = -15 + Math.sin(time * 0.1) * 2;
        tempUI.textContent = `${temp.toFixed(1)}°C`;
    }
}

triggerIceCrack() {
    // Visual ice crack effect
    const randomX = (Math.random() - 0.5) * 35;
    const randomZ = (Math.random() - 0.5) * 65;
    
    // Create crack flash (white flash)
    ...
    flash.position.set(randomX, 0.03, randomZ);
    this.rooms[0].add(flash);
    
    // Crack lines radiating out
    for (let i = 0; i < 5; i++) {
        const angle = (i / 5) * Math.PI * 2;
        const crackLine = new THREE.Mesh(
            new THREE.PlaneGeometry(0.03, 1 + Math.random()),
            new THREE.MeshBasicMaterial({
                color: 0x88aacc,
                transparent: true,
                opacity: 0.6
        ...
        crackLine.position.set(randomX, 0.03, randomZ);
        this.rooms[0].add(crackLine);
        
        // Fade out crack lines
        setTimeout(() => {
            let opacity = 0.6;
            const fadeInterval = setInterval(() => {
                opacity -= 0.05;
                crackLine.material.opacity = opacity;
                if (opacity <= 0) {
                    clearInterval(fadeInterval);
                    this.rooms[0].remove(crackLine);
                }
            }, 50);
        }, 100);
    }
    
    // Flash pulsing effect
    let pulses = 0;
    const pulseInterval = setInterval(() => {
        flash.material.opacity = flash.material.opacity === 0.8 ? 0.3 : 0.8;
        pulses++;
        if (pulses >= 6) {
            clearInterval(pulseInterval);
            this.rooms[0].remove(flash);
        }
    }, 100);
    
    console.log("🧊 Ice cracking sound at (" + randomX.toFixed(1) + ", " + randomZ.toFixed(1) + ")");
}
...
animate() {
    ...
    this.updateIcePalaceAnimations();   // ✓ ADD THIS LINE
