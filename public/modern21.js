...
class ThreeJSApp {
    constructor() {
        ...
        this.camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
         this.scene = new THREE.Scene();
    this.scene.fog = new THREE.Fog(0xe8e8e8, 20, 80); // Soft fog
...
this.setupMobileControls();
    ...
    addLighting() {
    // Very soft ambient (relying on apertures for main light)
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.15);
    this.scene.add(ambientLight);

    // Soft directional (simulating winter daylight)
    const mainLight = new THREE.DirectionalLight(0xfffff8, 0.6);
    ...
    // Cool fill light (very subtle)
    const fillLight = new THREE.DirectionalLight(0xf0f8ff, 0.25);
    fillLight.position.set(-20, 18, -15);
   ...
   createGallery() {
    ...
    const roomWidth = 50;
    const roomDepth = 50;
    const roomHeight = 12; // Monumental height
    
    // ========================================
    // MONOLITHIC MATERIAL PALETTE
    // ========================================
    
    // Polished concrete floor (cool gray with subtle warmth)
    const concreteFloorMaterial = new THREE.MeshStandardMaterial({
        color: 0xd8d8d6, // Soft mineral gray
        roughness: 0.25,
        metalness: 0.15,
        envMapIntensity: 0.8
    });
    const concreteTexture = this.generatePolishedConcrete(2048, 2048);
    ...
    concreteTextureObj.repeat.set(10, 10);
    concreteFloorMaterial.map = concreteTextureObj;
    
    // Primary wall material (warm limestone white)
    const wallMaterial = new THREE.MeshStandardMaterial({
        color: 0xf5f5f0, // Warm mineral white
        roughness: 0.90,
        metalness: 0.0,
        envMapIntensity: 0.3
    });
    
    // Secondary wall material (cool porcelain white) 
    const wallMaterialCool = new THREE.MeshStandardMaterial({
        color: 0xfafafa, // Cool white
        roughness: 0.88,
        metalness: 0.0,
        envMapIntensity: 0.3
    });
    
    // Ceiling material (slightly darker for depth)
    ...
    // Brushed metal for structural elements (low-reflectivity)
    const structuralMetalMaterial = new THREE.MeshStandardMaterial({
        color: 0xc0c0c0,
        roughness: 0.45,
        metalness: 0.75,
        envMapIntensity: 0.6
    ...
    // Frameless glass
    const framelessGlassMaterial = new THREE.MeshStandardMaterial({
        color: 0xffffff,
        roughness: 0.02,
        metalness: 0.0,
        transparent: true,
    ...
    // Subtle expansion joints (minimal)
    ...
    const jointPositions = [-15, 0, 15];
    jointPositions.forEach(pos => {
        const jointX = new THREE.Mesh(
            new THREE.BoxGeometry(roomWidth, 0.01, 0.02),
            jointMaterial
        );
        jointX.position.set(0, 0.005, pos);
        room1.add(jointX);
        
        const jointZ = new THREE.Mesh(
            new THREE.BoxGeometry(0.02, 0.01, roomDepth),
            jointMaterial
        );
        jointZ.position.set(pos, 0.005, 0);
        room1.add(jointZ);
    });
    ...
    // North wall (entry side) - warm white
    ...
    // South wall - transitioning to cooler white
    ...
    // ========================================
    // VERTICAL LIGHT SEAMS (Structural Joints)
    // ========================================
    
    const lightSeamMaterial = new THREE.MeshStandardMaterial({
        color: 0xffffff,
        emissive: 0xfff5e6,
    ...
    const cornerPositions = [
        { x: roomWidth / 2 - 0.05, z: -roomDepth / 2 },
        { x: roomWidth / 2 - 0.05, z: roomDepth / 2 },
        { x: -roomWidth / 2 + 0.05, z: -roomDepth / 2 },
        { x: -roomWidth / 2 + 0.05, z: roomDepth / 2 }
    ];
    ...
    cornerPositions.forEach(pos => {
        const seam = new THREE.Mesh(
            new THREE.BoxGeometry(0.05, roomHeight, 0.05),
            lightSeamMaterial
        );
        seam.position.set(pos.x, roomHeight / 2, pos.z);
        room1.add(seam);
        
        // Subtle point light from seam
        const seamLight = new THREE.PointLight(0xfff5e6, 0.6, 15);
        seamLight.position.copy(seam.position);
        room1.add(seamLight);
    });
    
    // ========================================
    // CEILING WITH LINEAR LIGHT APERTURES
    // ========================================
    
    const mainCeiling = new THREE.Mesh(
        new THREE.BoxGeometry(roomWidth, 0.2, roomDepth),
        ceilingMaterial
    );
    mainCeiling.position.y = roomHeight;
    mainCeiling.receiveShadow = true;
    room1.add(mainCeiling);
    
    // Linear ceiling apertures (concealed lighting)
    const aperturePositions = [-12, -6, 6, 12];
    
    aperturePositions.forEach(pos => {
        // Light-emitting aperture
        const aperture = new THREE.Mesh(
            new THREE.BoxGeometry(roomWidth * 0.85, 0.1, 0.3),
        ...
        aperture.position.set(0, roomHeight - 0.15, pos);
        room1.add(aperture);
        
        // Rect area light from aperture
        const apertureLight = new THREE.RectAreaLight(
            0xfffff0, 
            4.0, 
            roomWidth * 0.85, 
            0.3
        );
        apertureLight.position.set(0, roomHeight - 0.2, pos);
        apertureLight.rotation.x = -Math.PI / 2;
        room1.add(apertureLight);
    });
    
    // ========================================
    // GRAND CENTRAL VOID WITH OCULUS
    // ========================================
    
    const voidWidth = 8;
    const voidDepth = 8;
    const voidHeight = roomHeight + 4; // Extends above main ceiling
    
    // Void walls (recessed)
    const voidWallMaterial = new THREE.MeshStandardMaterial({
        color: 0xf0f0f0,
        roughness: 0.92,
        metalness: 0.0
    });
    ...
    // North void wall
    const voidNorthWall = new THREE.Mesh(
        new THREE.BoxGeometry(voidWidth, voidHeight, 0.2),
        voidWallMaterial
    );
    voidNorthWall.position.set(0, voidHeight / 2, -voidDepth / 2);
    voidNorthWall.receiveShadow = true;
    room1.add(voidNorthWall);
    
    // South void wall
    const voidSouthWall = new THREE.Mesh(
        new THREE.BoxGeometry(voidWidth, voidHeight, 0.2),
        voidWallMaterial
    );
    voidSouthWall.position.set(0, voidHeight / 2, voidDepth / 2);
    voidSouthWall.receiveShadow = true;
    room1.add(voidSouthWall);
    
    // East void wall
    const voidEastWall = new THREE.Mesh(
        new THREE.BoxGeometry(0.2, voidHeight, voidDepth),
        voidWallMaterial
    );
    voidEastWall.position.set(voidWidth / 2, voidHeight / 2, 0);
    voidEastWall.receiveShadow = true;
    room1.add(voidEastWall);
    
    // West void wall
    const voidWestWall = new THREE.Mesh(
        new THREE.BoxGeometry(0.2, voidHeight, voidDepth),
        voidWallMaterial
    );
    voidWestWall.position.set(-voidWidth / 2, voidHeight / 2, 0);
    voidWestWall.receiveShadow = true;
    room1.add(voidWestWall);
    
    // Oculus skylight (diffused circular opening)
    const oculusRadius = 3.5;
    const oculus = new THREE.Mesh(
        new THREE.CircleGeometry(oculusRadius, 64),
        new THREE.MeshStandardMaterial({
            color: 0xffffff,
            emissive: 0xfffffa,
    ...
    oculus.position.y = voidHeight - 0.1;
    room1.add(oculus);
    
    // Oculus light (controlled column of light)
    const oculusLight = new THREE.SpotLight(
        0xfffffa,
        8.0,
        voidHeight,
        Math.PI / 8,
    ...
    oculusLight.position.set(0, voidHeight - 0.5, 0);
    oculusLight.target.position.set(0, 0, 0);
    oculusLight.castShadow = true;
    oculusLight.shadow.mapSize.width = 2048;
    oculusLight.shadow.mapSize.height = 2048;
    room1.add(oculusLight);
    room1.add(oculusLight.target);
    
    // ========================================
    // FRAMELESS GLASS PARTITIONS (Sparse)
    // ========================================
    
    const glassPartitions = [
        { x: -18, z: 0, rot: Math.PI / 2 },
        { x: 18, z: 0, rot: Math.PI / 2 }
    ];
    
    glassPartitions.forEach(partition => {
        const glass = new THREE.Mesh(
            new THREE.BoxGeometry(0.04, roomHeight * 0.8, 6),
            framelessGlassMaterial
        );
        glass.position.set(partition.x, roomHeight * 0.4, partition.z);
        glass.rotation.y = partition.rot;
        glass.receiveShadow = true;
        room1.add(glass);
        
        // Minimal structural support (brushed metal)
        const support = new THREE.Mesh(
            new THREE.BoxGeometry(0.06, roomHeight * 0.8, 0.06),
            structuralMetalMaterial
        );
        support.position.copy(glass.position);
        support.rotation.y = partition.rot;
        room1.add(support);
    });
    ...
    // Soft fill lights (simulating reflected light from walls)
    const fillLightPositions = [
        new THREE.Vector3(-20, roomHeight / 2, 0),
        new THREE.Vector3(20, roomHeight / 2, 0),
        new THREE.Vector3(0, roomHeight / 2, -20),
        new THREE.Vector3(0, roomHeight / 2, 20)
    ];
    
    fillLightPositions.forEach(pos => {
        const fillLight = new THREE.PointLight(0xfff8f0, 1.2, 30);
        fillLight.position.copy(pos);
        room1.add(fillLight);
    });
    
    // Store references for animations
    this.glassSpotlights = []; // Clear old spotlight system
    this.wallLights = [];
    this.centerSculpture = null; // Remove sculpture reference
    
    room1.position.set(0, 0, 0);
    this.rooms.push(room1);
    this.rooms.forEach(room => this.scene.add(room));
    
    console.log("✨ Architectural masterpiece gallery created");
}

// ========================================
// NEW HELPER FUNCTION: Polished Concrete Texture
// ========================================
...
generatePolishedConcrete(width, height) {
    ...
    // Base concrete color (cool gray with warmth)
    ctx.fillStyle = '#d8d8d6';
    ctx.fillRect(0, 0, width, height);
    
    // Microscopic aggregate (subtle speckles)
    for (let i = 0; i < 12000; i++) {
        const x = Math.random() * width;
        const y = Math.random() * height;
        const size = Math.random() * 1.5 + 0.3;
        
        // Mix of light and dark aggregates
        const isLight = Math.random() > 0.5;
        const shade = isLight 
            ? Math.random() * 20 + 225  // Light stones
            : Math.random() * 30 + 180; // Darker stones
        
        ctx.fillStyle = `rgba(${shade}, ${shade}, ${shade}, ${0.3 + Math.random() * 0.4})`;
        ctx.fillRect(x, y, size, size);
    }
    
    // Polished finish (subtle shine variation)
    for (let i = 0; i < 40; i++) {
        const x = Math.random() * width;
        const y = Math.random() * height;
    ...
    // Very subtle concrete texture (barely visible grain)
...
createSkyGradient() {
    ...
    gradient.addColorStop(0, '#f5f5f0');   // Warm white at horizon
    gradient.addColorStop(0.5, '#e8e8e8'); // Mid gray
    gradient.addColorStop(1, '#d0d0d0');   // Slightly darker at zenith
...
updateLighting() {
    ...
    // Subtle variation in oculus intensity (breathing effect)
    const oculusLight = this.scene.children.find(
        child => child instanceof THREE.SpotLight && child.intensity > 5
    );
    if (oculusLight) {
        oculusLight.intensity = 7.5 + Math.sin(time * 0.3) * 0.5;
    }
