...
createGallery() {
    ...
    const penthouseWidth = 20;   // 20 meters (65 feet) - luxury penthouse size
    const penthouseDepth = 16;   // 16 meters (52 feet)
    const penthouseHeight = 3.6; // 3.6 meters (12 feet) - high ceilings
    
    // ========================================
    // PREMIUM LUXURY MATERIALS
    // ========================================
    
    // White Carrara marble floor with gold veining
    ...
    marbleTextureObj.repeat.set(8, 6);
    marbleFloorMaterial.map = marbleTextureObj;
    
    // Polished white walls
    ...
    const floor = new THREE.Mesh(
        new THREE.PlaneGeometry(penthouseWidth, penthouseDepth),
        marbleFloorMaterial
    );
    ...
    // Gold inlay border around perimeter
    const borderWidth = 0.08;
    const borderPositions = [
        // Front
        { w: penthouseWidth * 0.95, d: borderWidth, x: 0, z: penthouseDepth / 2 - 0.5 },
        // Back
        { w: penthouseWidth * 0.95, d: borderWidth, x: 0, z: -penthouseDepth / 2 + 0.5 },
        // Left
        { w: borderWidth, d: penthouseDepth * 0.95, x: -penthouseWidth / 2 + 0.5, z: 0 },
        // Right
        { w: borderWidth, d: penthouseDepth * 0.95, x: penthouseWidth / 2 - 0.5, z: 0 }
    ];
    
    borderPositions.forEach(border => {
        const inlay = new THREE.Mesh(
            new THREE.PlaneGeometry(border.w, border.d),
            goldMaterial
        );
        inlay.rotation.x = -Math.PI / 2;
        inlay.position.set(border.x, 0.001, border.z);
        room1.add(inlay);
    });
    
    // ========================================
    // FLOOR-TO-CEILING WINDOWS (3 walls)
    // ========================================
    
    // Generate city skyline view (simple silhouette)
    const createCityView = () => {
        ...
        // Gradient sky (sunset/dusk)
        const gradient = ctx.createLinearGradient(0, 0, 0, 1024);
        gradient.addColorStop(0, '#1a3a5f');
        gradient.addColorStop(0.5, '#ff6b4a');
        gradient.addColorStop(1, '#ffd89b');
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, 2048, 1024);
        
        // City buildings (random heights)
        ctx.fillStyle = '#0a0a0a';
        for (let i = 0; i < 30; i++) {
            const x = i * 70;
            const height = Math.random() * 600 + 200;
            ctx.fillRect(x, 1024 - height, 65, height);
            
            // Windows on buildings
            ctx.fillStyle = '#ffdd66';
            for (let y = 1024 - height; y < 1024; y += 25) {
                for (let wx = x + 5; wx < x + 60; wx += 15) {
                    if (Math.random() > 0.3) {
                        ctx.fillRect(wx, y + 5, 8, 12);
                    }
                }
            }
            ctx.fillStyle = '#0a0a0a';
    ...
    const cityViewTexture = new THREE.CanvasTexture(createCityView());
    cityViewTexture.needsUpdate = true;
    
    const cityViewMaterial = new THREE.MeshStandardMaterial({
        map: cityViewTexture,
        emissive: 0xffffff,
    ...
    // Front wall - massive window
    const frontWindow = new THREE.Mesh(
        new THREE.PlaneGeometry(penthouseWidth * 0.9, penthouseHeight * 0.95),
        glassMaterial
    );
    frontWindow.position.set(0, penthouseHeight / 2, penthouseDepth / 2 - 0.05);
    room1.add(frontWindow);
    
    // City view behind window
    const cityView1 = new THREE.Mesh(
        new THREE.PlaneGeometry(penthouseWidth * 0.9, penthouseHeight * 0.95),
        cityViewMaterial
    );
    cityView1.position.set(0, penthouseHeight / 2, penthouseDepth / 2 + 2);
    room1.add(cityView1);
    
    // Left wall - window
    const leftWindow = new THREE.Mesh(
        new THREE.PlaneGeometry(penthouseDepth * 0.9, penthouseHeight * 0.95),
        glassMaterial
    );
    leftWindow.position.set(-penthouseWidth / 2 + 0.05, penthouseHeight / 2, 0);
    leftWindow.rotation.y = Math.PI / 2;
    room1.add(leftWindow);
    
    // City view left
    const cityView2 = new THREE.Mesh(
        new THREE.PlaneGeometry(penthouseDepth * 0.9, penthouseHeight * 0.95),
        cityViewMaterial
    );
    cityView2.position.set(-penthouseWidth / 2 - 2, penthouseHeight / 2, 0);
    cityView2.rotation.y = Math.PI / 2;
    room1.add(cityView2);
    ...
    rightWindow.position.set(penthouseWidth / 2 - 0.05, penthouseHeight / 2, 0);
    rightWindow.rotation.y = -Math.PI / 2;
    room1.add(rightWindow);
    
    // City view right
    const cityView3 = new THREE.Mesh(
        new THREE.PlaneGeometry(penthouseDepth * 0.9, penthouseHeight * 0.95),
        cityViewMaterial
    );
    cityView3.position.set(penthouseWidth / 2 + 2, penthouseHeight / 2, 0);
    cityView3.rotation.y = -Math.PI / 2;
    room1.add(cityView3);
    
    // ========================================
    // WINDOW FRAMES (Brushed Chrome)
    // ========================================
    
    const createWindowFrame = (width, height, x, y, z, rotY) => {
        const frameThickness = 0.05;
        const frameDepth = 0.12;
        
        const positions = [
            // Vertical
            { w: frameThickness, h: height, offsetX: -width / 2, offsetY: 0 },
            { w: frameThickness, h: height, offsetX: width / 2, offsetY: 0 },
            // Horizontal
            { w: width, h: frameThickness, offsetX: 0, offsetY: height / 2 },
            { w: width, h: frameThickness, offsetX: 0, offsetY: -height / 2 }
        ];
        
        positions.forEach(pos => {
            const frame = new THREE.Mesh(
                new THREE.BoxGeometry(pos.w, pos.h, frameDepth),
                chromeMaterial
            );
            frame.position.set(x + pos.offsetX, y + pos.offsetY, z);
    ...
    createWindowFrame(penthouseWidth * 0.9, penthouseHeight * 0.95, 0, penthouseHeight / 2, penthouseDepth / 2, 0);
    createWindowFrame(penthouseDepth * 0.9, penthouseHeight * 0.95, -penthouseWidth / 2, penthouseHeight / 2, 0, Math.PI / 2);
    createWindowFrame(penthouseDepth * 0.9, penthouseHeight * 0.95, penthouseWidth / 2, penthouseHeight / 2, 0, -Math.PI / 2);
    
    // ========================================
    // BACK WALL (White Feature Wall)
    // ========================================
    
    const backWall = new THREE.Mesh(
        new THREE.BoxGeometry(penthouseWidth, penthouseHeight, 0.25),
        wallMaterial
    );
    backWall.position.set(0, penthouseHeight / 2, -penthouseDepth / 2);
    backWall.receiveShadow = true;
    room1.add(backWall);
    
    // Backlit panels on back wall
    const panelPositions = [
        { x: -6, y: 1.8 }, { x: 0, y: 1.8 }, { x: 6, y: 1.8 }
    ];
    
    panelPositions.forEach(pos => {
        const panel = new THREE.Mesh(
            new THREE.BoxGeometry(2.5, 2.5, 0.08),
        ...
        panel.position.set(pos.x, pos.y, -penthouseDepth / 2 + 0.2);
        room1.add(panel);
        
        // Hidden light behind panel
        const backlight = new THREE.PointLight(0xffffff, 1.5, 4);
        backlight.position.set(pos.x, pos.y, -penthouseDepth / 2 + 0.15);
        room1.add(backlight);
    });
    
    // ========================================
    // CEILING (Modern Dropped Design)
    // ========================================
    ...
    ceiling.position.y = penthouseHeight;
    ceiling.receiveShadow = true;
    room1.add(ceiling);
    
    // Recessed lighting coves (perimeter)
    const covePositions = [
        { x: -penthouseWidth / 2 + 0.5, z: 0, w: 0.4, d: penthouseDepth * 0.9 },
        { x: penthouseWidth / 2 - 0.5, z: 0, w: 0.4, d: penthouseDepth * 0.9 },
        { x: 0, z: -penthouseDepth / 2 + 0.5, w: penthouseWidth * 0.9, d: 0.4 }
    ];
    
    covePositions.forEach(cove => {
        const coveBox = new THREE.Mesh(
            new THREE.BoxGeometry(cove.w, 0.15, cove.d),
            new THREE.MeshStandardMaterial({
                color: 0xffffff,
                emissive: 0xffffee,
        ...
        coveBox.position.set(cove.x, penthouseHeight - 0.08, cove.z);
        room1.add(coveBox);
        
        // LED strip light
        const ledStrip = new THREE.PointLight(0xffffee, 1.2, 15);
        ledStrip.position.set(cove.x, penthouseHeight - 0.2, cove.z);
        room1.add(ledStrip);
    });
    ...
    const spotlightGrid = [
        { x: -6, z: -5 }, { x: 0, z: -5 }, { x: 6, z: -5 },
        { x: -6, z: 0 }, { x: 0, z: 0 }, { x: 6, z: 0 },
        { x: -6, z: 5 }, { x: 0, z: 5 }, { x: 6, z: 5 }
    ];
    
    spotlightGrid.forEach(pos => {
        // Recessed housing
        const housing = new THREE.Mesh(
            new THREE.CylinderGeometry(0.15, 0.12, 0.2, 24),
            chromeMaterial
        );
        housing.position.set(pos.x, penthouseHeight - 0.1, pos.z);
        room1.add(housing);
        ...
        lens.position.set(pos.x, penthouseHeight - 0.25, pos.z);
        room1.add(lens);
        
        // Spotlight
        const spotlight = new THREE.SpotLight(0xffffee, 2.8, 20, Math.PI / 7, 0.5);
        spotlight.position.set(pos.x, penthouseHeight - 0.3, pos.z);
    ...
    // Modern sectional sofa (black leather)
    const sofaBase = new THREE.Mesh(
        new THREE.BoxGeometry(4, 0.4, 1.8),
        velvetMaterial
    );
    sofaBase.position.set(-5, 0.2, 0);
    ...
    sofaBack.position.set(-5, 0.5, -0.75);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        cushion.position.set(-5 + (i - 1) * 1.3, 0.48, -0.3);
    ...
    legOffsets.forEach(leg => {
        const sofaLeg = new THREE.Mesh(
            new THREE.CylinderGeometry(0.04, 0.04, 0.15, 16),
            chromeMaterial
        );
        sofaLeg.position.set(-5 + leg.x, 0.075, leg.z);
    ...
    tableTop.position.set(-5, 0.45, 2.5);
    ...
    tableLegs.forEach(leg => {
        const tableLeg = new THREE.Mesh(
            new THREE.CylinderGeometry(0.03, 0.05, 0.45, 16),
            goldMaterial
        );
        tableLeg.position.set(-5 + leg.x, 0.225, 2.5 + leg.z);
    ...
    chairBase.position.set(3, 0.175, 3);
    chairBase.castShadow = true;
    room1.add(chairBase);
    
    const chairBack = new THREE.Mesh(
        new THREE.BoxGeometry(0.9, 0.8, 0.15),
        new THREE.MeshStandardMaterial({
            color: 0x2a2a2a,
            roughness: 0.9
        })
    );
    chairBack.position.set(3, 0.575, 2.575);
    ...
    lampBase.position.set(7, 0.04, 5);
    room1.add(lampBase);
    ...
    lampPole.position.set(7, 1.1, 5);
    room1.add(lampPole);
    
    const lampShade = new THREE.Mesh(
        new THREE.CylinderGeometry(0.3, 0.35, 0.4, 24, 1, true),
        new THREE.MeshStandardMaterial({
            color: 0xffffff,
            emissive: 0xffffdd,
            emissiveIntensity: 0.8,
    ...
    lampShade.position.set(7, 2.1, 5);
    room1.add(lampShade);
    
    const lampLight = new THREE.PointLight(0xffffdd, 1.8, 8);
    lampLight.position.set(7, 2.1, 5);
    room1.add(lampLight);
    ...
    vase.position.set(-5, 0.53, 2.5);
    vase.castShadow = true;
    room1.add(vase);
    
    // ========================================
    // GLASS ACCENT DIVIDER
    // ========================================
    
    const divider = new THREE.Mesh(
        new THREE.BoxGeometry(0.05, penthouseHeight * 0.7, 4),
        tintedGlassMaterial
    );
    divider.position.set(0, penthouseHeight * 0.35, -4);
    divider.castShadow = true;
    room1.add(divider);
    
    // Gold frame around divider
    const dividerFrame = new THREE.Mesh(
        new THREE.BoxGeometry(0.08, penthouseHeight * 0.7 + 0.1, 0.05),
        goldMaterial
    );
    dividerFrame.position.set(0, penthouseHeight * 0.35, -2);
    room1.add(dividerFrame);
    ...
    // Natural window light (simulated daylight)
    const windowLight = new THREE.DirectionalLight(0xffffee, 1.2);
    windowLight.position.set(0, 10, 20);
    ...
    this.scene.fog = new THREE.Fog(0xf5f5f5, 25, 45);
    
    room1.position.set(0, 0, 0);
    this.rooms.push(room1);
    this.rooms.forEach(room => this.scene.add(room));
    
    console.log("🏙️ Luxury Penthouse Gallery created - Modern, sophisticated, breathtaking views!");
}
