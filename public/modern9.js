...
this.setupMobileControls();
   ...
   addLighting() {
    // Reduced ambient (dramatic gallery lighting)
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.3);
    this.scene.add(ambientLight);

    // Key directional light (simulating natural skylight)
    const skylight = new THREE.DirectionalLight(0xf5f5ff, 1.2);
    skylight.position.set(10, 20, 10);
    skylight.castShadow = true;
    skylight.shadow.mapSize.width = 4096;
    skylight.shadow.mapSize.height = 4096;
...
createGallery() {
    ...
    // Polished concrete floor (industrial chic)
    ...
    // Exposed ceiling (industrial black)
    ...
    // Black aluminum frames (modern minimalist)
    ...
    // Subtle floor joints (expansion joints)
    ...
    // Grid of floor joints
    for (let i = -3; i <= 3; i++) {
        // Horizontal joints
        const jointH = new THREE.Mesh(
            new THREE.BoxGeometry(roomWidth, 0.02, 0.05),
            jointMaterial
        );
        jointH.position.set(0, 0.01, i * 5);
        jointH.rotation.x = -Math.PI / 2;
        room1.add(jointH);
        
        // Vertical joints
        const jointV = new THREE.Mesh(
            new THREE.BoxGeometry(0.05, 0.02, roomDepth),
            jointMaterial
        );
        jointV.position.set(i * 5, 0.01, 0);
        jointV.rotation.x = -Math.PI / 2;
        room1.add(jointV);
    }
    ...
    // Exposed ceiling beams (industrial look)
    ...
    for (let i = -3; i <= 3; i++) {
        const beamH = new THREE.Mesh(
            new THREE.BoxGeometry(roomWidth, 0.3, 0.3),
            beamMaterial
        );
        beamH.position.set(0, roomHeight - 0.15, i * 6);
        beamH.castShadow = true;
        room1.add(beamH);
        
        const beamV = new THREE.Mesh(
            new THREE.BoxGeometry(0.3, 0.3, roomDepth),
            beamMaterial
        );
        beamV.position.set(i * 6, roomHeight - 0.15, 0);
        beamV.castShadow = true;
        room1.add(beamV);
    }

    // ====== SOPHISTICATED TRACK LIGHTING ======
    
    this.ceilingLights = [];
    this.glassSpotlights = [];

    // Main track lighting (professional gallery style)
    const trackPositions = [];
    for (let x = -15; x <= 15; x += 6) {
        for (let z = -15; z <= 15; z += 6) {
            trackPositions.push({ x, z });
        }
    }

    trackPositions.forEach(pos => {
        // Modern track fixture (cylindrical)
        ...
        fixture.position.set(pos.x, roomHeight - 0.4, pos.z);
        room1.add(fixture);

        // Cool white spotlight (5000K color temperature)
        const spotlight = new THREE.SpotLight(0xf5f5ff, 2.0, 20, Math.PI / 5, 0.4);
        spotlight.position.set(pos.x, roomHeight - 0.5, pos.z);
    ...
    // Dedicated artwork spotlights (museum quality)
    const artLightPositions = [
        // Back wall
        { x: -12, z: -19.5, targetZ: -19.8, intensity: 3.0 },
        { x: -6, z: -19.5, targetZ: -19.8, intensity: 3.0 },
        { x: 0, z: -19.5, targetZ: -19.8, intensity: 3.0 },
        { x: 6, z: -19.5, targetZ: -19.8, intensity: 3.0 },
        { x: 12, z: -19.5, targetZ: -19.8, intensity: 3.0 },
        
        // Left wall
        { x: -19.5, z: -10, targetX: -19.8, intensity: 3.0 },
        { x: -19.5, z: 0, targetX: -19.8, intensity: 3.0 },
        { x: -19.5, z: 10, targetX: -19.8, intensity: 3.0 },
        ...
        // Front wall
        { x: -10, z: 19.5, targetZ: 19.8, intensity: 3.0 },
        { x: 0, z: 19.5, targetZ: 19.8, intensity: 3.0 },
        { x: 10, z: 19.5, targetZ: 19.8, intensity: 3.0 }
    ];

    artLightPositions.forEach(lightPos => {
        // Professional LED track fixture
        const fixtureBody = new THREE.Mesh(
            new THREE.CylinderGeometry(0.07, 0.1, 0.2, 16),
            steelMaterial
        );
        fixtureBody.position.set(lightPos.x, roomHeight - 0.3, lightPos.z);
        room1.add(fixtureBody);

        // Precise artwork spotlight (narrow beam)
        const spotLight = new THREE.SpotLight(0xffffff, lightPos.intensity, 18, Math.PI / 8, 0.3);
        spotLight.position.set(lightPos.x, roomHeight - 0.4, lightPos.z);
    ...
    const walls = [
        new THREE.Mesh(new THREE.BoxGeometry(roomWidth, roomHeight, 0.3), wallMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(roomWidth, roomHeight, 0.3), wallMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(0.3, roomHeight, roomDepth), wallMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(0.3, roomHeight, roomDepth), wallMaterial)
    ];
    ...
    // ====== MINIMALIST CENTER SCULPTURE PLATFORM ======
    ...
    const platform = new THREE.Mesh(
        new THREE.CylinderGeometry(2.0, 2.2, 0.15, 32),
        platformMaterial
    );
    ...
    const platformRim = new THREE.Mesh(
        new THREE.TorusGeometry(2.0, 0.02, 16, 32),
        steelMaterial
    );
    platformRim.position.set(0, 0.15, 0);
    platformRim.rotation.x = Math.PI / 2;
    room1.add(platformRim);

    // ====== MODERN SCULPTURAL PIECE (Abstract) ======
    
    const sculptureGroup = new THREE.Group();
    
    // Contemporary abstract sculpture (inspired by Anish Kapoor)
    ...
    for (let i = 0; i < 12; i++) {
        const segment = new THREE.Mesh(
            new THREE.TorusGeometry(0.3 + i * 0.05, 0.08, 16, 32),
            sculptureMaterial
        );
        segment.position.y = 0.2 + i * 0.15;
        segment.rotation.y = (i * Math.PI) / 6;
        segment.rotation.x = Math.PI / 2 + (i * Math.PI) / 12;
        segment.castShadow = true;
        sculptureGroup.add(segment);
    }

    sculptureGroup.position.set(0, 0.15, 0);
    sculptureGroup.castShadow = true;
    room1.add(sculptureGroup);

    // ====== MINIMALIST BENCHES (Scandinavian Design) ======
    ...
    const benchSteelMaterial = new THREE.MeshStandardMaterial({
        color: 0xa0a0a0,
        roughness: 0.3,
        metalness: 0.95
    });
    ...
    benchPositions.forEach(pos => {
        ...
        const seat = new THREE.Mesh(
            new THREE.BoxGeometry(3.5, 0.08, 0.5),
            benchWoodMaterial
        );
        ...
        // Steel legs (modern hairpin style)
        const legPositions = [
            { x: -1.5, z: 0 },
            { x: 1.5, z: 0 }
        ];
        ...
        legPositions.forEach(legPos => {
            ...
            legLeft.position.set(legPos.x - 0.15, 0.22, legPos.z);
            legLeft.rotation.z = 0.1;
            ...
            legRight.position.set(legPos.x + 0.15, 0.22, legPos.z);
            legRight.rotation.z = -0.1;
    ...
    columnPositions.forEach(pos => {
        const column = new THREE.Mesh(
            new THREE.CylinderGeometry(0.4, 0.4, roomHeight, 32),
            columnMaterial
        );
        ...
        // Column capital (modern minimalist)
        const capital = new THREE.Mesh(
            new THREE.CylinderGeometry(0.5, 0.4, 0.15, 32),
            steelMaterial
        );
        capital.position.set(pos.x, roomHeight - 0.075, pos.z);
        room1.add(capital);
    });
    ...
    kioskPositions.forEach(pos => {
        ...
        infoIcon.position.set(0, 1.3, 0.05);
        kioskGroup.add(infoIcon);
        
        kioskGroup.position.set(pos.x, 0, pos.z);
        kioskGroup.rotation.y = pos.rot;
        room1.add(kioskGroup);
    });

    // ====== MINIMAL POTTED PLANTS (Contemporary) ======
    
    const modernPotMaterial = new THREE.MeshStandardMaterial({
        color: 0x2a2a2a,
        roughness: 0.4,
        metalness: 0.7
    });
    ...
    plantPositions.forEach(pos => {
        ...
        // Minimal plant (snake plant style)
        for (let i = 0; i < 5; i++) {
...
generateConcreteTexture(width, height) {
    ...
    for (let i = 0; i < 5000; i++) {
        ...
        const shade = Math.random() * 50 + 100;
