...
this.setupMobileControls();
    ...
    // Polished marble floor (Carrara white with gray veining)
    ...
    const marbleTexture = new THREE.Texture(this.generateBlackMarquina(2048, 2048));
   
    marbleTexture.needsUpdate = true;
    marbleTexture.wrapS = marbleTexture.wrapT = THREE.RepeatWrapping;
    marbleTexture.repeat.set(4, 4);
    marbleFloorMaterial.map = marbleTexture;
    
    const marbleNormalTexture = new THREE.Texture(this.generateMarbleNormalMap(2048, 2048));
    marbleNormalTexture.needsUpdate = true;
    marbleNormalTexture.wrapS = marbleNormalTexture.wrapT = THREE.RepeatWrapping;
    marbleNormalTexture.repeat.set(4, 4);
    marbleFloorMaterial.normalMap = marbleNormalTexture;
    marbleFloorMaterial.normalScale = new THREE.Vector2(0.3, 0.3);

    // Museum white walls (soft cream)
    ...
    // Sophisticated ceiling (off-white with recessed lighting)
    ...
    const baseboards = [
        new THREE.Mesh(new THREE.BoxGeometry(roomWidth, baseboardHeight, baseboardDepth), brassMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(roomWidth, baseboardHeight, baseboardDepth), brassMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(baseboardDepth, baseboardHeight, roomDepth), brassMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(baseboardDepth, baseboardHeight, roomDepth), brassMaterial)
    ];
    
    baseboards[0].position.set(0, baseboardHeight / 2, -roomDepth / 2);
    baseboards[1].position.set(0, baseboardHeight / 2, roomDepth / 2);
    baseboards[2].position.set(-roomWidth / 2, baseboardHeight / 2, 0);
    baseboards[3].position.set(roomWidth / 2, baseboardHeight / 2, 0);
    ...
    // Recessed ceiling coffers (9 panels in 3x3 grid)
    const cofferSize = 8;
    const cofferDepth = 0.3;
    ...
    for (let x = -1; x <= 1; x++) {
        for (let z = -1; z <= 1; z++) {
            const coffer = new THREE.Mesh(
                new THREE.BoxGeometry(cofferSize - 0.5, cofferDepth, cofferSize - 0.5),
                cofferMaterial
            );
            coffer.position.set(
                x * cofferSize,
            ...
            // Brass trim around each coffer
            const cofferTrim = new THREE.Mesh(
                new THREE.TorusGeometry(cofferSize / 2 - 0.25, 0.03, 8, 4),
                brassMaterial
            );
            cofferTrim.position.set(
                x * cofferSize,
                roomHeight - cofferDepth - 0.02,
                z * cofferSize
            );
            cofferTrim.rotation.x = Math.PI / 2;
            room1.add(cofferTrim);
        }
    }

    // ====== SOPHISTICATED LIGHTING ======
    
    // ====== ULTRA-DRAMATIC MUSEUM LIGHTING ======

this.ceilingLights = [];

// Main gallery wash lights (9 coffers)
for (let x = -1; x <= 1; x++) {
    for (let z = -1; z <= 1; z++) {
        // Premium recessed fixture
        ...
        fixture.position.set(
            x * cofferSize,
            roomHeight - cofferDepth - 0.15,
            z * cofferSize
        );
        room1.add(fixture);

        // Warm dramatic spotlight
        const spotlight = new THREE.SpotLight(0xffeaa7, 2.5, 25, Math.PI / 4.5, 0.3);
        spotlight.position.set(
            x * cofferSize,
            roomHeight - cofferDepth - 0.2,
            z * cofferSize
        );
        spotlight.target.position.set(x * cofferSize, 0, z * cofferSize);
        spotlight.castShadow = true;
        spotlight.shadow.mapSize.width = 2048;
        spotlight.shadow.mapSize.height = 2048;
        spotlight.shadow.bias = -0.0001;
        spotlight.shadow.radius = 3; // Soft shadows
        room1.add(spotlight);
...
// Track lighting for artwork (more dramatic)
this.glassSpotlights = [];

const trackLightPositions = [
    // Back wall (5 lights)
    { x: -10, z: -14.5, targetZ: -15, intensity: 2.5 },
    { x: -5, z: -14.5, targetZ: -15, intensity: 2.5 },
    { x: 0, z: -14.5, targetZ: -15, intensity: 2.5 },
    { x: 5, z: -14.5, targetZ: -15, intensity: 2.5 },
    { x: 10, z: -14.5, targetZ: -15, intensity: 2.5 },
    
    // Left wall (3 lights)
    { x: -14.5, z: -8, targetX: -15, intensity: 2.5 },
    { x: -14.5, z: 0, targetX: -15, intensity: 2.5 },
    { x: -14.5, z: 8, targetX: -15, intensity: 2.5 },
    ...
    // Front wall (3 lights)
    { x: -8, z: 14.5, targetZ: 15, intensity: 2.5 },
    { x: 0, z: 14.5, targetZ: 15, intensity: 2.5 },
    { x: 8, z: 14.5, targetZ: 15, intensity: 2.5 }
];

trackLightPositions.forEach(lightPos => {
    // Premium track fixture
    const fixtureBody = new THREE.Mesh(
        new THREE.CylinderGeometry(0.1, 0.14, 0.3, 12),
        brassMaterial
    );
    fixtureBody.position.set(lightPos.x, roomHeight - 0.4, lightPos.z);
    room1.add(fixtureBody);

    // Focused beam spotlight
    const spotLight = new THREE.SpotLight(0xfff5e6, lightPos.intensity, 15, Math.PI / 6, 0.4);
    spotLight.position.set(lightPos.x, roomHeight - 0.5, lightPos.z);
...
// ✨ DRAMATIC UPLIGHTING (floor-mounted accent lights)
...
uplightPositions.forEach(pos => {
    const uplightFixture = new THREE.Mesh(
    ...
    uplightFixture.position.set(pos.x, 0.05, pos.z);
    room1.add(uplightFixture);

    const uplight = new THREE.SpotLight(0xffd700, 1.5, 10, Math.PI / 3, 0.5);
    ...
    const walls = [
        new THREE.Mesh(new THREE.BoxGeometry(roomWidth, roomHeight, 0.4), wallMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(roomWidth, roomHeight, 0.4), wallMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(0.4, roomHeight, roomDepth), wallMaterial),
        new THREE.Mesh(new THREE.BoxGeometry(0.4, roomHeight, roomDepth), wallMaterial)
    ];
    ...
    // ====== ELEGANT CENTER SCULPTURE PEDESTAL ======
    ...
    const brassRing = new THREE.Mesh(
        new THREE.TorusGeometry(1.0, 0.04, 12, 24),
        goldFrameMaterial
    );
    brassRing.position.set(0, 3.3, 0);
    brassRing.rotation.x = Math.PI / 2;
    room1.add(brassRing);

    // ====== MUSEUM BENCHES (Minimalist & Elegant) ======
    // ====== MUSEUM VELVET ROPES (Red Velvet + Brass Stanchions) ======
...
// Rope barriers around center pedestal
const ropePositions = [
    { start: { x: -2.5, z: -2.5 }, end: { x: 2.5, z: -2.5 } },
    { start: { x: 2.5, z: -2.5 }, end: { x: 2.5, z: 2.5 } },
    { start: { x: 2.5, z: 2.5 }, end: { x: -2.5, z: 2.5 } },
    { start: { x: -2.5, z: 2.5 }, end: { x: -2.5, z: -2.5 } }
];
...
ropePositions.forEach(rope => {
    // Start stanchion
    const stanchionStart = new THREE.Group();
    
    const baseStart = new THREE.Mesh(
        new THREE.CylinderGeometry(0.25, 0.3, 0.08, 16),
        stanchionMaterial
    );
    baseStart.position.y = 0.04;
    stanchionStart.add(baseStart);
    
    const poleStart = new THREE.Mesh(
        new THREE.CylinderGeometry(0.04, 0.04, 1.2, 16),
        stanchionMaterial
    );
    poleStart.position.y = 0.64;
    stanchionStart.add(poleStart);
    
    const topStart = new THREE.Mesh(
        new THREE.SphereGeometry(0.08, 16, 16),
        stanchionMaterial
    );
    topStart.position.y = 1.28;
    stanchionStart.add(topStart);
    
    stanchionStart.position.set(rope.start.x, 0, rope.start.z);
    stanchionStart.castShadow = true;
    room1.add(stanchionStart);
    
    // End stanchion
    const stanchionEnd = stanchionStart.clone();
    stanchionEnd.position.set(rope.end.x, 0, rope.end.z);
    room1.add(stanchionEnd);
    
    // Velvet rope
    const ropeLength = Math.sqrt(
        Math.pow(rope.end.x - rope.start.x, 2) + 
        Math.pow(rope.end.z - rope.start.z, 2)
    );
    
    const ropeMesh = new THREE.Mesh(
        new THREE.CylinderGeometry(0.03, 0.03, ropeLength, 16),
        velvetMaterial
    );
    
    const midX = (rope.start.x + rope.end.x) / 2;
    const midZ = (rope.start.z + rope.end.z) / 2;
    ropeMesh.position.set(midX, 1.1, midZ);
    
    const angle = Math.atan2(rope.end.z - rope.start.z, rope.end.x - rope.start.x);
    ropeMesh.rotation.z = Math.PI / 2;
    ropeMesh.rotation.y = angle;
    
    room1.add(ropeMesh);
    
    // Gold tassels at rope ends
    const tasselStart = new THREE.Mesh(
        new THREE.ConeGeometry(0.06, 0.15, 8),
        goldFrameMaterial
    );
    tasselStart.position.set(rope.start.x, 1.0, rope.start.z);
    room1.add(tasselStart);
    
    const tasselEnd = tasselStart.clone();
    tasselEnd.position.set(rope.end.x, 1.0, rope.end.z);
    room1.add(tasselEnd);
});



    // ====== LUXURY TUFTED LEATHER BENCHES (Chesterfield Style) ======

const luxuryBenchMaterial = new THREE.MeshStandardMaterial({
    color: 0x3d2817,
    roughness: 0.4,
    metalness: 0.6
});

const tufteLeatherMaterial = new THREE.MeshStandardMaterial({
    color: 0x654321,
    roughness: 0.8,
    metalness: 0.1
});
...
benchPositions.forEach(pos => {
    ...
    // Main seat with tufted leather
    const seat = new THREE.Mesh(
        new THREE.BoxGeometry(3.0, 0.2, 0.9),
        luxuryBenchMaterial
    );
    seat.position.y = 0.5;
    ...
    const cushion = new THREE.Mesh(
        new THREE.BoxGeometry(2.9, 0.15, 0.85),
        tufteLeatherMaterial
    );
    cushion.position.y = 0.63;
    cushion.castShadow = true;
    benchGroup.add(cushion);
    
    // Button tufting details (9 buttons in 3x3 grid)
    ...
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            ...
            button.position.set(i * 0.7, 0.65, j * 0.2);
            button.scale.y = 0.5;
            benchGroup.add(button);
        }
    }
    
    // Elegant carved wooden legs (4 legs)
    ...
    legPositions.forEach(legPos => {
        ...
        leg.position.set(legPos.x, 0.225, legPos.z);
        leg.castShadow = true;
        benchGroup.add(leg);
        
        // Decorative foot cap (brass)
        const cap = new THREE.Mesh(
            new THREE.SphereGeometry(0.09, 8, 8),
            brassMaterial
        );
        cap.position.set(legPos.x, 0.05, legPos.z);
        cap.scale.y = 0.5;
        benchGroup.add(cap);
    });
    
    // Brass decorative rail under seat
...
// ====== MUSEUM SCULPTURES & DECORATIVE OBJECTS ======

// **1. CLASSICAL BUST on Center Pedestal**
...
nameplate.position.set(0, 3.05, 0.5);
bust.add(nameplate);

bust.castShadow = true;
room1.add(bust);

// **2. MODERN ABSTRACT SCULPTURE (Corner)**
const modernSculptureMaterial = new THREE.MeshStandardMaterial({
    color: 0x2a2a2a,
    roughness: 0.2,
    metalness: 0.9
});

const modernSculpture = new THREE.Group();

// Twisted geometric form
for (let i = 0; i < 8; i++) {
    const segment = new THREE.Mesh(
        new THREE.BoxGeometry(0.3, 0.3, 0.3),
        modernSculptureMaterial
    );
    segment.position.y = 0.3 + i * 0.35;
    segment.rotation.y = (i * Math.PI) / 4;
    segment.castShadow = true;
    modernSculpture.add(segment);
}

modernSculpture.position.set(-10, 0, -10);
modernSculpture.scale.set(1.2, 1.2, 1.2);
room1.add(modernSculpture);

// Small pedestal for modern sculpture
const modernPedestal = new THREE.Mesh(
    new THREE.CylinderGeometry(0.6, 0.7, 0.3, 16),
    pedestalMaterial
);
modernPedestal.position.set(-10, 0.15, -10);
modernPedestal.castShadow = true;
modernPedestal.receiveShadow = true;
room1.add(modernPedestal);

// **3. BRONZE HORSE SCULPTURE (Greek Style)**
...
horseNeck.position.set(0.25, 1.55, 0);
...
horseHead.position.set(0.45, 1.75, 0);
horse.add(horseHead);
...
legPositions.forEach(legPos => {
    const leg = new THREE.Mesh(
        new THREE.CylinderGeometry(0.06, 0.05, 0.9, 12),
        bronzeMaterial
    );
    leg.position.set(legPos.x, 0.85, legPos.z);
    horse.add(leg);
});

horse.position.set(10, 0.3, -10);
horse.castShadow = true;
room1.add(horse);

// Pedestal for horse
const horsePedestal = new THREE.Mesh(
    new THREE.BoxGeometry(1.2, 0.6, 1.2),
    pedestalMaterial
);
horsePedestal.position.set(10, 0.3, -10);
horsePedestal.castShadow = true;
horsePedestal.receiveShadow = true;
room1.add(horsePedestal);

// **4. DECORATIVE VASES (Ancient Greek Style)**
...
vasePositions.forEach(pos => {
    ...
    const handleLeft = new THREE.Mesh(handleGeometry, vaseMaterial);
    handleLeft.position.set(-0.3, 1.5, 0);
    handleLeft.rotation.y = Math.PI / 2;
    vase.add(handleLeft);
    
    const handleRight = handleLeft.clone();
    handleRight.position.x = 0.3;
    vase.add(handleRight);
    ...
    vase.position.set(pos.x, 0, pos.z);
    vase.castShadow = true;
    room1.add(vase);
});

// **5. DECORATIVE WALL SCONCES (4 walls)**
...
sconcePositions.forEach(pos => {
    ...
    holder.position.set(0.05, 0.3, 0);
    sconce.add(holder);
    ...
    flame.position.set(0.05, 0.4, 0);
    sconce.add(flame);
    
    // Point light for realistic glow
    const sconceLight = new THREE.PointLight(0xffd700, 0.5, 5);
    sconceLight.position.set(0.05, 0.4, 0);
    sconce.add(sconceLight);
    
    sconce.position.set(pos.x, 3, pos.z);
    sconce.rotation.y = pos.rot;
    sconce.castShadow = true;
    room1.add(sconce);
});

// ====== GRAND CRYSTAL CHANDELIER (Museum Centerpiece) ======
// Copy this entire block and paste it into your createGallery() method
// Place it AFTER ceiling lights setup, BEFORE sculptures
// ====== CREATIVE LIGHTWEIGHT CHANDELIER ======
// Modern museum elegance with minimal geometry
// Perfect balance: stunning visual + low memory
...
// We'll add these dynamically next to each artwork later
// For now, add sample plaques at fixed positions
const plaquePositions = [
    { x: -13, z: -9, rot: Math.PI / 2 },
    { x: 13, z: -9, rot: -Math.PI / 2 }
];
...
plaquePositions.forEach(pos => {
    ...
    plaque.position.set(pos.x, 2.0, pos.z);
    plaque.rotation.y = pos.rot;
    plaque.castShadow = true;
    room1.add(plaque);
});

    // ====== POTTED PLANTS (Minimalist & Modern) ======
    ...
    plantPositions.forEach(pos => {
        ...
        const pot = new THREE.Mesh(
            new THREE.CylinderGeometry(0.35, 0.3, 0.6, 16),
            potMaterial
        );
        ...
        for (let i = 0; i < 8; i++) {
            ...
            leaf.rotation.z = Math.PI / 6 + Math.random() * Math.PI / 6;
    ...
    this.wallLights = []; // Initialize for compatibility
