...
createGallery() {
    // ========================================
    // FLOATING SKY ISLANDS GALLERY SYSTEM
    // ========================================
    ...
    this.cloudParticles = [];
    this.hotAirBalloons = [];
    this.propellerPlatforms = [];
    this.windTime = 0;
    
    this.createSkybox();
    this.createFloatingIslands();
    this.createChainBridges();
    this.createWaterfalls();
    this.createCloudParticles();
    this.createHotAirBalloons();
    this.createPropellerPlatforms();
    this.createSkyIslandsUI();
    
    console.log("🏝️ Floating Sky Islands Gallery created!");
}
...
// ========================================
// FLOATING ISLANDS (5 major islands)
// ========================================
...
createFloatingIslands() {
    ...
    const islandConfigs = [
        // Main central island (largest)
        { x: 0, y: 0, z: 0, radius: 15, height: 8, type: 'main' },
        // North island
        { x: 0, y: 2, z: -35, radius: 12, height: 6, type: 'north' },
        // South island
        { x: 0, y: -3, z: 35, radius: 10, height: 5, type: 'south' },
        // East island
        { x: 35, y: 1, z: 0, radius: 11, height: 6, type: 'east' },
        // West island
        { x: -35, y: -2, z: 0, radius: 11, height: 6, type: 'west' }
    ];
    
    islandConfigs.forEach((config, index) => {
        const island = this.createIsland(config);
        island.userData.config = config;
        island.userData.floatPhase = index * (Math.PI * 2 / islandConfigs.length);
        island.userData.floatSpeed = 0.3 + Math.random() * 0.2;
        island.userData.floatAmount = 0.5 + Math.random() * 0.3;
        island.userData.baseY = config.y;
...
createIsland(config) {
    ...
    // Main island rock (inverted cone shape)
    ...
    rockMesh.position.y = -config.height / 2;
    rockMesh.castShadow = true;
    rockMesh.receiveShadow = true;
    islandGroup.add(rockMesh);
    
    // Add rocky details (smaller rocks)
    for (let i = 0; i < 8; i++) {
        ...
        smallRock.position.set(
            ...
            -config.height * 0.3 + Math.random() * 2,
    ...
    const grassSurface = new THREE.Mesh(
        new THREE.CylinderGeometry(config.radius, config.radius, 0.5, 32),
        new THREE.MeshStandardMaterial({
            color: 0x90ee90,
            roughness: 0.8
        })
    );
    grassSurface.position.y = 0.25;
    grassSurface.receiveShadow = true;
    islandGroup.add(grassSurface);
    
    // Grass edge (darker rim)
    const grassEdge = new THREE.Mesh(
        new THREE.TorusGeometry(config.radius, 0.3, 8, 32),
        new THREE.MeshStandardMaterial({
            color: 0x228b22,
            roughness: 0.9
        })
    ...
    grassEdge.position.y = 0.25;
    islandGroup.add(grassEdge);
    ...
    const numTrees = Math.floor(config.radius / 2);
    for (let i = 0; i < numTrees; i++) {
        const angle = (i / numTrees) * Math.PI * 2 + Math.random();
        const distance = config.radius * 0.5 + Math.random() * config.radius * 0.3;
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const distance = Math.random() * config.radius * 0.8;
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const distance = config.radius + 3 + Math.random() * 2;
        ...
        floatingRock.position.set(
            Math.cos(angle) * distance,
            height,
            Math.sin(angle) * distance
        );
        floatingRock.rotation.set(
            Math.random() * Math.PI,
            Math.random() * Math.PI,
            Math.random() * Math.PI
        );
        floatingRock.castShadow = true;
        floatingRock.userData.floatPhase = i;
        floatingRock.userData.floatSpeed = 0.5 + Math.random() * 0.5;
        floatingRock.userData.rotationSpeed = (Math.random() - 0.5) * 0.02;
        floatingRock.userData.baseY = height;
        islandGroup.add(floatingRock);
    }
    
    // ========================================
    // ARTWORK DISPLAY STANDS
    // ========================================
    
    if (config.type === 'main') {
        // Central island gets 6 artworks in a circle
        this.skyArtworkSpots = [];
        for (let i = 0; i < 6; i++) {
            ...
            this.skyArtworkSpots.push({
                x: config.x + Math.cos(angle) * distance,
                y: config.y + 3,
                z: config.z + Math.sin(angle) * distance,
                rot: angle + Math.PI
            });
        }
    } else {
        // Other islands get 2-3 artworks
        const numArtworks = 2 + Math.floor(Math.random() * 2);
        for (let i = 0; i < numArtworks; i++) {
            ...
            if (!this.skyArtworkSpots) this.skyArtworkSpots = [];
            this.skyArtworkSpots.push({
                x: config.x + Math.cos(angle) * distance,
                y: config.y + 3,
                z: config.z + Math.sin(angle) * distance,
    ...
    islandGroup.position.set(config.x, config.y, config.z);
    return islandGroup;
}

// ========================================
// CHAIN BRIDGES (connect islands)
// ========================================

createChainBridges() {
    const bridgeConnections = [
        { from: 0, to: 1 }, // Main to North
        { from: 0, to: 2 }, // Main to South
        { from: 0, to: 3 }, // Main to East
        { from: 0, to: 4 }  // Main to West
    ];
    
    bridgeConnections.forEach(connection => {
        const island1 = this.islands[connection.from];
        const island2 = this.islands[connection.to];
        
        const bridge = this.createChainBridge(
            island1.position,
            island2.position,
            island1.userData.config.radius,
            island2.userData.config.radius
        );
        
        this.rooms[0].add(bridge);
        this.bridges.push(bridge);
    });
}

createChainBridge(pos1, pos2, radius1, radius2) {
    const bridgeGroup = new THREE.Group();
    
    // Calculate bridge endpoints (at island edges)
    const direction = new THREE.Vector3().subVectors(pos2, pos1).normalize();
    const start = pos1.clone().add(direction.clone().multiplyScalar(radius1));
    const end = pos2.clone().sub(direction.clone().multiplyScalar(radius2));
    
    const distance = start.distanceTo(end);
    const midPoint = new THREE.Vector3().addVectors(start, end).multiplyScalar(0.5);
    midPoint.y -= 3; // Sag in the middle
    
    // ========================================
    // WOODEN PLANKS (walkway)
    // ========================================
    
    const numPlanks = Math.floor(distance / 1.5);
    ...
    for (let i = 0; i < numPlanks; i++) {
        ...
        plank.userData.swayPhase = i * 0.2;
        bridgeGroup.add(plank);
    }
    
    // ========================================
    // ROPE RAILINGS (both sides)
    // ========================================
    ...
    [-0.8, 0.8].forEach(offset => {
        ...
        for (let i = 0; i <= 20; i++) {
            ...
            const offsetPos = new THREE.Vector3(x, y, z).add(perpendicular.multiplyScalar(offset));
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const x = start.x + (end.x - start.x) * t;
        const z = start.z + (end.z - start.z) * t;
        const y = start.y + (end.y - start.y) * t + Math.sin(t * Math.PI) * -3;
        
        [-0.8, 0.8].forEach(offset => {
            const perpendicular = new THREE.Vector3(-direction.z, 0, direction.x).normalize();
            const chainPos = new THREE.Vector3(x, y, z).add(perpendicular.multiplyScalar(offset));
            ...
            chain.position.copy(chainPos);
            chain.position.y += 0.75;
            bridgeGroup.add(chain);
        });
    }
    
    bridgeGroup.userData.planks = bridgeGroup.children.filter(c => 
        c.geometry?.type === 'BoxGeometry' && c.material?.color?.getHex() === 0x8b4513
    );
...
createWaterfalls() {
    // Add waterfalls to edge of each island
    this.islands.forEach((island, index) => {
        if (index === 0) return; // Skip main island
        
        const config = island.userData.config;
        const waterfall = this.createWaterfall(config);
        island.add(waterfall);
        this.waterfalls.push(waterfall);
    });
}

createWaterfall(config) {
    const waterfallGroup = new THREE.Group();
    
    // Water stream (vertical plane with transparency)
    ...
    waterStream.position.set(config.radius - 2, -10, 0);
    waterStream.rotation.y = Math.PI / 4;
    waterfallGroup.add(waterStream);
    
    // Water particles falling
    ...
    for (let i = 0; i < 30; i++) {
        ...
        particle.position.set(
            config.radius - 2 + (Math.random() - 0.5) * 1.5,
            Math.random() * 20,
            (Math.random() - 0.5) * 0.5
        );
        particle.userData.fallSpeed = 0.1 + Math.random() * 0.1;
        particle.userData.maxHeight = 0;
        particleGroup.add(particle);
    }
    waterfallGroup.add(particleGroup);
    waterfallGroup.userData.particles = particleGroup;
    ...
    mist.position.set(config.radius - 2, -20, 0);
    mist.scale.set(1, 0.5, 1);
    waterfallGroup.add(mist);
    waterfallGroup.userData.mist = mist;
    
    return waterfallGroup;
}

// ========================================
// CLOUD PARTICLE SYSTEM (ambient)
// ========================================
...
createHotAirBalloons() {
    for (let i = 0; i < 3; i++) {
        ...
        balloon.userData.orbitPhase = (i / 3) * Math.PI * 2;
        balloon.userData.orbitRadius = 40 + Math.random() * 20;
        balloon.userData.orbitSpeed = 0.001 + Math.random() * 0.001;
        balloon.userData.bobPhase = Math.random() * Math.PI * 2;
        balloon.userData.bobSpeed = 0.5 + Math.random() * 0.5;
        balloon.userData.bobAmount = 1 + Math.random() * 2;
...
createHotAirBalloon() {
    ...
    const envelope = new THREE.Mesh(
        new THREE.SphereGeometry(3, 16, 16, 0, Math.PI * 2, 0, Math.PI * 0.7),
        new THREE.MeshStandardMaterial({
            color: balloonColor,
            roughness: 0.6,
            metalness: 0.1
    ...
    for (let i = 0; i < 4; i++) {
        ...
        const points = [
            new THREE.Vector3(Math.cos(angle) * 0.7, 1, Math.sin(angle) * 0.7),
            new THREE.Vector3(Math.cos(angle) * 2, 5, Math.sin(angle) * 2)
        ];
        const ropeCurve = new THREE.CatmullRomCurve3(points);
        const ropeGeometry = new THREE.TubeGeometry(ropeCurve, 10, 0.03, 4, false);
...
// ========================================
// PROPELLER PLATFORMS (moving)
// ========================================

createPropellerPlatforms() {
    // Small floating platforms with propellers
    for (let i = 0; i < 3; i++) {
        const platform = this.createPropellerPlatform();
        platform.position.set(
            (i - 1) * 20,
            5 + i * 3,
            -20 + i * 10
        );
        platform.userData.pathPhase = (i / 3) * Math.PI * 2;
        platform.userData.pathRadius = 10;
        platform.userData.pathSpeed = 0.002;
        platform.userData.centerX = platform.position.x;
        platform.userData.centerZ = platform.position.z;
        
        this.rooms[0].add(platform);
        this.propellerPlatforms.push(platform);
    }
}

createPropellerPlatform() {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        // Propeller blades
        for (let j = 0; j < 3; j++) {
            const bladeAngle = (j / 3) * Math.PI * 2;
            ...
            blade.position.x = Math.cos(bladeAngle) * 0.4;
            blade.position.z = Math.sin(bladeAngle) * 0.4;
        ...
        propellerGroup.userData.rotationSpeed = 0.2;
        platformGroup.add(propellerGroup);
        propellers.push(propellerGroup);
    }
    
    platformGroup.userData.propellers = propellers;
...
createSkyIslandsUI() {
    const skyUI = document.createElement('div');
    skyUI.id = 'skyIslandsUI';
    skyUI.style.cssText = `
        position: fixed;
        top: 20px;
        left: 50%;
   ...
   ;
    ...
    document.body.appendChild(skyUI);
}

// ========================================
// SKY ISLANDS ANIMATIONS
// ========================================

updateSkyIslandsAnimations() {
    const time = this.time || Date.now() * 0.001;
    
    // 1. FLOATING ISLANDS (gentle bobbing)
    if (this.islands) {
        this.islands.forEach(island => {
            island.position.y = island.userData.baseY + 
                Math.sin(time * island.userData.floatSpeed + island.userData.floatPhase) * 
                island.userData.floatAmount;
            
            // Rotate floating rocks around each island
            island.children.forEach(child => {
                if (child.userData.rotationSpeed) {
                    ...
                    child.position.y = child.userData.baseY + 
                        Math.sin(time * child.userData.floatSpeed + child.userData.floatPhase) * 0.3;
                }
            });
        });
    }
    ...
    if (this.bridges) {
        this.bridges.forEach((bridge, bridgeIndex) => {
            const planks = bridge.userData.planks;
            if (planks) {
                planks.forEach((plank, plankIndex) => {
                    const sway = Math.sin(time * 2 + plank.userData.swayPhase) * 0.02;
                    plank.rotation.z = sway;
                    plank.position.y += Math.sin(time + plankIndex * 0.3) * 0.005;
                });
            }
        });
    }
    ...
    if (this.waterfalls) {
        this.waterfalls.forEach(waterfall => {
            // Animate water particles falling
            const particles = waterfall.userData.particles;
            if (particles) {
            ...
            // Pulsing mist
            const mist = waterfall.userData.mist;
            if (mist) {
                mist.scale.setScalar(1 + Math.sin(time * 2) * 0.1);
                mist.material.opacity = 0.25 + Math.sin(time * 1.5) * 0.1;
            }
        });
    }
    
    // 4. CLOUD PARTICLES DRIFTING
    if (this.cloudParticles) {
        this.cloudParticles.forEach(cloud => {
            ...
            // Gentle vertical bob
            cloud.position.y += Math.sin(time * 0.5 + cloud.position.x * 0.1) * 0.01;
        });
    }
    
    // 5. HOT AIR BALLOONS ORBITING
    if (this.hotAirBalloons) {
        this.hotAirBalloons.forEach(balloon => {
            // Circular orbit
            balloon.userData.orbitPhase += balloon.userData.orbitSpeed;
            balloon.position.x = Math.cos(balloon.userData.orbitPhase) * balloon.userData.orbitRadius;
            balloon.position.z = Math.sin(balloon.userData.orbitPhase) * balloon.userData.orbitRadius;
            
            // Vertical bobbing
            balloon.userData.bobPhase += 0.02 * balloon.userData.bobSpeed;
            balloon.position.y += Math.sin(balloon.userData.bobPhase) * 0.02;
            
            // Flame flicker
            if (balloon.userData.flame) {
                balloon.userData.flame.scale.y = 1 + Math.sin(time * 10) * 0.2;
                balloon.userData.flame.material.opacity = 0.6 + Math.sin(time * 8) * 0.2;
            }
        });
    }
    
    // 6. PROPELLER PLATFORMS
    if (this.propellerPlatforms) {
        this.propellerPlatforms.forEach(platform => {
            // Circular path
            platform.userData.pathPhase += platform.userData.pathSpeed;
            platform.position.x = platform.userData.centerX + 
                Math.cos(platform.userData.pathPhase) * platform.userData.pathRadius;
            platform.position.z = platform.userData.centerZ + 
                Math.sin(platform.userData.pathPhase) * platform.userData.pathRadius;
            
            // Rotate propellers
            platform.userData.propellers.forEach(propeller => {
                propeller.rotation.y += propeller.userData.rotationSpeed;
            });
        });
    }
    
    // 7. WIND TURBULENCE (camera shake)
    if (!this.isFocused && Math.random() < 0.3) {
        const windStrength = 0.002;
        this.camera.position.x += (Math.random() - 0.5) * windStrength;
        this.camera.position.y += (Math.random() - 0.5) * windStrength * 0.5;
    }
}
...
animate() {
    ...
    this.updateLighting();
    this.updateSkyIslandsAnimations(); // ✓ ADD THIS LINE
