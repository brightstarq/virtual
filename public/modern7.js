...
// ============================================================================
// OPTIMIZED ENCHANTED FOREST GALLERY - FIXED UNIFORM LIMIT ERROR
// Replace your createGallery() method with this version
// ============================================================================

createGallery() {
    const room1 = new THREE.Group();
    const forestSize = 40;
    const forestHeight = 12;

    // **SHARED MATERIALS (CRITICAL: Reuse to prevent uniform overflow)**
    ...
    const glowMushroomMaterial = new THREE.MeshStandardMaterial({
        color: 0xff69b4,
        emissive: 0xff69b4,
    ...
    const floor = new THREE.Mesh(
        new THREE.CircleGeometry(forestSize, 64),
        mossMaterial
    );
    ...
    // **SCATTERED ROCKS** (Reduced count, shared material)
    for (let i = 0; i < 20; i++) { // Reduced from 30
        const rock = new THREE.Mesh(
            new THREE.DodecahedronGeometry(0.3 + Math.random() * 0.4, 0),
            rockMaterial // Shared material
        );
        rock.position.set(
            (Math.random() - 0.5) * forestSize * 1.5,
            0.15,
            (Math.random() - 0.5) * forestSize * 1.5
        );
        rock.rotation.set(
    ...
    // **GIANT TREES** (5 main trees with shared materials)
    const treePositions = [
        { x: 0, z: 0 },
        { x: -12, z: -12 },
        { x: 12, z: -12 },
    ...
    treePositions.forEach((pos) => {
        ...
        const trunk = new THREE.Mesh(
            new THREE.CylinderGeometry(trunkRadius, trunkRadius + 0.3, trunkHeight, 16),
            barkMaterial // Shared
        );
        ...
        for (let i = 0; i < 8; i++) {
            const ring = new THREE.Mesh(
                new THREE.TorusGeometry(trunkRadius + 0.05, 0.1, 8, 16),
                barkRingMaterial // Shared
            );
        ...
        for (let level = 0; level < canopyLevels; level++) {
            const canopyRadius = 3 - level * 0.5;
            const canopy = new THREE.Mesh(
                new THREE.SphereGeometry(canopyRadius, 8, 8),
                leafMaterial // Shared
            );
            canopy.position.y = trunkHeight + 1 + level * 1.5;
    ...
    for (let i = 0; i < 40; i++) { // Reduced from 50
        ...
        const cap = new THREE.Mesh(
            ...
            glowMushroomMaterial // Shared
        );
        cap.position.y = 0.35;
        mushroomGroup.add(cap);
        
        // CRITICAL: Only add lights to every 4th mushroom
        if (i % 4 === 0) {
            const mushroomLight = new THREE.PointLight(0xff69b4, 0.5, 3);
            mushroomLight.position.y = 0.35;
        ...
        mushroomGroup.scale.set(
            0.5 + Math.random() * 1,
            0.5 + Math.random() * 1,
            0.5 + Math.random() * 1
        );
        
        room1.add(mushroomGroup);
    }

    // **FIREFLIES** (Reduced count, shared material)
    for (let i = 0; i < 100; i++) { // Reduced from 150
        const firefly = new THREE.Mesh(
            new THREE.SphereGeometry(0.04, 6, 6),
            fireflyMaterial // Shared material!
        );
        firefly.position.set(
            (Math.random() - 0.5) * forestSize * 1.5,
            Math.random() * forestHeight,
            (Math.random() - 0.5) * forestSize * 1.5
        );
        firefly.userData.floatSpeed = Math.random() * 0.02 + 0.01;
        firefly.userData.floatOffset = Math.random() * Math.PI * 2;
        firefly.userData.driftX = (Math.random() - 0.5) * 0.03;
        firefly.userData.driftZ = (Math.random() - 0.5) * 0.03;
        firefly.userData.glowSpeed = Math.random() * 2 + 1;
        
        room1.add(firefly);
    }

    // **HANGING VINES** (Shared material)
    for (let i = 0; i < 20; i++) {
        const angle = (i / 20) * Math.PI * 2;
        const radius = 15 + Math.random() * 10;
        
        const vine = new THREE.Mesh(
            new THREE.CylinderGeometry(0.03, 0.02, 4 + Math.random() * 3, 8),
            vineMaterial // Shared
        );
        vine.position.set(
        ...
        vine.userData.swaySpeed = 0.001 + Math.random() * 0.001;
        vine.userData.swayAmount = 0.1 + Math.random() * 0.1;
        
        room1.add(vine);
    }

    // **BAMBOO CORNERS** (Shared material)
    ...
    bambooPositions.forEach(pos => {
        for (let i = 0; i < 5; i++) {
            const stalk = new THREE.Mesh(
                new THREE.CylinderGeometry(0.08, 0.06, 3.5, 12),
                bambooMaterial // Shared
            );
            stalk.position.set(
    ...
    // **MYSTICAL TREE** (Center feature)
    const mysticalTrunk = new THREE.Mesh(
        new THREE.CylinderGeometry(2, 2.5, forestHeight, 16),
    ...
    mysticalTrunk.position.set(0, forestHeight / 2, 0);
    room1.add(mysticalTrunk);

    // Glowing runes on center tree (Reduced lights)
    for (let i = 0; i < 8; i++) {
        ...
        // CRITICAL: Only add lights to every other rune
        if (i % 2 === 0) {
            const runeLight = new THREE.PointLight(0x00ffaa, 0.8, 5);
    ...
    const ambientLight = new THREE.AmbientLight(0x3a4f2a, 0.5); // Slightly brighter
    room1.add(ambientLight);

    const moonlight = new THREE.DirectionalLight(0xb0c4de, 0.8);
    moonlight.position.set(10, forestHeight + 5, 10);
    ...
    const rimLight = new THREE.DirectionalLight(0x6a8fb0, 0.3);
    rimLight.position.set(-8, forestHeight + 3, -8);
    room1.add(rimLight);
    ...
    console.log("🌲 Optimized enchanted forest gallery created (uniform limit fixed)");
}
...
toggleHelpOverlay() {
 ...
 this.scene.traverse((child) => {
    if (child.userData.floatSpeed) {
        ...
        if (child.userData.glowSpeed) {
            child.material.emissiveIntensity = 2.0 + Math.sin(time * child.userData.glowSpeed) * 1.5;
        }
        
        // Wrap around
        if (Math.abs(child.position.x) > 40) child.position.x *= -1;
        if (Math.abs(child.position.z) > 40) child.position.z *= -1;
    }
