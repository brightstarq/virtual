...
createGallery() {
    ...
    const studioWidth = 18;  // 18 meters (60 feet) - real loft size
    const studioDepth = 24;  // 24 meters (80 feet)
    const studioHeight = 5;  // 5 meters (16 feet) - industrial ceiling height
    
    // ========================================
    // REALISTIC MATERIALS
    // ========================================
    
    // Polished concrete floor with subtle variations
    ...
    // Add procedural concrete texture
    const concreteCanvas = this.generateConcreteTexture(2048, 2048);
    const concreteTexture = new THREE.CanvasTexture(concreteCanvas);
    concreteTexture.wrapS = concreteTexture.wrapT = THREE.RepeatWrapping;
    concreteTexture.repeat.set(6, 8);
    ...
    // Weathered wood (reclaimed beams)
    ...
    const floor = new THREE.Mesh(
        new THREE.PlaneGeometry(studioWidth, studioDepth),
    ...
    // Paint splatters on floor (colorful spots)
    const paintColors = [0xff6b6b, 0x4ecdc4, 0xffe66d, 0x95e1d3, 0xf38181, 0xaa96da];
    for (let i = 0; i < 80; i++) {
        const splatter = new THREE.Mesh(
            new THREE.CircleGeometry(Math.random() * 0.15 + 0.05, 16),
        ...
        splatter.position.set(
            (Math.random() - 0.5) * studioWidth * 0.9,
            0.01,
            (Math.random() - 0.5) * studioDepth * 0.9
        );
        room1.add(splatter);
    }
    
    // ========================================
    // EXPOSED BRICK WALLS (Two walls)
    // ========================================
    
    // Create realistic brick pattern
    const createBrickWall = (width, height, posX, posY, posZ, rotY) => {
        const brickWidth = 0.25;
        const brickHeight = 0.12;
        const mortarGap = 0.02;
        
        const rows = Math.floor(height / (brickHeight + mortarGap));
        const cols = Math.floor(width / (brickWidth + mortarGap));
        
        for (let row = 0; row < rows; row++) {
            const offset = (row % 2) * (brickWidth / 2); // Staggered pattern
            for (let col = 0; col < cols; col++) {
                const brick = new THREE.Mesh(
                    new THREE.BoxGeometry(brickWidth, brickHeight, 0.15),
                    new THREE.MeshStandardMaterial({
                        color: new THREE.Color().setHex(
                            [0x8b4513, 0x7a3a2e, 0x9b4f3a][Math.floor(Math.random() * 3)]
                        ),
                        roughness: 0.95,
                        metalness: 0.0
                    })
                ...
                const x = posX + (col * (brickWidth + mortarGap) + offset - width / 2);
                const y = posY + (row * (brickHeight + mortarGap) - height / 2);
                
                brick.position.set(x, y, posZ);
                brick.rotation.y = rotY;
    ...
    // Left brick wall
    createBrickWall(studioDepth, studioHeight, -studioWidth / 2, studioHeight / 2, 0, Math.PI / 2);
    
    // Back brick wall (partial)
    createBrickWall(studioWidth * 0.6, studioHeight, 0, studioHeight / 2, -studioDepth / 2, 0);
    
    // ========================================
    // WHITE PAINTED WALLS
    // ========================================
    
    // Right wall (white drywall)
    const rightWall = new THREE.Mesh(
        new THREE.BoxGeometry(studioDepth, studioHeight, 0.2),
        drywallMaterial
    );
    rightWall.position.set(studioWidth / 2, studioHeight / 2, 0);
    rightWall.rotation.y = Math.PI / 2;
    rightWall.receiveShadow = true;
    room1.add(rightWall);
    
    // Front wall section (white, with window cutout handled separately)
    const frontWallLeft = new THREE.Mesh(
        new THREE.BoxGeometry(4, studioHeight, 0.2),
        drywallMaterial
    );
    frontWallLeft.position.set(-studioWidth / 2 + 2, studioHeight / 2, studioDepth / 2);
    ...
    frontWallRight.position.set(studioWidth / 2 - 2, studioHeight / 2, studioDepth / 2);
    ...
    // Floor-to-ceiling window on front wall
    const windowWidth = studioWidth * 0.5;
    const windowHeight = studioHeight * 0.85;
    ...
    windowGlass.position.set(0, studioHeight / 2, studioDepth / 2 + 0.05);
    room1.add(windowGlass);
    
    // Industrial window frames (steel)
    const frameThickness = 0.08;
    const framePositions = [
        // Vertical frames
        { w: frameThickness, h: windowHeight, x: -windowWidth / 2, y: 0 },
        { w: frameThickness, h: windowHeight, x: windowWidth / 2, y: 0 },
        { w: frameThickness, h: windowHeight, x: 0, y: 0 }, // Center mullion
        // Horizontal frames
        { w: windowWidth, h: frameThickness, x: 0, y: windowHeight / 2 },
        { w: windowWidth, h: frameThickness, x: 0, y: -windowHeight / 2 },
    ];
    
    framePositions.forEach(frame => {
        const framePiece = new THREE.Mesh(
            new THREE.BoxGeometry(frame.w, frame.h, 0.12),
            steelMaterial
        );
        framePiece.position.set(frame.x, studioHeight / 2 + frame.y, studioDepth / 2 + 0.1);
    ...
    skylightPositions.forEach(pos => {
        ...
        skylight.position.set(pos.x, studioHeight - 0.05, pos.z);
        room1.add(skylight);
        ...
        frame.position.set(pos.x, studioHeight - 0.1, pos.z);
        room1.add(frame);
        
        // Natural light from skylight
        const skylightLight = new THREE.PointLight(0xffffee, 2.5, 15);
        skylightLight.position.set(pos.x, studioHeight - 0.5, pos.z);
    ...
    for (let z = -studioDepth / 2 + 3; z < studioDepth / 2; z += 6) {
        const beam = new THREE.Mesh(
            new THREE.BoxGeometry(studioWidth, 0.3, 0.25),
            steelMaterial
        );
        beam.position.set(0, studioHeight - 0.15, z);
    ...
    ductPositions.forEach(pos => {
        ...
        duct.position.set(pos.x, studioHeight - 0.5, pos.z);
        room1.add(duct);
    });
    ...
    // Authentic track system
    for (let x = -6; x <= 6; x += 6) {
        // Black aluminum track
        const track = new THREE.Mesh(
            new THREE.BoxGeometry(0.5, 0.08, studioDepth * 0.9),
            new THREE.MeshStandardMaterial({
                color: 0x1a1a1a,
                roughness: 0.3,
                metalness: 0.9
        ...
        track.position.set(x, studioHeight - 0.6, 0);
        room1.add(track);
        
        // Adjustable spotlights on track
        for (let z = -10; z <= 10; z += 5) {
            // Modern cylindrical fixture
            const fixture = new THREE.Mesh(
                new THREE.CylinderGeometry(0.12, 0.15, 0.35, 16),
                new THREE.MeshStandardMaterial({
                    color: 0x2a2a2a,
                    roughness: 0.25,
                    metalness: 0.85
            ...
            fixture.position.set(x, studioHeight - 0.9, z);
            room1.add(fixture);
            ...
            led.position.set(x, studioHeight - 1.15, z);
            room1.add(led);
            
            // Spotlight
            const spotlight = new THREE.SpotLight(0xffe6cc, 3.0, 18, Math.PI / 5, 0.6);
            spotlight.position.set(x, studioHeight - 1.2, z);
    ...
    pendantPositions.forEach(pos => {
        ...
        cord.position.set(pos.x, studioHeight - 0.75, pos.z);
        room1.add(cord);
        ...
        bulb.position.set(pos.x, studioHeight - 1.6, pos.z);
        room1.add(bulb);
        
        // Warm glow
        const pendantLight = new THREE.PointLight(0xff9933, 1.8, 8);
        pendantLight.position.copy(bulb.position);
        room1.add(pendantLight);
    });
    
    // ========================================
    // WORK TABLES & FURNITURE
    // ========================================
    
    // Large work table (reclaimed wood)
    const tableTop = new THREE.Mesh(
        new THREE.BoxGeometry(4, 0.08, 2),
        woodMaterial
    );
    tableTop.position.set(-6, 0.75, 8);
    ...
    legPositions.forEach(leg => {
        ...
        tableLeg.position.set(-6 + leg.x, 0.375, 8 + leg.z);
        tableLeg.castShadow = true;
        room1.add(tableLeg);
    });
    
    // Paint cans on table
    const canColors = [0xff6b6b, 0x4ecdc4, 0xffe66d];
    canColors.forEach((color, i) => {
        ...
        can.position.set(-6 + (i - 1) * 0.5, 0.92, 8);
        can.castShadow = true;
        room1.add(can);
    });
    ...
    const easelPositions = [
        { x: 3, z: -8, rot: Math.PI / 6 },
        { x: -3, z: -8, rot: -Math.PI / 6 }
    ];
    
    easelPositions.forEach(pos => {
        // Easel frame (simple A-frame)
        const frame = new THREE.Mesh(
            new THREE.BoxGeometry(0.06, 1.8, 0.06),
            woodMaterial
        );
        frame.position.set(pos.x, 0.9, pos.z);
        ...
        canvas.position.set(pos.x, 1.5, pos.z);
        canvas.rotation.y = pos.rot;
        canvas.castShadow = true;
        room1.add(canvas);
    });
    
    // ========================================
    // STORAGE SHELVING (Industrial Metal)
    // ========================================
    
    const shelfUnit = (x, z, rot) => {
        // Vertical posts
        for (let i = 0; i < 2; i++) {
            ...
            post.position.set(x + (i * 2 - 1) * 0.9, 1.25, z);
            post.rotation.y = rot;
            room1.add(post);
        }
        ...
        for (let shelf = 0; shelf < 4; shelf++) {
            const shelfBoard = new THREE.Mesh(
                new THREE.BoxGeometry(2, 0.04, 0.4),
                woodMaterial
            );
            shelfBoard.position.set(x, 0.5 + shelf * 0.6, z);
            shelfBoard.rotation.y = rot;
            room1.add(shelfBoard);
        }
    };
    ...
    const dropCloth = new THREE.Mesh(
        new THREE.PlaneGeometry(3, 3),
        new THREE.MeshStandardMaterial({
            color: 0xd4d4c8,
            roughness: 0.95,
    ...
    dropCloth.rotation.x = -Math.PI / 2;
    dropCloth.position.set(0, 0.02, -3);
    dropCloth.receiveShadow = true;
    room1.add(dropCloth);
    
    // Random paint on drop cloth
    for (let i = 0; i < 30; i++) {
        const splash = new THREE.Mesh(
            new THREE.CircleGeometry(Math.random() * 0.1 + 0.03, 12),
            new THREE.MeshStandardMaterial({
                color: paintColors[Math.floor(Math.random() * paintColors.length)],
                roughness: 0.9
            })
    ...
    const windowLight = new THREE.DirectionalLight(0xffffee, 1.5);
    windowLight.position.set(0, 8, 15);
    ...
    this.scene.fog = new THREE.Fog(0xe8e8e8, 20, 35);
    
    room1.position.set(0, 0, 0);
    this.rooms.push(room1);
    this.rooms.forEach(room => this.scene.add(room));
    
    console.log("🎨 Realistic Artist Studio created - Modern, functional, authentic!");
}
