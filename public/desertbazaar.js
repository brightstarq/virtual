...
this.renderer.toneMappingExposure = 1.8; // ✓ was 1.2, now BRIGHTER
...
addLighting() {
    // ✓ LIGHTER fog
    this.scene.fog = new THREE.FogExp2(0x2a2a3a, 0.008); // was 0.015, now less dense
    
    // ✓ BRIGHTER ambient light
    const ambientLight = new THREE.HemisphereLight(
        0xbbd7ff, // Brighter sky color
        0x665544, // Brighter ground color
        0.6       // was 0.3, now MUCH brighter
    );
    this.scene.add(ambientLight);

    // ✓ BRIGHTER moonlight
    const moonlight = new THREE.DirectionalLight(0xddeeff, 2.0); // was 1.2, now 2.0
    ...
    // ✓ BRIGHTER rim light
    const rimLight = new THREE.DirectionalLight(0xffddaa, 1.2); // was 0.6, now 1.2
    rimLight.position.set(-20, 10, 20);
    this.scene.add(rimLight);

    // ✓ BRIGHTER bounce light
    const bounceLight = new THREE.PointLight(0xffeedd, 1.0, 50); // was 0.4, 30
    bounceLight.position.set(0, 0.5, 0);
    this.scene.add(bounceLight);
    
    // ✓ NEW: Additional fill lights around the bazaar
    const fillLights = [
        { pos: [15, 8, 15], color: 0xffcc99, intensity: 0.8 },
        { pos: [-15, 8, 15], color: 0xffcc99, intensity: 0.8 },
        { pos: [15, 8, -15], color: 0xffcc99, intensity: 0.8 },
        { pos: [-15, 8, -15], color: 0xffcc99, intensity: 0.8 }
    ];
...
setupPostProcessing() {
    ...
    this.ssaoPass = new SAOPass(this.scene, this.camera, false, true);
    this.ssaoPass.params.saoBias = 0.5;
    this.ssaoPass.params.saoIntensity = 0.005;
    this.ssaoPass.params.saoScale = 1;
    this.ssaoPass.params.saoKernelRadius = 100;
    ...
    this.colorGradingPass = new ShaderPass({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                gl_FragColor = vec4(filmicToneMapping(color), texel.a);
            }
        `
    });
...
createAdvancedParticles() {
    ...
    this.dustParticles.userData.velocities = dustVelocities;
...
createFireflies() {
    ...
    for (let i = 0; i < fireflyCount; i++) {
        const angle = (i / fireflyCount) * Math.PI * 2;
        const radius = 20 + Math.random() * 10;
        
        fireflyPositions[i * 3] = Math.cos(angle) * radius;
        fireflyPositions[i * 3 + 1] = 2 + Math.random() * 4;
        fireflyPositions[i * 3 + 2] = Math.sin(angle) * radius;

        const color = new THREE.Color().setHSL(0.15 + Math.random() * 0.1, 1.0, 0.5);
        fireflyColors[i * 3] = color.r;
        fireflyColors[i * 3 + 1] = color.g;
        fireflyColors[i * 3 + 2] = color.b;
    }
    ...
    const fireflyMaterial = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                vec3 pos = position;
                float offset = float(gl_VertexID) * 0.5;
                pos.y += sin(time * 2.0 + offset) * 0.3;
                pos.x += cos(time * 1.5 + offset) * 0.2;
                pos.z += sin(time * 1.8 + offset) * 0.2;
                
                vIntensity = sin(time * 3.0 + offset) * 0.5 + 0.5;
                
                vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
                gl_PointSize = (15.0 / -mvPosition.z) * pixelRatio * (0.5 + vIntensity);
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                vec3 glow = vColor * (1.0 + vIntensity);
    ...
    this.fireflySystem = new THREE.Points(fireflyGeometry, fireflyMaterial);
    this.scene.add(this.fireflySystem);
    this.fireflies.push(this.fireflySystem);
}

createMiniMap() {
    const miniMapSize = 200;
    this.miniMapCamera = new THREE.OrthographicCamera(-30, 30, 30, -30, 0.1, 100);
    this.miniMapCamera.position.set(0, 50, 0);
    this.miniMapCamera.lookAt(0, 0, 0);

    this.miniMapRenderer = new THREE.WebGLRenderer({ alpha: true, antialias: false });
...
toggleMiniMap() {
    ...
    this.miniMapRenderer.domElement.style.display = this.miniMapEnabled ? 'block' : 'none';
    console.log(`🗺️ Mini-map ${this.miniMapEnabled ? 'enabled' : 'disabled'}`);
}
...
setupCinematicPaths() {
    ...
    this.cinematicPath = new THREE.CatmullRomCurve3(points, true);
}
...
createAdvancedUI() {
    ...
    panel.innerHTML = `
        <h3 style="margin: 0 0 10px 0; color: #4CAF50;">⚡ Advanced Settings</h3>
        
        <div style="margin: 10px 0;">
            <label><input type="checkbox" id="bloomToggle" checked> Bloom Effect</label>
        </div>
        <div style="margin: 10px 0;">
            <label><input type="checkbox" id="ssaoToggle" checked> Shadows (SSAO)</label>
        </div>
        <div style="margin: 10px 0;">
            <label><input type="checkbox" id="fireflyToggle" checked> Fireflies</label>
        </div>
        <div style="margin: 10px 0;">
            <label><input type="checkbox" id="dustToggle" checked> Dust Particles</label>
        </div>
        <button id="cinematicBtn" style="width: 100%; padding: 8px; margin: 5px 0; background: #2196F3; border: none; color: white; border-radius: 5px; cursor: pointer;">
            🎬 Cinematic Tour
        </button>
        <button id="miniMapBtn" style="width: 100%; padding: 8px; margin: 5px 0; background: #FF9800; border: none; color: white; border-radius: 5px; cursor: pointer;">
            🗺️ Mini-Map
        </button>
        <button id="sandstormBtn" style="width: 100%; padding: 8px; margin: 5px 0; background: #9C27B0; border: none; color: white; border-radius: 5px; cursor: pointer;">
            🌪️ Sandstorm
        </button>
        <div style="margin: 10px 0;">
            <label>Time: <span id="timeValue">21:00</span>
                <input type="range" id="timeSlider" min="0" max="24" step="0.5" value="21" style="width: 100%;">
            </label>
        </div>
    `;
    ...
    document.getElementById('ssaoToggle').addEventListener('change', (e) => {
        this.ssaoPass.enabled = e.target.checked;
    });
    document.getElementById('fireflyToggle').addEventListener('change', (e) => {
        this.fireflies.forEach(f => f.visible = e.target.checked);
    });
    document.getElementById('dustToggle').addEventListener('change', (e) => {
        if (this.dustParticles) this.dustParticles.visible = e.target.checked;
    });
    ...
    document.getElementById('sandstormBtn').addEventListener('click', () => {
        this.toggleSandstorm();
    });
    document.getElementById('timeSlider').addEventListener('input', (e) => {
        this.timeOfDay = parseFloat(e.target.value);
        const hours = Math.floor(this.timeOfDay);
        const mins = Math.round((this.timeOfDay % 1) * 60);
...
toggleSandstorm() {
    if (this.sandStorm) {
        this.scene.remove(this.sandStorm);
        this.sandStorm.geometry.dispose();
        this.sandStorm.material.dispose();
        this.sandStorm = null;
        this.scene.fog.density = 0.012;
        console.log("🏜️ Sandstorm stopped");
    } else {
        this.createSandstorm();
        this.scene.fog.density = 0.025;
        console.log("🌪️ Sandstorm started");
    }
}
...
createSandstorm() {
    ...
    this.sandStorm = new THREE.Points(geo, mat);
    this.scene.add(this.sandStorm);
}

updateSandstorm() {
    if (!this.sandStorm) return;
    const positions = this.sandStorm.geometry.attributes.position.array;
    
    for (let i = 0; i < positions.length; i += 3) {
        positions[i] += Math.sin(this.time * 2 + i) * 0.1;
        positions[i + 1] -= 0.05;
        positions[i + 2] += Math.cos(this.time * 1.5 + i) * 0.08;
    ...
    this.sandStorm.geometry.attributes.position.needsUpdate = true;
}
...
updateTimeOfDay() {
    ...
    const fogColor = new THREE.Color().setHSL(
        0.62, // Blue hue
        0.35, // More saturation
        0.08 + moonIntensity * 0.18 // Darker to lighter
    );
    this.scene.fog.color.copy(fogColor);
    this.renderer.setClearColor(fogColor);
    
    // ✓ Dynamic fog density
    this.scene.fog.density = 0.018 - moonIntensity * 0.005;

    // Update lights...
    this.scene.children.forEach(child => {
        if (child instanceof THREE.DirectionalLight) {
            child.intensity = 0.8 + moonIntensity * 0.6;
        }
    });

    this.lanterns.forEach(lantern => {
        if (lantern.userData.light) {
            lantern.userData.light.intensity = 2.5 + moonIntensity * 1.5;
        }
    });
}
...
createGallery() {
    ...
    // CREATE THE BAZAAR STRUCTURE FIRST (this populates this.rooms[0])
    this.createBazaarStructure();
    
    // Now setup everything else that depends on this.rooms[0]
    this.setupPostProcessing();
    this.createAdvancedParticles();
    this.createFireflies();
    this.createMiniMap();
    this.setupCinematicPaths();
    
    this.createMarketStalls();
    this.createFortuneTellerTent();
    this.createPrayerFlags();
    this.createShootingStars();
    this.createAdvancedUI();
    
    // Continue with other creation methods
...
createFortuneTellerTent() {
    const tent = new THREE.Group();
    
    // Main tent structure (larger, more mysterious)
    ...
    tentCone.position.y = 4;
    tent.add(tentCone);
    
    // Mystical symbols on tent
    ...
    crystalBall.position.set(0, 1, 0);
    tent.add(crystalBall);
    tent.userData.crystalBall = crystalBall;
    ...
    const crystalLight = new THREE.PointLight(0x9966ff, 3, 10);
    crystalLight.position.copy(crystalBall.position);
    tent.add(crystalLight);
    tent.userData.light = crystalLight;
    ...
    [-1, 1].forEach(x => {
        ...
        curtain.position.set(x * 1.5, 2, 4.5);
        curtain.rotation.y = x < 0 ? 0.3 : -0.3;
        tent.add(curtain);
    });
    
    tent.position.set(0, 0, -20);
    tent.userData.isFortuneTent = true;
    this.rooms[0].add(tent);
    this.fortuneTent = tent;
}
...
createMarketStalls() {
    const stallPositions = [
        { x: -18, z: -20, type: 'rug', color: 0xcc3333 },
        { x: 18, z: -20, type: 'pottery', color: 0x8b6914 },
        { x: -18, z: 5, type: 'lamp', color: 0xd4af37 },
        { x: 18, z: 5, type: 'spice', color: 0xff8800 }
    ];

    stallPositions.forEach(stall => {
        ...
        for (let i = 0; i < 3; i++) {
            const shelf = new THREE.Mesh(
                new THREE.BoxGeometry(3.5, 0.1, 0.5),
                new THREE.MeshStandardMaterial({ color: 0x8b7355 })
            );
            shelf.position.set(0, 0.5 + i * 0.7, 0);
            stallGroup.add(shelf);
            
            // Random goods on shelves
            for (let j = 0; j < 4; j++) {
                const item = this.createStallItem(stall.type);
                item.position.set(-1.5 + j, 0.6 + i * 0.7, 0);
                stallGroup.add(item);
            }
        }
        
        stallGroup.position.set(stall.x, 0, stall.z);
        this.rooms[0].add(stallGroup);
    });
}

createStallItem(type) {
    const materials = {
        rug: new THREE.MeshStandardMaterial({ color: Math.random() * 0xffffff, roughness: 0.8 }),
        pottery: new THREE.MeshStandardMaterial({ color: 0x8b7355, roughness: 0.6 }),
        lamp: new THREE.MeshStandardMaterial({ color: 0xffd700, metalness: 0.9, roughness: 0.2 }),
        spice: new THREE.MeshStandardMaterial({ color: Math.random() > 0.5 ? 0xff6600 : 0xffcc00 })
    };
    
    const geometries = {
        rug: new THREE.BoxGeometry(0.3, 0.1, 0.4),
        pottery: new THREE.CylinderGeometry(0.1, 0.15, 0.3, 8),
        lamp: new THREE.SphereGeometry(0.15, 8, 8),
        spice: new THREE.BoxGeometry(0.2, 0.15, 0.2)
    };
    
    return new THREE.Mesh(geometries[type], materials[type]);
}
// ========================================
...
createFountain() {
    ...
    fountain.add(water);
    fountain.userData.water = water;
    
    fountain.position.set(10, 0, 25);
...
createPrayerFlags() {
    ...
    const flagColors = [0xff3333, 0x3333ff, 0xffff33, 0x33ff33, 0xff33ff];
    ...
    for (let i = 0; i < 15; i++) {
        ...
        flag.userData.baseY = pos.y;
        flag.userData.phase = i * 0.5;
        this.rooms[0].add(flag);
        this.prayerFlags = this.prayerFlags || [];
        this.prayerFlags.push(flag);
    }
}
    ...
    setInterval(() => {
        if (Math.random() < 0.3) {
            ...
            setTimeout(() => {
                ...
                this.shootingStars = this.shootingStars.filter(s => s !== star);
            }, 3000);
        }
    }, 5000);
}
createBazaarStructure() {
  ...
  const floor = this.createRealisticGround();
...
createHangingLantern() {
  ...
  const brassMaterial = new THREE.MeshStandardMaterial({
    ...
    emissive: 0x332211,   // ✓ Subtle self-illumination
    ...
    // Glass panels
  const glassMaterial = new THREE.MeshPhysicalMaterial({ // ✓ Changed to Physical
    color: 0xffaa44,
    transparent: true,
    opacity: 0.7, // was 0.6
    transmission: 0.9, // ✓ NEW: Glass-like transmission
    thickness: 0.5,
    roughness: 0.1,
    metalness: 0.0,
    emissive: 0xff8800,
    emissiveIntensity: 0.8,
    clearcoat: 1.0, // ✓ NEW: Glossy coat
    ...
    // Point light
  const light = new THREE.PointLight(0xffaa44, 6, 20); // was 3, 15 - now BRIGHTER
...
createMerchant(type) {
    ...
    merchantGroup.userData.type = type;
...
showMerchantDialog(type) {
    const merchantDialogs = {
        carpet: {
            greeting: "🧙‍♂️ Carpet Merchant",
            messages: [
                "Ah, welcome traveler! These carpets are woven with ancient magic.",
                "Feel the silk threads - they carry the dreams of a thousand nights!",
                "This one here can fly... well, in your imagination at least!",
                "Each pattern tells a story of the desert winds."
            ],
            color: '#8b4513'
        },
        jewelry: {
            greeting: "💎 Jeweler",
            messages: [
                "Precious stones from the depths of the earth!",
                "This ruby was kissed by dragons, I swear!",
                "Gold and gems that shine brighter than the stars above.",
                "Each piece carries the blessing of the moon goddess."
            ],
            color: '#4b0082'
        },
        spice: {
            greeting: "🌶️ Spice Trader",
            messages: [
                "Spices from the four corners of the world!",
                "Saffron worth its weight in gold, only for you!",
                "This cinnamon will make your taste buds dance!",
                "Smell the cardamom - fresh from the eastern mountains!"
            ],
            color: '#b8860b'
        },
        pottery: {
            greeting: "🏺 Potter",
            messages: [
                "Clay vessels blessed by the river spirits!",
                "Each vase is unique, shaped by these very hands.",
                "This pottery keeps water cool even in the scorching sun.",
                "Ancient designs passed down for generations!"
            ],
            color: '#2f4f4f'
        },
        lamp: {
            greeting: "🪔 Lamp Merchant",
            messages: [
                "Magic lamps! Rub one and see what happens...",
                "These lamps hold the light of a thousand wishes!",
                "Brass so polished you can see your future in it!",
                "Legend says a genie lives in one of these... which one?"
            ],
            color: '#8b0000'
        }
    };
    
    const merchant = merchantDialogs[type] || merchantDialogs.carpet;
    const randomMessage = merchant.messages[Math.floor(Math.random() * merchant.messages.length)];
    ...
    modal.style.cssText = `
        ...
        background: linear-gradient(135deg, ${merchant.color} 0%, #1a1a1a 100%);
        color: #ffd700;
        padding: 30px;
        border-radius: 20px;
        max-width: 450px;
    ...
    modal.innerHTML = `
        <h2 style="margin: 0 0 20px 0; font-size: 24px;">${merchant.greeting}</h2>
        <p style="font-size: 18px; line-height: 1.6; font-style: italic; color: white;">"${randomMessage}"</p>
    ...
    console.log(`🗣️ ${merchant.greeting} spoke!`);
}
...
createRealisticGround() {
    ...
    // Create procedural sand texture (if no image available)
    ...
    const floorGeometry = new THREE.PlaneGeometry(60, 80, 100, 100);
...
updateBazaarAnimations() {
    ...
    if (this.fortuneTent) {
    const crystal = this.fortuneTent.userData.crystalBall;
    const light = this.fortuneTent.userData.light;
    
    // Rotating crystal ball
    crystal.rotation.y += 0.01;
    
    // Pulsing light
    light.intensity = 3 + Math.sin(time * 2) * 1.5;
    
    // Swirling colors
    const hue = (time * 0.1) % 1;
    crystal.material.emissive.setHSL(hue, 1, 0.5);
...
updateCinematicCamera() {
    if (!this.cinematicMode || !this.cinematicPath) return;
    
    this.cinematicProgress += 0.0005;
    if (this.cinematicProgress > 1) this.cinematicProgress = 0;

    const position = this.cinematicPath.getPoint(this.cinematicProgress);
    const lookAt = this.cinematicPath.getPoint((this.cinematicProgress + 0.01) % 1);
...
updateMiniMap() {
    ...
    this.playerMarker.position.set(this.camera.position.x, 0, this.camera.position.z);
    const dir = new THREE.Vector3();
    this.camera.getWorldDirection(dir);
    this.playerMarker.rotation.y = Math.atan2(dir.x, dir.z);
...
updateDustParticles() {
    ...
    for (let i = 0; i < positions.length; i += 3) {
        ...
        velocities[idx].x += (Math.random() - 0.5) * 0.001;
        velocities[idx].z += (Math.random() - 0.5) * 0.001;

        if (positions[i + 1] > 15) positions[i + 1] = 0;
        if (Math.abs(positions[i]) > 30) positions[i] = (Math.random() - 0.5) * 60;
        if (Math.abs(positions[i + 2]) > 40) positions[i + 2] = (Math.random() - 0.5) * 80;
    }
...
createAtmosphericScattering() {
    // God rays shader
    const godRayShader = new ShaderPass({
        uniforms: {
            tDiffuse: { value: null },
            fExposure: { value: 0.6 },
            fDecay: { value: 0.93 },
        ...
        fragmentShader: `
            ...
            void main() {
                vec2 texCoord = vUv;
                vec2 lightPos = vec2(0.5, 0.8); // Light source position
                
                vec2 deltaTexCoord = texCoord - lightPos;
                deltaTexCoord *= 1.0 / float(NUM_SAMPLES) * fDensity;
    ...
    this.composer.addPass(godRayShader);
}
updateFireflies() {
    // Animated via shader - no CPU update needed
}
...
animate() {
    ...
    this.updateLighting();
    this.updateBazaarAnimations();
    
    // NEW: Advanced updates
    ...
    if (this.fireflySystem) {
        this.fireflySystem.material.uniforms.time.value = this.time;
    }

    // REPLACE: Use composer instead of renderer
    this.composer.render(); // CHANGED FROM: this.renderer.render(this.scene, this.camera);
...
onCanvasClick(event) {
    ...
    if (timeSinceLastClick < this.clickDelay) {
        ...
        // Add merchants to interactive objects
        if (this.merchants) {
            this.merchants.forEach(merchant => {
                merchant.traverse(child => {
                    if (child.isMesh) {
        ...
        // Add fortune tent meshes
        const currentRoom = this.rooms[this.currentRoom || 0];
        if (currentRoom && this.fortuneTent) {
            this.fortuneTent.traverse(child => {
                if (child.isMesh) {
                    interactiveObjects.push(child);
                }
            });
        ...
        if (intersects.length > 0) {
            ...
            // ✅ Check if clicked on merchant
            if (obj.parent?.userData?.type) {
                const merchantType = obj.parent.userData.type;
                this.showMerchantDialog(merchantType);
                return;
            }
            
            // Check if clicked on fortune tent
            if (obj.parent?.userData?.isFortuneTent) {
                this.showFortune();
                return;
            }
            
            // Handle existing click interactions
            if (this.isFocused) {
                this.resetCamera();
                this.closeSlider();
...
// Define showFortune as a separate method
showFortune() {
    const fortunes = [
        "Your creativity will illuminate paths you never knew existed.",
        "A distant friend thinks of you under the same moon.",
        "The artwork you seek is closer than you think.",
        "Three wishes will manifest before the next full moon.",
        "Your journey through this bazaar is written in the stars.",
        "A hidden treasure awaits in the shadows of dawn.",
        "The merchant's words hold more truth than gold.",
        "Your destiny intertwines with ancient wisdom tonight."
    ];
    ...
    modal.style.cssText = `
        ...
        left: 50%;
        transform: translate(-50%, -50%);
        background: linear-gradient(135deg, #4b0082 0%, #9966ff 100%);
        color: #ffd700;
        padding: 30px;
        border-radius: 20px;
        max-width: 400px;
        ...
        animation: mysticalAppear 0.6s ease;
    `;
    
    modal.innerHTML = `
        <h2 style="margin: 0 0 20px 0; font-size: 24px;">🔮 Your Fortune 🔮</h2>
        <p style="font-size: 18px; line-height: 1.6; font-style: italic;">"${fortune}"</p>
        <button onclick="this.parentElement.remove()" style="
            margin-top: 20px;
            padding: 10px 20px;
            background: #ffd700;
