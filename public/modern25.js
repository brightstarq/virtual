...
createGallery() {
    ...
    // Polished marble floor with veining
    const marbleFloor = new THREE.MeshStandardMaterial({
        color: 0xf5f0e8,
        roughness: 0.25,
        metalness: 0.15,
        envMapIntensity: 1.2,
        map: new THREE.CanvasTexture(this.generateCarraraMarble(512, 512))
    });
    
    // Rich mahogany wood with grain
    ...
    // Polished championship gold (highly reflective)
    const championshipGold = new THREE.MeshStandardMaterial({
        color: 0xffd700,
        roughness: 0.12,
        metalness: 1.0,
        envMapIntensity: 2.0
    ...
    // Tarnished silver (aged, oxidized)
    const tarnishedSilver = new THREE.MeshStandardMaterial({
        color: 0xb8b8b8,
        roughness: 0.35,
        metalness: 0.85,
        envMapIntensity: 1.3
    ...
    // Aged bronze (green patina)
    const agedBronze = new THREE.MeshStandardMaterial({
        color: 0x6b5244,
        roughness: 0.7,
        metalness: 0.6,
        envMapIntensity: 0.9
    ...
    // Weathered brass
    const weatheredBrass = new THREE.MeshStandardMaterial({
        color: 0x8a7742,
        roughness: 0.55,
        metalness: 0.7,
        envMapIntensity: 1.1
    ...
    // Museum glass (ultra-clear, minimal tint)
    ...
    // Deep burgundy fabric (velvet-like)
    const burgundyVelvet = new THREE.MeshStandardMaterial({
        color: 0x6b1f1f,
        roughness: 0.85,
        metalness: 0.0,
        envMapIntensity: 0.4
    ...
    const ceramicTile = new THREE.MeshStandardMaterial({
        color: 0xe8e4d8,
        roughness: 0.3,
        metalness: 0.05
    });
    ...
    // Worn leather (benches)
    const wornLeather = new THREE.MeshStandardMaterial({
        color: 0x4a3228,
        roughness: 0.8,
        metalness: 0.0
    });
    ...
    // Club crest (navy blue with gold ring)
    const crestBase = new THREE.Mesh(
        new THREE.CylinderGeometry(4, 4, 0.08, 64),
    ...
    crestBase.position.y = 0.04;
    room1.add(crestBase);
    
    // Gold outer ring (polished)
    const crestRing = new THREE.Mesh(
        new THREE.TorusGeometry(4, 0.18, 16, 64),
        championshipGold
    );
    crestRing.rotation.x = Math.PI / 2;
    crestRing.position.y = 0.05;
    room1.add(crestRing);
    
    // Inner burgundy shield
    const shield = new THREE.Mesh(
        new THREE.CircleGeometry(2.5, 6),
        burgundyVelvet
    );
    ...
    // Championship stars (polished gold)
    for (let i = 0; i < 5; i++) {
        const star = new THREE.Mesh(
            new THREE.CylinderGeometry(0.15, 0.15, 0.04, 5),
            championshipGold
        );
        star.rotation.x = -Math.PI / 2;
        star.rotation.z = Math.PI / 10;
        star.position.set((i - 2) * 0.8, 0.06, -3);
        room1.add(star);
    }
    
    // Floor border (dark marble inlay)
    ...
    const borderPositions = [
        { w: hallWidth, d: 0.3, x: 0, z: -hallLength / 2 + 0.15 },
        { w: hallWidth, d: 0.3, x: 0, z: hallLength / 2 - 0.15 },
        { w: 0.3, d: hallLength, x: -hallWidth / 2 + 0.15, z: 0 },
        { w: 0.3, d: hallLength, x: hallWidth / 2 - 0.15, z: 0 }
    ];
    
    borderPositions.forEach(b => {
        const border = new THREE.Mesh(
            new THREE.PlaneGeometry(b.w, b.d),
            borderMaterial
        );
        border.rotation.x = -Math.PI / 2;
        border.position.set(b.x, 0.002, b.z);
        room1.add(border);
    });
    
    // ========================================
    // MAHOGANY WOOD-PANELED WALLS
    // ========================================
    
    const createWoodPanel = (width, height, x, y, z, rotationY = 0) => {
        ...
        for (let i = 0; i < 3; i++) {
            const trim = new THREE.Mesh(
                new THREE.BoxGeometry(width, 0.04, 0.03),
                weatheredBrass
            );
            trim.position.set(x, y - height / 2 + 1 + i * (height - 2) / 2, z + 0.08);
    ...
    // Back wall
    for (let x = -hallWidth / 2 + 3; x < hallWidth / 2; x += 6) {
        createWoodPanel(5.8, hallHeight - 2, x, hallHeight / 2, -hallLength / 2 + 0.5);
    }
    
    // Side walls
    for (let z = -hallLength / 2 + 5; z < hallLength / 2 - 10; z += 6) {
        createWoodPanel(0.15, hallHeight - 2, -hallWidth / 2 + 0.5, hallHeight / 2, z, Math.PI / 2);
        createWoodPanel(0.15, hallHeight - 2, hallWidth / 2 - 0.5, hallHeight / 2, z, Math.PI / 2);
    }
    
    // ========================================
    // REALISTIC PLAYER STATUE COLUMNS
    // ========================================
    
    const createLegendStatue = (x, z, jerseyNumber) => {
        // Polished marble pedestal
        const pedestal = new THREE.Mesh(
            new THREE.CylinderGeometry(1.2, 1.4, 1.5, 16),
            new THREE.MeshStandardMaterial({
                color: 0xf8f4e8,
                roughness: 0.2,
                metalness: 0.2,
                map: new THREE.CanvasTexture(this.generateCarraraMarble(256, 256))
            })
        );
        ...
        // Realistic bronze statue (more detailed)
        const bronzeMaterial = agedBronze;
        ...
        feet.position.set(x, 1.65, z);
        feet.castShadow = true;
        room1.add(feet);
        
        // Legs (more realistic proportions)
        const leftLeg = new THREE.Mesh(
            new THREE.CylinderGeometry(0.18, 0.15, 1.8, 12),
            bronzeMaterial
        );
        leftLeg.position.set(x - 0.2, 2.7, z);
        ...
        rightLeg.position.set(x + 0.2, 2.7, z);
        rightLeg.castShadow = true;
        room1.add(rightLeg);
        
        // Torso (athletic build)
        const torso = new THREE.Mesh(
            new THREE.CylinderGeometry(0.45, 0.35, 1.8, 12),
            bronzeMaterial
        );
        torso.position.set(x, 4.5, z);
        torso.castShadow = true;
        room1.add(torso);
        
        // Arms
        const leftArm = new THREE.Mesh(
            new THREE.CylinderGeometry(0.12, 0.1, 1.5, 10),
            bronzeMaterial
        );
        leftArm.position.set(x - 0.6, 4.3, z);
        ...
        rightArm.position.set(x + 0.6, 4.3, z);
        ...
        ball.position.set(x + 0.5, 1.52, z + 0.6);
        ball.castShadow = true;
        room1.add(ball);
        
        // Jersey number plaque (polished gold)
        const plaque = new THREE.Mesh(
            new THREE.BoxGeometry(0.9, 0.6, 0.08),
            championshipGold
        );
        plaque.position.set(x, 0.5, z + 1.5);
        plaque.castShadow = true;
        room1.add(plaque);
        
        // Support column to ceiling
        const column = new THREE.Mesh(
            new THREE.CylinderGeometry(0.5, 0.5, hallHeight - 6, 16),
            new THREE.MeshStandardMaterial({
                color: 0xe8dcc8,
                roughness: 0.5,
                map: new THREE.CanvasTexture(this.generateCarraraMarble(256, 256))
        ...
        column.position.set(x, hallHeight - 3, z);
        ...
        const capital = new THREE.Mesh(
            new THREE.CylinderGeometry(0.9, 0.5, 0.8, 16),
            polishedBrass
        );
        capital.position.set(x, hallHeight - 0.4, z);
        capital.castShadow = true;
        room1.add(capital);
        
        // Decorative rings on capital
        for (let i = 0; i < 3; i++) {
            const ring = new THREE.Mesh(
                new THREE.TorusGeometry(0.6 + i * 0.1, 0.03, 8, 16),
                weatheredBrass
            );
            ring.position.set(x, hallHeight - 0.6 + i * 0.15, z);
            ring.rotation.x = Math.PI / 2;
            room1.add(ring);
        }
        
        // Warm dramatic spotlight
        const statueLight = new THREE.SpotLight(0xffd9a3, 3.5, 14, Math.PI / 6, 0.7);
        statueLight.position.set(x, hallHeight - 1.5, z + 2.5);
        statueLight.target.position.set(x, 3, z);
        statueLight.castShadow = true;
        statueLight.shadow.mapSize.width = 2048;
        statueLight.shadow.mapSize.height = 2048;
        room1.add(statueLight);
        room1.add(statueLight.target);
    };
    
    // 8 legendary players
    const legendaryNumbers = [10, 7, 9, 11, 14, 6, 8, 4];
    
    for (let i = 0; i < 4; i++) {
        const z = -hallLength / 2 + 10 + i * 12;
        createLegendStatue(-12, z, legendaryNumbers[i]);
        createLegendStatue(12, z, legendaryNumbers[i + 4]);
    }
    
    // ========================================
    // MUSEUM-QUALITY TROPHY CASES
    // ========================================
    
    const createTrophyDisplay = (x, z, trophyType) => {
        // Polished brass base
        const displayBase = new THREE.Mesh(
            new THREE.BoxGeometry(2.8, 0.2, 1.8),
            polishedBrass
        );
        displayBase.position.set(x, 1.1, z);
        displayBase.castShadow = true;
        room1.add(displayBase);
        
        // Ultra-clear museum glass enclosure
        const glassCase = new THREE.Mesh(
            new THREE.BoxGeometry(2.6, 2.8, 1.6),
            museumGlass
        );
        glassCase.position.set(x, 2.5, z);
        glassCase.castShadow = true;
        glassCase.receiveShadow = true;
        room1.add(glassCase);
        
        // Brass frame edges (refined)
        const frameEdges = [
            { w: 2.6, h: 0.06, d: 0.06, y: 3.9, zOffset: 0.8 },
            { w: 2.6, h: 0.06, d: 0.06, y: 3.9, zOffset: -0.8 },
            { w: 2.6, h: 0.06, d: 0.06, y: 1.1, zOffset: 0.8 },
            { w: 2.6, h: 0.06, d: 0.06, y: 1.1, zOffset: -0.8 }
        ];
        
        frameEdges.forEach(edge => {
            const frame = new THREE.Mesh(
                new THREE.BoxGeometry(edge.w, edge.h, edge.d),
                polishedBrass
            );
            frame.position.set(x, edge.y, z + edge.zOffset);
            room1.add(frame);
        });
        
        // Realistic trophy (detailed geometry)
        let trophyMaterial, trophyColor;
        if (trophyType === 'gold') {
            trophyMaterial = championshipGold;
            trophyColor = 0xffd700;
        } else if (trophyType === 'silver') {
            trophyMaterial = tarnishedSilver;
            trophyColor = 0xc0c0c0;
        } else {
            trophyMaterial = new THREE.MeshStandardMaterial({
                color: 0xcd7f32,
                roughness: 0.4,
                metalness: 0.8,
                envMapIntensity: 1.2
            });
            trophyColor = 0xcd7f32;
        }
        
        // Trophy base (multi-tier)
        const trophyBase1 = new THREE.Mesh(
            new THREE.CylinderGeometry(0.5, 0.55, 0.15, 16),
            trophyMaterial
        );
        trophyBase1.position.set(x, 1.8, z);
        trophyBase1.castShadow = true;
        room1.add(trophyBase1);
        
        const trophyBase2 = new THREE.Mesh(
            new THREE.CylinderGeometry(0.4, 0.5, 0.2, 16),
            trophyMaterial
        );
        trophyBase2.position.set(x, 2.0, z);
        trophyBase2.castShadow = true;
        room1.add(trophyBase2);
        ...
        // Trophy cup (realistic proportions)
        const cup = new THREE.Mesh(
            new THREE.CylinderGeometry(0.4, 0.25, 1.0, 20),
            trophyMaterial
        );
        cup.position.set(x, 3.3, z);
        cup.castShadow = true;
        room1.add(cup);
        
        // Trophy handles (ornate)
        const handleLeft = new THREE.Mesh(
            new THREE.TorusGeometry(0.2, 0.04, 10, 16, Math.PI),
            trophyMaterial
        );
        handleLeft.position.set(x - 0.4, 3.3, z);
        handleLeft.rotation.z = -Math.PI / 2;
        room1.add(handleLeft);
        
        const handleRight = new THREE.Mesh(
            new THREE.TorusGeometry(0.2, 0.04, 10, 16, Math.PI),
            trophyMaterial
        );
        handleRight.position.set(x + 0.4, 3.3, z);
        handleRight.rotation.z = Math.PI / 2;
        room1.add(handleRight);
        
        // Championship ball on top
        const topBall = new THREE.Mesh(
            new THREE.SphereGeometry(0.15, 16, 16),
            trophyMaterial
        );
        topBall.position.set(x, 3.9, z);
        topBall.castShadow = true;
        room1.add(topBall);
        
        // Intense spotlight (museum lighting)
        const trophySpot = new THREE.SpotLight(trophyColor, 5, 10, Math.PI / 7, 0.4);
        trophySpot.position.set(x, 6, z);
        trophySpot.target.position.set(x, 2.5, z);
        trophySpot.castShadow = true;
        trophySpot.shadow.mapSize.width = 2048;
        trophySpot.shadow.mapSize.height = 2048;
        room1.add(trophySpot);
        room1.add(trophySpot.target);
        
        // Descriptive plaque
        const namePlaque = new THREE.Mesh(
            new THREE.BoxGeometry(2.2, 0.35, 0.06),
            new THREE.MeshStandardMaterial({
                color: 0x1a1410,
                roughness: 0.4
            })
        );
        namePlaque.position.set(x, 0.95, z + 0.9);
        room1.add(namePlaque);
        
        // Gold text plate on plaque
        const textPlate = new THREE.Mesh(
            new THREE.BoxGeometry(2.0, 0.25, 0.02),
            championshipGold
        );
        textPlate.position.set(x, 0.95, z + 0.93);
        room1.add(textPlate);
    };
    
    // Trophy cases with variety
    const trophyTypes = ['gold', 'gold', 'silver', 'gold', 'bronze', 'gold'];
    for (let i = 0; i < 6; i++) {
        const z = -hallLength / 2 + 12 + i * 8;
        createTrophyDisplay(-6, z, trophyTypes[i]);
        createTrophyDisplay(6, z, trophyTypes[i]);
    }
    
    // ========================================
    // SACRED TURF UNDER GLASS
    // ========================================
    
    const turfSoil = new THREE.Mesh(
    ...
    turfSoil.position.set(0, 0.125, hallLength / 2 - 10);
    turfSoil.receiveShadow = true;
    room1.add(turfSoil);
    
    // Grass surface (realistic green)
    ...
    grassSurface.rotation.x = -Math.PI / 2;
    grassSurface.position.set(0, 0.26, hallLength / 2 - 10);
    grassSurface.receiveShadow = true;
    room1.add(grassSurface);
    ...
    for (let i = 0; i < 300; i++) {
        ...
        blade.position.set(
            (Math.random() - 0.5) * 5.8,
            0.36,
            hallLength / 2 - 10 + (Math.random() - 0.5) * 3.8
        );
    ...
    // Museum-grade glass dome
    const turfGlass = new THREE.Mesh(
        new THREE.BoxGeometry(6.5, 1.8, 4.5),
        museumGlass
    );
    turfGlass.position.set(0, 1.15, hallLength / 2 - 10);
    turfGlass.castShadow = true;
    turfGlass.receiveShadow = true;
    room1.add(turfGlass);
    
    // Polished brass frame
    const domeFrame = new THREE.Mesh(
        new THREE.BoxGeometry(6.5, 0.1, 4.5),
        polishedBrass
    );
    domeFrame.position.set(0, 2.05, hallLength / 2 - 10);
    room1.add(domeFrame);
    
    // Commemorative plaque
    const turfPlaque = new THREE.Mesh(
    ...
    turfPlaque.position.set(0, 0.6, hallLength / 2 - 12.5);
    room1.add(turfPlaque);
    
    // Gold text plate
    const turfText = new THREE.Mesh(
        new THREE.BoxGeometry(3.2, 0.35, 0.02),
        championshipGold
    );
    turfText.position.set(0, 0.6, hallLength / 2 - 12.46);
    room1.add(turfText);
    
    // Dramatic turf lighting
    const turfLight = new THREE.SpotLight(0xffe6a3, 6, 12, Math.PI / 5, 0.8);
    turfLight.position.set(0, 7, hallLength / 2 - 10);
    turfLight.target.position.set(0, 0.25, hallLength / 2 - 10);
    turfLight.castShadow = true;
    turfLight.shadow.mapSize.width = 2048;
    turfLight.shadow.mapSize.height = 2048;
    room1.add(turfLight);
    room1.add(turfLight.target);
    
    // ========================================
    // CHAMPIONSHIP BANNERS
    // ========================================
    
    const createChampionshipBanner = (x, z, year) => {
        // Burgundy velvet fabric
        const bannerFabric = new THREE.Mesh(
            new THREE.PlaneGeometry(1.8, 3.5),
            burgundyVelvet
        );
        bannerFabric.position.set(x, hallHeight - 2, z);
        bannerFabric.castShadow = true;
        bannerFabric.receiveShadow = true;
        room1.add(bannerFabric);
        
        // Gold border trim (top)
        const borderTop = new THREE.Mesh(
            new THREE.BoxGeometry(1.8, 0.1, 0.03),
            championshipGold
        );
        borderTop.position.set(x, hallHeight - 0.25, z + 0.02);
        room1.add(borderTop);
        
        // Gold border trim (bottom)
        const borderBottom = new THREE.Mesh(
            new THREE.BoxGeometry(1.8, 0.1, 0.03),
            championshipGold
        );
        borderBottom.position.set(x, hallHeight - 3.75, z + 0.02);
        room1.add(borderBottom);
        
        // Brass hanging chain
        const chain = new THREE.Mesh(
            new THREE.CylinderGeometry(0.025, 0.025, 2.5, 10),
            weatheredBrass
        );
        chain.position.set(x, hallHeight - 0.75, z);
        room1.add(chain);
        
        // Gold year medallion
        const yearMedallion = new THREE.Mesh(
            new THREE.CylinderGeometry(0.6, 0.6, 0.08, 32),
            championshipGold
        );
        yearMedallion.rotation.x = Math.PI / 2;
        yearMedallion.position.set(x, hallHeight - 2, z + 0.05);
        yearMedallion.castShadow = true;
        room1.add(yearMedallion);
        
        // Soft banner light
        const bannerLight = new THREE.PointLight(0xffd9a3, 1.2, 6);
        bannerLight.position.set(x, hallHeight - 2, z + 1);
        room1.add(bannerLight);
    };
    ...
    championships.forEach(c => {
        createChampionshipBanner(c.x, c.z, c.year);
    });
    
    // ========================================
    // VINTAGE LOCKER ROOM SECTION
    // ========================================
    
    const lockerRoomZ = hallLength / 2 - 15;
    
    // Ceramic tile walls
    for (let y = 0; y < hallHeight - 2; y += 0.3) {
        for (let x = -hallWidth / 2 + 1; x < hallWidth / 2; x += 0.3) {
            const tile = new THREE.Mesh(
                new THREE.PlaneGeometry(0.28, 0.28),
                ceramicTile
            );
            tile.position.set(x, y + 0.15, lockerRoomZ);
            tile.receiveShadow = true;
            room1.add(tile);
        }
    }
    
    // Wooden benches with leather padding
    for (let x = -10; x <= 10; x += 5) {
        ...
        benchBase.position.set(x, 0.5, lockerRoomZ + 2);
        ...
        const cushion = new THREE.Mesh(
            new THREE.BoxGeometry(3.8, 0.15, 0.48),
            wornLeather
        );
        cushion.position.set(x, 0.675, lockerRoomZ + 2);
        ...
        [-1.8, 1.8].forEach(offset => {
            ...
            leg.position.set(x + offset, 0.25, lockerRoomZ + 2);
            leg.castShadow = true;
            room1.add(leg);
        });
    }
    
    // Metal lockers (weathered)
    const lockerMetal = new THREE.MeshStandardMaterial({
        color: 0x5a6a7a,
        roughness: 0.6,
        metalness: 0.7
    });
    ...
    for (let x = -12; x <= 12; x += 3) {
        const locker = new THREE.Mesh(
            new THREE.BoxGeometry(2.5, 3, 0.6),
            lockerMetal
        );
        locker.position.set(x, 1.5, lockerRoomZ - 0.3);
        ...
        const handle = new THREE.Mesh(
            new THREE.CylinderGeometry(0.04, 0.04, 0.35, 10),
            weatheredBrass
        );
        handle.rotation.z = Math.PI / 2;
        handle.position.set(x + 1, 1.5, lockerRoomZ + 0.05);
        room1.add(handle);
    }
    ...
    const pressWall = new THREE.Mesh(
        new THREE.PlaneGeometry(hallWidth - 4, 4.5),
        navyFabric
    );
    pressWall.position.set(0, 2.25, -hallLength / 2 + 2);
    pressWall.receiveShadow = true;
    room1.add(pressWall);
    
    // Sponsor logos (realistic)
    const sponsorLogos = [
        { x: -10, color: 0xffffff }, // White
        { x: -5, color: 0xff0000 },  // Red
        { x: 0, color: 0xffd700 },   // Gold
        { x: 5, color: 0x00ff00 },   // Green
        { x: 10, color: 0xffffff }   // White
    ];
    ...
    sponsorLogos.forEach(s => {
        ...
        logo.position.set(s.x, 2.25, -hallLength / 2 + 2.05);
        room1.add(logo);
    });
    
    // Press table
    const pressTable = new THREE.Mesh(
        new THREE.BoxGeometry(12, 0.25, 2.5),
        mahoganyWood
    );
    pressTable.position.set(0, 1.125, -hallLength / 2 + 6);
    pressTable.castShadow = true;
    pressTable.receiveShadow = true;
    room1.add(pressTable);
    ...
    // Brass molding
    const ceilingMolding = new THREE.Mesh(
        new THREE.BoxGeometry(hallWidth - 2, 0.35, 0.35),
        weatheredBrass
    );
    ceilingMolding.position.set(0, hallHeight - 0.175, 0);
    room1.add(ceilingMolding);
    ...
    for (let x = -12; x <= 12; x += 6) {
        for (let z = -hallLength / 2 + 8; z < hallLength / 2 - 10; z += 8) {
            spotlightPositions.push({ x, z });
        }
    }
    ...
    spotlightPositions.forEach(pos => {
        ...
        fixture.position.set(pos.x, hallHeight - 0.25, pos.z);
        room1.add(fixture);
        
        // Warm spotlight
        const spot = new THREE.SpotLight(0xffd9a3, 2.5, 18, Math.PI / 6.5, 0.65);
        spot.position.set(pos.x, hallHeight - 0.3, pos.z);
    ...
    // Artwork wall washers
    for (let z = -hallLength / 2 + 6; z < hallLength / 2 - 12; z += 6) {
        // Left wall
        const leftWash = new THREE.SpotLight(0xffeec2, 2.0, 14, Math.PI / 7, 0.75);
        leftWash.position.set(-14, hallHeight - 2, z);
        leftWash.target.position.set(-hallWidth / 2 + 1, 4, z);
        leftWash.castShadow = true;
        room1.add(leftWash);
        room1.add(leftWash.target);
        
        // Right wall
        const rightWash = new THREE.SpotLight(0xffeec2, 2.0, 14, Math.PI / 7, 0.75);
        rightWash.position.set(14, hallHeight - 2, z);
        rightWash.target.position.set(hallWidth / 2 - 1, 4, z);
        rightWash.castShadow = true;
        room1.add(rightWash);
        room1.add(rightWash.target);
    }
    ...
    // Dim ambient (museum lighting)
    const ambient = new THREE.AmbientLight(0xfff5e6, 0.3);
    this.scene.add(ambient);
    
    // Warm hemisphere
    const hemi = new THREE.HemisphereLight(0xffeec2, 0xe8dcc8, 0.4);
    hemi.position.set(0, hallHeight, 0);
    this.scene.add(hemi);
    
    // Main directional (window light simulation)
    const mainLight = new THREE.DirectionalLight(0xffd9a3, 0.6);
    mainLight.position.set(18, hallHeight - 2, 22);
    mainLight.castShadow = true;
    mainLight.shadow.mapSize.width = 2048;
    mainLight.shadow.mapSize.height = 2048;
    mainLight.shadow.camera.near = 0.5;
    ...
    // Warm, reverent atmosphere
    this.scene.background = new THREE.Color(0x2a2420);
    this.scene.fog = new THREE.Fog(0x3a3430, 40, 70);
    
    room1.position.set(0, 0, 0);
    this.rooms.push(room1);
    this.rooms.forEach(room => this.scene.add(room));
    
    console.log("⚽ Photorealistic Football Museum created");
}
