...
createGallery() {
    // ========================================
    // 1950s DINER GALLERY SYSTEM
    // ========================================
    
    this.dinerAnimations = [];
    this.neonSigns = [];
    this.jukeboxLights = [];
    
    this.createDinerRoom();
    this.createJukebox();
    this.createNeonSigns();
    this.createDinerUI();
    
    console.log("🍔 1950s Diner Gallery created!");
}
...
createDinerRoom() {
    const dinerRoom = new THREE.Group();
    dinerRoom.visible = true;
    ...
    const formicaMaterial = new THREE.MeshStandardMaterial({
        color: 0xffffff,
        roughness: 0.2,
        metalness: 0.3
    });
    ...
    const tilesZ = Math.ceil(roomLength / tileSize);
    ...
    for (let x = 0; x < tilesX; x++) {
        for (let z = 0; z < tilesZ; z++) {
            ...
            tile.position.set(
                -roomWidth/2 + x * tileSize + tileSize/2,
                0,
                -roomLength/2 + z * tileSize + tileSize/2
            );
    ...
    // Wall material (mint green/cream)
    ...
    const wallPositions = [
        { pos: [0, wallHeight/2, -roomLength/2], rot: [0, 0, 0], size: [roomWidth, wallHeight, 0.5] },
        { pos: [0, wallHeight/2, roomLength/2], rot: [0, Math.PI, 0], size: [roomWidth, wallHeight, 0.5] },
        { pos: [-roomWidth/2, wallHeight/2, 0], rot: [0, Math.PI/2, 0], size: [roomLength, wallHeight, 0.5] },
        { pos: [roomWidth/2, wallHeight/2, 0], rot: [0, -Math.PI/2, 0], size: [roomLength, wallHeight, 0.5] }
    ];
    ...
    wallPositions.forEach(wall => {
        ...
        dinerRoom.add(wallMesh);
        
        // Chrome trim at bottom
        const trim = new THREE.Mesh(
            new THREE.BoxGeometry(wall.size[0], 0.3, 0.6),
            chromeMaterial
        );
        trim.position.set(...wall.pos);
        trim.position.y = 1;
        trim.rotation.set(...wall.rot);
        dinerRoom.add(trim);
    });
    ...
    dinerRoom.add(ceiling);
    
    // ========================================
    // COUNTER WITH CHROME STOOLS
    // ========================================
    ...
    const counterBase = new THREE.Mesh(
        new THREE.BoxGeometry(counterLength, counterHeight, counterDepth),
        new THREE.MeshStandardMaterial({
            color: 0x8b4513,
            roughness: 0.6
        })
    );
    counterBase.position.set(0, counterHeight/2, -15);
    counterBase.castShadow = true;
    dinerRoom.add(counterBase);
    
    // Counter top (white formica)
    const counterTop = new THREE.Mesh(
        new THREE.BoxGeometry(counterLength + 0.5, 0.2, counterDepth + 1),
        formicaMaterial
    );
    counterTop.position.set(0, counterHeight, -15);
    counterTop.castShadow = true;
    dinerRoom.add(counterTop);
    
    // Chrome edge
    const chromeEdge = new THREE.Mesh(
        new THREE.BoxGeometry(counterLength + 0.6, 0.15, 0.2),
        chromeMaterial
    );
    chromeEdge.position.set(0, counterHeight, -15 + counterDepth/2 + 0.5);
    dinerRoom.add(chromeEdge);
    
    // Chrome stools (8 stools)
    for (let i = 0; i < 8; i++) {
        ...
        piping.position.y = 2.65;
        piping.rotation.x = Math.PI / 2;
        stoolGroup.add(piping);
        ...
        footrest.position.y = 1;
        footrest.rotation.x = Math.PI / 2;
        stoolGroup.add(footrest);
        
        stoolGroup.position.set(
            -counterLength/2 + 2 + i * 3.5,
            0,
            -15 + counterDepth/2 + 2
        );
        dinerRoom.add(stoolGroup);
    }
    
    // ========================================
    // RED VINYL BOOTHS (6 booths along walls)
    // ========================================
    
    const boothPositions = [
        { x: -16, z: -20, rot: Math.PI/2 },
        { x: -16, z: -10, rot: Math.PI/2 },
    ...
    boothPositions.forEach(booth => {
        const boothGroup = new THREE.Group();
        
        // Table top (white formica)
        ...
        const tableEdge = new THREE.Mesh(
            new THREE.BoxGeometry(4.1, 0.1, 2.1),
            chromeMaterial
        );
        tableEdge.position.y = 2.4;
        boothGroup.add(tableEdge);
        
        // Table leg (chrome pedestal)
        const tableLeg = new THREE.Mesh(
            new THREE.CylinderGeometry(0.3, 0.5, 2.5, 16),
            chromeMaterial
        );
        tableLeg.position.y = 1.25;
        boothGroup.add(tableLeg);
        
        // Booth seats (red vinyl)
        [-1.2, 1.2].forEach(zOffset => {
            // Seat cushion
            const seat = new THREE.Mesh(
                new THREE.BoxGeometry(3.5, 0.4, 1.5),
                vinylMaterial
            );
            seat.position.set(0, 1.5, zOffset);
            boothGroup.add(seat);
            ...
            backrest.position.set(0, 2.5, zOffset + (zOffset > 0 ? 0.6 : -0.6));
            boothGroup.add(backrest);
            
            // Chrome piping on seat
            const seatPiping = new THREE.Mesh(
                new THREE.BoxGeometry(3.6, 0.05, 1.6),
                chromeMaterial
            );
            seatPiping.position.set(0, 1.7, zOffset);
            boothGroup.add(seatPiping);
        });
        
        // Condiments holder (chrome)
        const condiments = new THREE.Mesh(
            new THREE.BoxGeometry(0.8, 0.5, 0.6),
            chromeMaterial
        );
        condiments.position.set(0, 2.85, 0);
        boothGroup.add(condiments);
        ...
        [-0.25, 0.25].forEach(xOff => {
            ...
            shaker.position.set(xOff, 3.15, 0);
            boothGroup.add(shaker);
        });
        
        boothGroup.position.set(booth.x, 0, booth.z);
        boothGroup.rotation.y = booth.rot;
        dinerRoom.add(boothGroup);
    });
    
    // ========================================
    // ROTATING PIE DISPLAY CASE
    // ========================================
    
    this.pieDisplay = new THREE.Group();
    ...
    glassDome.position.y = 4.5;
    this.pieDisplay.add(glassDome);
    
    // Chrome base
    const displayBase = new THREE.Mesh(
        new THREE.CylinderGeometry(1.8, 2, 0.5, 32),
        chromeMaterial
    );
    displayBase.position.y = 3.5;
    this.pieDisplay.add(displayBase);
    ...
    this.pieDisplay.add(platform);
    this.pieDisplay.userData.platform = platform;
    
    // 6 pie slices
    const pieColors = [0xff6347, 0xffd700, 0x8b4513, 0x90ee90, 0xff69b4, 0xffa500];
    for (let i = 0; i < 6; i++) {
        ...
        this.pieDisplay.add(pie);
    }
    
    this.pieDisplay.position.set(-8, 0, -15);
    this.pieDisplay.userData.rotationSpeed = 0.003;
    dinerRoom.add(this.pieDisplay);
    ...
    displayWindow.position.set(0, 4.5, 0.6);
    registerGroup.add(displayWindow);
    ...
    registerGroup.position.set(8, 0, -15);
    dinerRoom.add(registerGroup);
    ...
    // Machine body (red)
    const cokeMachine = new THREE.Mesh(
    ...
    cokeMachine.position.y = 2.5;
    cokeGroup.add(cokeMachine);
    
    // Chrome trim
    const cokeChrome = new THREE.Mesh(
        new THREE.BoxGeometry(2.1, 0.3, 2.1),
        chromeMaterial
    );
    cokeChrome.position.y = 5.1;
    cokeGroup.add(cokeChrome);
    
    // Coca-Cola logo panel
    ...
    logoPanel.position.set(0, 4, 1.01);
    cokeGroup.add(logoPanel);
    ...
    coinSlot.position.set(0, 3, 1.02);
    cokeGroup.add(coinSlot);
    
    // Dispensing door (chrome)
    const dispenseDoor = new THREE.Mesh(
        new THREE.BoxGeometry(1, 0.8, 0.2),
        chromeMaterial
    );
    dispenseDoor.position.set(0, 1, 1.05);
    cokeGroup.add(dispenseDoor);
    
    cokeGroup.position.set(-15, 0, 15);
    dinerRoom.add(cokeGroup);
    ...
    const milkshakeGroup = new THREE.Group();
    
    // Base
    const milkshakeBase = new THREE.Mesh(
        new THREE.CylinderGeometry(0.5, 0.6, 1, 16),
        chromeMaterial
    );
    milkshakeBase.position.y = 4;
    milkshakeGroup.add(milkshakeBase);
    
    // Glass containers (3)
    [-0.6, 0, 0.6].forEach(xOff => {
        const container = new THREE.Mesh(
            new THREE.CylinderGeometry(0.25, 0.25, 1.5, 16),
            new THREE.MeshPhysicalMaterial({
                color: 0xffffff,
                transparent: true,
                opacity: 0.3,
        ...
        container.position.set(xOff, 5.25, 0);
        milkshakeGroup.add(container);
        
        // Milkshake inside (random colors)
        const colors = [0xffb6c1, 0x8b4513, 0xfffacd];
        const milkshake = new THREE.Mesh(
            new THREE.CylinderGeometry(0.23, 0.23, 1.2, 16),
            new THREE.MeshStandardMaterial({
                color: colors[Math.floor(xOff + 1)],
                roughness: 0.3
            })
        );
        milkshake.position.set(xOff, 4.65, 0);
        milkshakeGroup.add(milkshake);
    });
    
    milkshakeGroup.position.set(5, 0, -15);
    dinerRoom.add(milkshakeGroup);
    
    // ========================================
    // ARTWORK DISPLAY POSITIONS (Retro Posters)
    // ========================================
    
    this.dinerArtworkSpots = [
        // Back wall
        { x: -10, y: 6, z: -24, rot: 0 },
        { x: -5, y: 6, z: -24, rot: 0 },
        { x: 0, y: 6, z: -24, rot: 0 },
        ...
        { x: 10, y: 6, z: -24, rot: 0 },
        // Side walls
        { x: -18, y: 6, z: 10, rot: Math.PI/2 },
        { x: -18, y: 6, z: 15, rot: Math.PI/2 },
        { x: 18, y: 6, z: 10, rot: -Math.PI/2 },
        { x: 18, y: 6, z: 15, rot: -Math.PI/2 }
    ];
    
    dinerRoom.position.set(0, 0, 0);
    dinerRoom.userData.theme = 'diner';
    this.rooms.push(dinerRoom);
    this.scene.add(dinerRoom);
}
...
createJukebox() {
    ...
    const arch = new THREE.Mesh(
        new THREE.CylinderGeometry(1.2, 1.2, 0.5, 32, 1, false, 0, Math.PI),
        new THREE.MeshStandardMaterial({
            color: 0xffd700,
            roughness: 0.2,
            metalness: 0.9,
    ...
    arch.position.set(0, 4.25, 0);
    arch.rotation.z = Math.PI / 2;
    jukeboxGroup.add(arch);
    ...
    glassPanel.position.set(0, 2.5, 1.05);
    jukeboxGroup.add(glassPanel);
    
    // Vinyl record inside (spinning)
    ...
    record.position.set(0, 2.5, 0.8);
    jukeboxGroup.add(record);
    jukeboxGroup.userData.record = record;
    ...
    label.position.set(0, 2.5, 0.85);
    jukeboxGroup.add(label);
    
    // Selection buttons (chrome)
    for (let row = 0; row < 3; row++) {
    ...
    coinSlot.position.set(0.8, 3, 1.05);
    jukeboxGroup.add(coinSlot);
    
    // Decorative lights (neon strips)
    this.jukeboxLights = [];
    const lightColors = [0xff00ff, 0x00ffff, 0xff0000, 0x00ff00, 0xffff00];
    ...
    for (let i = 0; i < 5; i++) {
        ...
        neonStrip.position.set(-1 + i * 0.5, 2.5, 1.05);
        jukeboxGroup.add(neonStrip);
        
        const neonLight = new THREE.PointLight(lightColors[i], 1, 3);
        neonLight.position.copy(neonStrip.position);
        neonLight.position.z += 0.2;
        jukeboxGroup.add(neonLight);
        
        this.jukeboxLights.push({
            strip: neonStrip,
            light: neonLight,
            baseColor: lightColors[i],
            phase: i * (Math.PI * 2 / 5)
        });
    }
    
    jukeboxGroup.position.set(15, 0, 18);
    jukeboxGroup.userData.isJukebox = true;
    jukeboxGroup.userData.rotationSpeed = 0.02;
    this.rooms[0].add(jukeboxGroup);
    this.jukebox = jukeboxGroup;
}
...
createNeonSigns() {
    ...
    // "DINER" sign
    const dinerSign = this.createNeonText('DINER', 0xff00ff, 4);
    dinerSign.position.set(0, 10, -24.5);
    this.rooms[0].add(dinerSign);
    this.neonSigns.push({
        group: dinerSign,
        color: 0xff00ff,
        flickerSpeed: 2
    ...
    // "OPEN 24 HOURS" sign
    const openSign = this.createNeonText('OPEN', 0x00ff00, 2);
    openSign.position.set(-15, 8, 24.5);
    openSign.rotation.y = Math.PI;
    this.rooms[0].add(openSign);
    this.neonSigns.push({
        group: openSign,
        color: 0x00ff00,
        flickerSpeed: 3
    ...
    // "SHAKES" sign
    const shakesSign = this.createNeonText('SHAKES', 0xff0000, 2);
    shakesSign.position.set(-19, 8, 0);
    shakesSign.rotation.y = Math.PI/2;
    this.rooms[0].add(shakesSign);
    this.neonSigns.push({
        group: shakesSign,
        color: 0xff0000,
        flickerSpeed: 2.5
...
createNeonText(text, color, scale = 1) {
    const group = new THREE.Group();
    
    // Create simple box letters
    const letterWidth = 0.8 * scale;
    const letterHeight = 1.2 * scale;
    const letterDepth = 0.2 * scale;
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        letter.position.x = (i - text.length / 2) * spacing;
        group.add(letter);
        
        // Glow light
        const light = new THREE.PointLight(color, 3, 10);
        light.position.copy(letter.position);
        light.position.z += 0.5;
        group.add(light);
    }
    
    group.userData.lights = group.children.filter(c => c instanceof THREE.PointLight);
...
createDinerUI() {
    const dinerUI = document.createElement('div');
    dinerUI.id = 'dinerUI';
    dinerUI.style.cssText = `
        position: fixed;
        top: 20px;
        left: 50%;
    ...
    dinerUI.innerHTML = `
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                <div style="font-size: 22px; letter-spacing: 2px;">RETRO DINER</div>
                <div style="font-size: 12px; opacity: 0.9;">Est. 1955 • Open 24 Hours</div>
            </div>
            <span style="font-size: 28px;">🎵</span>
        </div>
    `;
    
    document.body.appendChild(dinerUI);
}
// ========================================
...
updateDinerAnimations() {
    const time = this.time || Date.now() * 0.001;
    
    // 1. ROTATING PIE DISPLAY
    if (this.pieDisplay) {
        this.pieDisplay.rotation.y += this.pieDisplay.userData.rotationSpeed;
    }
    
    // 2. JUKEBOX VINYL SPINNING
    if (this.jukebox && this.jukebox.userData.record) {
        this.jukebox.userData.record.rotation.z += this.jukebox.userData.rotationSpeed;
    }
    
    // 3. JUKEBOX NEON LIGHTS SEQUENCING
    if (this.jukeboxLights) {
        this.jukeboxLights.forEach((light, index) => {
            const pulse = Math.sin(time * 3 + light.phase) * 0.5 + 0.5;
            light.strip.material.emissiveIntensity = 0.5 + pulse * 0.5;
            light.light.intensity = 0.5 + pulse * 1.5;
        });
    }
    
    // 4. NEON SIGN FLICKERING
    if (this.neonSigns) {
        this.neonSigns.forEach((sign, index) => {
            const flicker = Math.sin(time * sign.flickerSpeed + index * 0.5);
            
            // Occasional power surge/dropout
            if (Math.random() < 0.005) {
                sign.group.userData.meshes.forEach(mesh => {
                    mesh.material.emissiveIntensity = 0;
                });
                sign.group.userData.lights.forEach(light => {
                    light.intensity = 0;
                });
            } else {
                sign.group.userData.meshes.forEach(mesh => {
                    mesh.material.emissiveIntensity = 0.8 + flicker * 0.2;
                });
                sign.group.userData.lights.forEach(light => {
                    light.intensity = 2 + flicker * 1;
                });
            }
        });
    }
    
    // 5. CEILING LIGHTS SUBTLE FLICKER
    if (this.ceilingLights) {
        this.ceilingLights.forEach((ceiling, index) => {
            const flicker = Math.sin(time * 10 + index) * 0.05 + 0.95;
            ceiling.light.intensity = ceiling.baseIntensity * flicker;
            ceiling.tube.material.emissiveIntensity = 0.9 + flicker * 0.1;
        });
    }
}
...
animate() {
    ...
    this.updateLighting();
    this.updateDinerAnimations(); // ✓ ADD THIS LINE
...
checkCollisions() {
    if (!this.isMobile) {
        ...
        // Sky Islands bounds (prevent falling off main island)
        const minX = -13;
        const maxX = 13;
        const minZ = -13;
        const maxZ = 13;
        const minY = -5; // Safety net (respawn if falling)
